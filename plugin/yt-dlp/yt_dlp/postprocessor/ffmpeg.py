...
class FFmpegPostProcessor(PostProcessor):
    ...
    def _get_ffmpeg_version(self, prog):
        ...
        mobj = re.search(
            r"(?m)^\s+libavformat\s+(?:[0-9. ]+)\s+/\s+(?P<runtime>[0-9. ]+)", out
        )
    ...
    def _versions(self):
        return filter_dict(
            {self.basename: self._version, self.probe_basename: self._probe_version}
        )
    ...
    def _get_version(self, kind):
        ...
        basename, version, features = next(
            filter(
                lambda x: x[1], ((p, *self._get_ffmpeg_version(p)) for p in executables)
            ),
            (None, None, {}),
        )
        if kind == "ffmpeg":
...
class FFmpegMetadataPP(FFmpegPostProcessor):
    ...
    def __init__(
        self, downloader, add_metadata=True, add_chapters=True, add_infojson="if_exists"
    ):
        FFmpegPostProcessor.__init__(self, downloader)
    ...
    def _get_metadata_opts(self, info):
        ...
        for fmt in info.get("requested_formats") or [info]:
            stream_count = (
                2 if "none" not in (fmt.get("vcodec"), fmt.get("acodec")) else 1
            )
            lang = ISO639Utils.short2long(fmt.get("language") or "") or fmt.get(
                "language"
            )
        ...
        if not infofn or not os.path.exists(infofn):
            ...
            infofn = infofn or "%s.temp" % (
                ...
                or replace_extension(
                    self._downloader.prepare_filename(info), "info.json", info["ext"]
                )
            )
...
class FFmpegConcatPP(FFmpegPostProcessor):
    ...
    def run(self, info):
        ...
        if not any(entries) or (
            ...
            entries, (..., lambda k, v: k == "requested_downloads" and len(v) > 1)
        ):
            raise PostProcessingError(
        ...
        in_files = (
            traverse_obj(entries, (..., "requested_downloads", 0, "filepath")) or []
        )
