...
class JSInterpreter:
            ...
            if not in_quote and char in _MATCHING_PARENS:
                ...
                if char in _QUOTES and in_quote in (char, None):
                    if in_quote or after_op or char != "/":
                        in_quote = (
                            None if in_quote and not in_regex_char_group else char
                        )
    ...
    def _operator(self, op, left_val, right_expr, expr, local_vars, allow_recursion):
        ...
        try:
            ...
            raise self.Exception(
                f"Failed to evaluate {left_val!r} {op} {right_val!r}", expr, cause=e
            )
    ...
    def interpret_statement(
        self, stmt, local_vars, allow_recursion=100, _is_var_declaration=False
    ):
        ...
        if m:
            ...
            if m.group("throw"):
                raise JS_Throw(
                    self.interpret_expression(expr, local_vars, allow_recursion)
                )
            ...
            if all(len(sub_expr) == 2 for sub_expr in sub_expressions):
                ...
                def dict_item(key, val):
                    ...
                    if re.match(_NAME_RE, key):
                        return key, val
                    return (
                        self.interpret_expression(key, local_vars, allow_recursion),
                        val,
                    )
        ...
        if m and m.group("attribute"):
            ...
            def eval_method():
                ...
                if isinstance(obj, type) and member.startswith("prototype."):
                    ...
                    assertion(
                        isinstance(argvals[0], obj), f"needs binding to type {obj}"
                    )
                    if func_prototype == "call":
                        ...
                        raise self.Exception(
                            f"Unsupported Function method {func_prototype}", expr
                        )
                    member = new_member
                ...
                if member == "split":
                    assertion(argvals, "takes one or more arguments")
                    assertion(
                        len(argvals) == 1, "with limit argument is not implemented"
                    )
            ...
            if fname in local_vars:
                return (
                    local_vars[fname](argvals, allow_recursion=allow_recursion),
                    should_return,
                )
            elif fname not in self._functions:
                self._functions[fname] = self.extract_function(fname)
            return (
                self._functions[fname](argvals, allow_recursion=allow_recursion),
                should_return,
            )
    ...
    def extract_object(self, objname, *global_stack):
        ...
        for f in fields_m:
            ...
            obj[name] = function_with_repr(
                self.build_function(argnames, f.group("code"), *global_stack),
                f"F<{name}>",
            )
