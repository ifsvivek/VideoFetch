...
def get_elements_by_attribute(*args, **kwargs):
    ...
    return [
        content
        for content, _ in get_elements_text_and_html_by_attribute(*args, **kwargs)
    ]
...
def get_elements_html_by_attribute(*args, **kwargs):
    ...
    return [
        whole for _, whole in get_elements_text_and_html_by_attribute(*args, **kwargs)
    ]
...
def unified_timestamp(date_str, day_first=True):
    ...
    if timetuple:
        return (
            calendar.timegm(timetuple) + pm_delta * 3600 - int(timezone.total_seconds())
        )
...
def datetime_round(dt_, precision="day"):
    ...
    timestamp = roundto(
        calendar.timegm(dt_.timetuple()) + dt_.microsecond / time_scale,
        unit_seconds[precision],
    )
...
def format_decimal_suffix(num, fmt="%d%s", *, factor=1000):
    ...
    exponent = (
        0 if num == 0 else min(int(math.log(num, factor)), len(POSSIBLE_SUFFIXES))
    )
...
def parse_bytes(s):
    ...
    return lookup_unit_table(
        {u: 1024**i for i, u in enumerate(["", *"KMGTPEZY"])}, s.upper(), strict=True
    )
...
def parse_resolution(s, *, lenient=False):
    ...
    if lenient:
        ...
        mobj = re.search(
            r"(?<![a-zA-Z0-9])(?P<w>\d+)\s*[xX×,]\s*(?P<h>\d+)(?![a-zA-Z0-9])", s
        )
    if mobj:
        return {
...
def url_or_none(url):
    ...
    return (
        url
        if re.match(
            r"(?:(?:https?|rt(?:m(?:pt?[es]?|fp)|sp[su]?)|mms|ftps?|wss?):)?//", url
        )
        else None
    )
...
def parse_duration(s):
    ...
    return sum(
        ...
        for part, mult in ((days, 86400), (hours, 3600), (mins, 60), (secs, 1), (ms, 1))
    )
...
def get_exe_version(
    exe, args=["--version"], version_re=None, unrecognized=("present", "broken")
):
...
class PagedList:
    ...
    def getpage(self, pagenum):
        ...
        if page_results is None:
            page_results = (
                [] if pagenum > self._pagecount else list(self._pagefunc(pagenum))
            )
        if self._use_cache:
...
def parse_age_limit(s):
    ...
    if s in US_RATINGS:
        return US_RATINGS[s]
    m = re.match(
        r"^TV[_-]?({})$".format("|".join(k[3:] for k in TV_PARENTAL_GUIDELINES)), s
    )
    if m:
...
def render_table(header_row, data, delim=False, extra_gap=0, hide_empty=False):
    ...
    def filter_using_list(row, filter_array):
        return [
            col
            for take, col in itertools.zip_longest(filter_array, row, fillvalue=True)
            if take
        ]
...
def _match_one(filter_part, dct, incomplete):
    ...
    if m:
        ...
        return op(
            actual_value,
            comparison_value if numeric_comparison is None else numeric_comparison,
        )
...
def cli_bool_option(
    ...
    return cli_option(
        {True: true_value, False: false_value}, command_option, param, separator
    )
...
class GeoUtils:
    ...
    def random_ipv4(cls, code_or_block):
        ...
        return str(
            socket.inet_ntoa(struct.pack("!L", random.randint(addr_min, addr_max)))
        )
...
def caesar(s, alphabet, shift):
    ...
    l = len(alphabet)
    return "".join(
        alphabet[(alphabet.index(c) + shift) % l] if c in alphabet else c for c in s
    )
...
@partial_application
def format_field(
    obj, field=None, template="%s", ignore=NO_DEFAULT, default="", func=IDENTITY
):
...
def scale_thumbnails_to_max_format_width(formats, thumbnails, url_width_re):
    ...
    * Update dimensions
    ...
    _keys = ("width", "height")
    max_dimensions = max(
        (tuple(fmt.get(k) or 0 for k in _keys) for fmt in formats), default=(0, 0)
    )
    ...
    return [
        merge_dicts(
            ...
            dict(zip(_keys, max_dimensions, strict=True)),
            thumbnail,
        )
...
def merge_headers(*dicts):
    ...
    return {
        k.title(): v for k, v in itertools.chain.from_iterable(map(dict.items, dicts))
    }
    ...
    for val in options:
        ...
        if val in alias_dict:
            val = (
                alias_dict[val]
                if not discard
                else [i[1:] if i.startswith("-") else f"-{i}" for i in alias_dict[val]]
            )
...
class FormatSorter:
        ...
        if key not in prop_obj:
            ...
            if key == "field":
                default = (
                    "preference"
                    if type_ == "extractor"
                    else (field,) if type_ in ("combined", "multiple") else field
                )
            elif key == "convert":
                default = (
                    "order"
    ...
    def _resolve_field_value(self, field, value, convert_none=False):
        ...
        if conversion == "string":
            ...
            if use_regex and value is not None:
                ...
                return list_length - (
                    order_list.index(value) if value in order_list else empty_pos
                )
        else:
            if value.isnumeric():
                return float(value)
                ...
                add_item(
                    ...
                    (
                        ...
                        if i < limit_count
                            ...
                            limits[0] if has_limit and not has_multiple_limits else None
                        )
                    ),
                )
    ...
    def print_verbose_info(self, write_debug):
        if self._sort_user:
            write_debug(
                "Sort order given by user: {}".format(", ".join(self._sort_user))
            )
    ...
    def calculate_preference(self, format):
        ...
        return tuple(
            self._calculate_field_preference(format, field) for field in self._order
        )
