...
if sys.version_info < (3, 10):
    raise ImportError(
        f"You are using an unsupported version of Python. Only Python versions 3.10 and above are supported by yt-dlp"
    )  # noqa: F541
    ...
    with YoutubeDL(ydl_opts) as ydl:
        ...
        if opts.list_impersonate_targets:
            ...
            def make_row(target, handler):
                return [
                    join_nonempty(target.client.title(), target.version, delim="-")
                    or "-",
                    join_nonempty(
                    ...
                    rows.insert(
                        ...
                        [
                            ...
                            for text in make_row(
                                known_target, f"{known_handler} (unavailable)"
                            )
                        ],
                    )
            ...
            ydl.to_stdout(
                render_table(["Client", "OS", "Source"], rows, extra_gap=2, delim="-")
            )
            return
