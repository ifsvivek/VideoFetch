...
class YoutubeDL:
    ...
    def __init__(self, params=None, auto_init=True):
        ...
        def process_color_policy(stream):
            ...
            policy = (
                traverse_obj(self.params, ("color", (stream_name, None), {str}, any))
                or "auto"
            )
    ...
    def _clean_remote_components(self, remote_components: set):
        if unsupported_remote_components := set(remote_components) - set(
            supported_remote_components.value
        ):
            self.report_warning(
    ...
    def _js_runtimes(self):
        ...
        for name, config in self.params.get("js_runtimes", {}).items():
            ...
            runtimes[name] = (
                runtime_cls(path=config.get("path")) if runtime_cls else None
            )
        return runtimes
    ...
    def process_ie_result(self, ie_result, download=True, extra_info=None):
        ...
        if result_type == "video":
            ...
            if additional_urls:
                ...
                self.write_debug(
                    'Additional URLs: "{}"'.format('", "'.join(additional_urls))
                )
    ...
    def __process_playlist(self, ie_result, download):
        ...
        if lazy:
            ...
            ie_result["requested_entries"], ie_result["entries"] = tuple(
                zip(*resolved_entries, strict=True)
            ) or ([], [])
    ...
    def _default_format_spec(self, info_dict):
        ...
        return (
            ...
            if prefer_best
            else "bestvideo+bestaudio/best" if compat else "bestvideo*+bestaudio/best"
        )
                    ...
                    if mobj is not None:
                        ...
                        _filter_f = (
                            ...
                            if format_type and format_modified  # bv*, ba*, wv*, wa*
                                ...
                                (lambda f: f.get(f"{not_format_type}codec") == "none")
    ...
    def process_video_result(self, info_dict, download=True):
        ...
        if self.params.get("check_formats") is True:
            formats = LazyList(
                self._check_formats(formats[::-1], warning=False), reverse=True
            )
        ...
        if download:
            ...
            for fmt, chapter in itertools.product(
                ...
                new_info.update(fmt)
                offset, duration = info_dict.get("section_start") or 0, info_dict.get(
                    "duration"
                ) or float("inf")
            ...
            try:
                ...
                def existing_video_file(*filepaths):
                    ...
                    file = self.existing_file(
                        itertools.chain(
                            *zip(map(converted, filepaths), filepaths, strict=True)
                        ),
            ...
            [
                ...
                join_nonempty(
                    ...
                    join_nonempty(
                        ...
                        (
                            self._format_out("Untested", self.Styles.WARNING)
        ...
        def get_encoding(stream):
            ...
            if not supports_terminal_sequences(stream):
                additional_info.append(
                    "No VT" if WINDOWS_VT_MODE.value is False else "No ANSI"
                )
            if additional_info:
                        ...
                        sorted(
                            (
                                ...
                                if runtime is None
                                    ...
                                    + (
                                        " (unsupported)"
                                        if runtime.info.supported is False
                                        else ""
                                    ),
                                )
                            )
    ...
    def _parse_impersonate_targets(self, impersonate):
        ...
        available_target = next(
            filter(self._impersonate_target_available, requested_targets), None
        )
