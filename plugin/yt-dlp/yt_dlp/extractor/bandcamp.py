...
class BandcampIE(InfoExtractor):
    ...
    def _extract_data_attr(self, webpage, video_id, attr="tralbum", fatal=True):
        return self._parse_json(
            self._html_search_regex(
                rf'data-{attr}=(["\'])({{.+?}})\1', webpage, attr + " data", group=2
            ),
            video_id,
            fatal=fatal,
        )
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "formats": formats,
            "tags": traverse_obj(
                webpage, ({find_elements(cls="tag")}, ..., {clean_html})
            ),
        }
...
class BandcampAlbumIE(BandcampIE):  # XXX: Do not subclass from concrete IE
    ...
    _VALID_URL = (
        r"https?://(?:(?P<subdomain>[^.]+)\.)?bandcamp\.com/album/(?P<id>[^/?#&]+)"
    )
...
class BandcampUserIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?!www\.)(?P<id>[^.]+)\.bandcamp\.com(?:/music)?/?(?:[#?]|$)"
    )
