...
class WeverseBaseIE(InfoExtractor):
    ...
    def _token_is_expired(self, key):
        is_expired = (
            jwt_decode_hs256(self._oauth_tokens[key])["exp"] - time.time() < 3600
        )
    ...
    def _refresh_access_token(self):
        ...
        if self._is_logged_in:
            headers["Authorization"] = (
                f"Bearer {self._oauth_tokens[self._ACCESS_TOKEN_KEY]}"
            )

        try:
            response = self._download_json(
                f"{self._ACCOUNT_API_BASE}/api/v1/token/refresh",
                None,
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 401:
                ...
                if self._oauth_cache_key == "cookies":
                    self.cookiejar.clear(
                        domain=".weverse.io", path="/", name=self._ACCESS_TOKEN_KEY
                    )
                    self.cookiejar.clear(
                        domain=".weverse.io", path="/", name=self._REFRESH_TOKEN_KEY
                    )
                else:
                    self.cache.store(
                        self._NETRC_MACHINE, self._oauth_cache_key, self._oauth_tokens
                    )
                self._report_login_error("expired_refresh_token")
            raise
        ...
        raise ExtractorError(
            join_nonempty(
                ...
                self._LOGIN_HINT_TMPL.format(
                    self._OAUTH_PREFIX, self._REFRESH_TOKEN_KEY, username
                ),
                "Or else you can u",
                self._login_hint(method="session_cookies")[1:],
                delim="",
            ),
            expected=True,
        )
    ...
    def _perform_login(self, username, password):
        ...
        self._oauth_tokens.update(
            self.cache.load(self._NETRC_MACHINE, self._oauth_cache_key, default={})
        )
    ...
    def _real_initialize(self):
        ...
        if not self._device_id:
            self._device_id = traverse_obj(
                cookies, (self._DEVICE_ID_KEY, "value")
            ) or str(uuid.uuid4())
    ...
    def _get_formats(self, data, video_id):
        ...
        for stream in traverse_obj(
            data,
            ("streams", lambda _, v: v["type"] == "HLS" and url_or_none(v["source"])),
        ):
            query = {}
...
class WeverseIE(WeverseBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.|m\.)?weverse\.io/(?P<artist>[^/?#]+)/live/(?P<id>[\d-]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if live_status == "is_upcoming":
            ...
            m3u8_url = traverse_obj(
                playback,
                ("media", lambda _, v: v["protocol"] == "HLS", "path", {url_or_none}),
                get_all=False,
            )
            ...
            video_info = self._download_json(
                ...
                query={
                    "key": in_key,
                    "sid": traverse_obj(post, ("extension", "video", "serviceId"))
                    or "2070",
                    "pid": str(uuid.uuid4()),
            ...
            has_drm = (
                traverse_obj(video_info, ("meta", "provider", "name", {str.lower}))
                == "drm"
            )
...
class WeverseMediaIE(WeverseBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.|m\.)?weverse\.io/(?P<artist>[^/?#]+)/media/(?P<id>[\d-]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        video_id = traverse_obj(
            ...
            (
                ...
                "data",
                lambda _, v: self._extract_live_status(v) in ("is_live", "is_upcoming"),
                "postId",
                {str},
            ),
            get_all=False,
