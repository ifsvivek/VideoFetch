...
class RaiBaseIE(InfoExtractor):
    ...
    def _extract_relinker_info(self, relinker_url, video_id, audio_only=False):
        ...
        if not audio_only and not is_live:
            formats.extend(
                self._create_http_urls(media_url, relinker_url, formats, video_id)
            )
    ...
    def _extract_subtitles(url, video_data):
        ...
        subtitles = {}
        subtitles_array = (
            video_data.get("subtitlesArray") or video_data.get("subtitleList") or []
        )
        ...
        for b in program.get("blocks") or []:
            for s in b.get("sets") or []:
                if extra_id:
                    if (
                        extra_id
                        != join_nonempty(b.get("name"), s.get("name"), delim="/")
                        .replace(" ", "-")
                        .upper()
                    ):
                        continue
        ...
        return {
            **info,
            ...
            "alt_title": traverse_obj(
                media, ("track_info", "media_name"), expected_type=strip_or_none
            ),
            "description": media.get("description"),
