...
class TikTokBaseIE(InfoExtractor):
    ...
    def _DEVICE_ID(self):
        return self._KNOWN_DEVICE_ID or str(
            random.randint(7250000000000000000, 7325099899999994577)
        )

    @functools.cached_property
    def _API_HOSTNAME(self):
        return self._configuration_arg(
            "api_hostname", ["api16-normal-c-useast1a.tiktokv.com"], ie_key=TikTokIE
        )[0]
            ...
            self._APP_INFO_POOL = [
                {
                    ...
                    **dict(
                        (k, v)
                        for k, v in zip(
                            self._APP_INFO_DEFAULTS, app_info.split("/"), strict=False
                        )
                        if v
                    ),
                }
        ...
        return filter_dict(
            {
                **query,
                ...
                "app_name": self._APP_INFO["app_name"],
                "version_code": "".join(
                    f"{int(v):02d}" for v in self._APP_INFO["app_version"].split(".")
                ),
    ...
    def _extract_web_data_and_status(self, url, video_id, fatal=True):
        ...
        res = self._download_webpage_handle(
            url, video_id, fatal=fatal, impersonate=True
        )
        if res is False:
        ...
        if universal_data := self._get_universal_data(webpage, video_id):
            ...
            status = (
                traverse_obj(
                    universal_data, ("webapp.video-detail", "statusCode", {int})
                )
                or 0
            )
            ...
            status = (
                traverse_obj(next_data, ("props", "pageProps", "statusCode", {int}))
                or 0
            )
            video_data = traverse_obj(
                next_data, ("props", "pageProps", "itemInfo", "itemStruct", {dict})
            )
            ...
            formats.extend(
                extract_addr(
                    video_info["play_addr"],
                    {
                        ...
                        "vcodec": (
                            "h265"
                            if traverse_obj(video_info, "is_bytevc1", "is_h265")
                            else "h264"
            ...
            formats.extend(
                extract_addr(
                    download_addr,
                    {
                        ...
                        % (", watermarked" if video_info.get("has_watermark") else ""),
                        "vcodec": "h264",
                        "width": dl_width,
                        "height": try_call(
                ...
                formats.extend(
                    extract_addr(
                        bitrate["play_addr"],
                        {
                            ...
                            "vcodec": (
                                "h265"
                                if traverse_obj(bitrate, "is_bytevc1", "is_h265")
                                else "h264"
                            ),
                            "fps": bitrate.get("FPS"),
                        },
        ...
        return {
            ...
            **author_info,
            ...
            "subtitles": self.extract_subtitles(
                aweme_detail,
                aweme_id,
                traverse_obj(author_info, "uploader", "uploader_id", "channel_id"),
            ),
            "thumbnails": [
                {
                    "id": cover_id,
    ...
    def _extract_web_formats(self, aweme_detail):
        ...
        if not formats and traverse_obj(
            ...
            ext = (
                traverse_obj(
                    parse_qs(audio_url),
                    ("mime_type", -1, {lambda x: x.replace("_", "/")}, {mimetype2ext}),
                )
                or "m4a"
            )
            formats.append(
    ...
    def _parse_aweme_video_web(
        ...
        return {
            "id": video_id,
            "formats": (
                None if extract_flat else self._extract_web_formats(aweme_detail)
            ),
            "subtitles": (
                None
                if extract_flat
                ...
                (
                    "music",
                    {
                        ...
                        "artists": (
                            "authorName",
                            {str},
                            {lambda x: re.split(r"(?:, | & )", x) if x else None},
                        ),
...
class TikTokUserIE(TikTokBaseIE):
    IE_NAME = "tiktok:user"
    _VALID_URL = (
        r"(?:tiktokuser:|https?://(?:www\.)?tiktok\.com/@)(?P<id>[\w.-]+)/?(?:$|[#?])"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if re.fullmatch(r"MS4wLjABAAAA[\w-]{64}", user_name):
            ...
            video_count = traverse_obj(
                detail, ("userInfo", ("stats", "statsV2"), "videoCount", {int}, any)
            )
            if not video_count and detail.get("statusCode") == 10222:
                ...
                raise ExtractorError(
                    "This account does not have any videos posted", expected=True
                )
    ...
    def _entries(self, collection_id):
        ...
        for page in itertools.count(1):
            ...
            for video in traverse_obj(response, ("itemList", lambda _, v: v["id"])):
                ...
                author = (
                    traverse_obj(
                        video, ("author", ("uniqueId", "secUid", "id"), {str}, any)
                    )
                    or "_"
                )
...
class TikTokVMIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:(?:vm|vt)\.tiktok\.com|(?:www\.)tiktok\.com/t)/(?P<id>\w+)"
    )
    IE_NAME = "vm.tiktok"
...
class TikTokLiveIE(TikTokBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if not room_id:
            webpage = self._download_webpage(
                format_field(uploader, None, self._UPLOADER_URL_FORMAT),
                uploader,
                impersonate=True,
            )
        ...
        if not uploader or not room_id:
            webpage = self._download_webpage(
                url, uploader or room_id, fatal=not room_id
            )
        ...
        def get_vcodec(*keys):
            return traverse_obj(
                live_info, ("stream_url", *keys, {parse_inner}, "VCodec", {str})
            )
        ...
        return {
            "id": room_id,
            "uploader": uploader,
            "uploader_url": format_field(uploader, None, self._UPLOADER_URL_FORMAT)
            or None,
            "is_live": True,
            "formats": formats,
