    ...
    def _real_extract(self, url):
        ...
        for stream in traverse_obj(media_data, ("streams", ..., {dict})):
            ...
            for media in traverse_obj(
                ...
                lang_code = (
                    traverse_obj(media, ("audios", 0, "languageCode", {str})) or "deu"
                )
        ...
        age_limit = traverse_obj(
            page_data, ("fskRating", {lambda x: remove_start(x, "FSK")}, {int_or_none})
        )
        return {
            "id": video_id,
            "display_id": display_id,
        ...
        def fetch_page(page_num):
            for item in traverse_obj(call_api(page_num), ("teasers", ..., {dict})):
                item_id = traverse_obj(
                    item, ("links", "target", ("urlId", "id")), "id", get_all=False
                )
...
class ARDAudiothekPlaylistIE(ARDAudiothekBaseIE):
    ...
    def _real_extract(self, url):
        ...
        for url in traverse_obj(
            playlist_info, ("items", "nodes", ..., "url", {url_or_none})
        ):
