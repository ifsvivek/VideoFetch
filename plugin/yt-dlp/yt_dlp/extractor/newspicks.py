...
class NewsPicksIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        fragment = self._search_nextjs_data(webpage, video_id)["props"]["pageProps"][
            "fragment"
        ]
        ...
        if traverse_obj(
            movie, ("viewable", {str})
        ) == "PARTIAL_FREE" and not traverse_obj(movie, ("canWatch", {bool})):
            self.report_warning(
                "Full video is for Premium members. Without cookies, "
                f"only the preview is downloaded. {self._login_hint()}",
                video_id,
            )

        m3u8_url = traverse_obj(
            movie, ("movieUrl", {url_or_none}, {require("m3u8 URL")})
        )
