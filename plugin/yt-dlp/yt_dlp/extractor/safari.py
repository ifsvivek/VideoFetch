...
class SafariBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        next_uri = urllib.parse.urljoin("https://api.oreilly.com", qs["next"][0])
        ...
        if not auth.get("logged_in") and not auth.get("redirect_uri") and credentials:
            raise ExtractorError(f"Unable to login: {credentials}", expected=True)
...
class SafariApiIE(SafariBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if "library/view" in web_url:
            ...
            web_url = (
                f'{web_url.rsplit("/", 1)[0]}/{natural_keys[0]}-{natural_keys[1][:-5]}'
            )
