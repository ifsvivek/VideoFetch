    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            **self._parse_metadata(metadata),
            "_old_archive_ids": (
                [make_archive_id("rtveinfantil", video_id)] if is_infantil else None
            ),
        }
...
class RTVEAudioIE(RTVEBaseIE):
    ...
    IE_DESC = "RTVE audio"
    _VALID_URL = (
        r"https?://(?:www\.)?rtve\.es/(alacarta|play)/audios/(?:[^/?#]+/){2}(?P<id>\d+)"
    )
...
class RTVEProgramIE(RTVEBaseIE):
    ...
    def _entries(self, page_data):
        for video in traverse_obj(
            page_data, ("page", "items", lambda _, v: url_or_none(v["htmlUrl"]))
        ):
    ...
    def _real_extract(self, url):
        ...
        program_id = self._html_search_meta(
            "DC.identifier", program_page, "Program ID", fatal=True
        )
        ...
        entries = InAdvancePagedList(
            lambda idx: self._entries(
                self._fetch_page(program_id, idx + 1) if idx else first_page
            ),
            page_count,
            self._PAGE_SIZE,
        )
        ...
        return self.playlist_result(
            entries, program_id, self._html_extract_title(program_page)
        )
