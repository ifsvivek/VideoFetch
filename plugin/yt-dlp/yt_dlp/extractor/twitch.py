...
class TwitchBaseIE(InfoExtractor):
    ...
    def _extract_twitch_m3u8_formats(
        self, path, video_id, token, signature, live_from_start=False
    ):
        try:
            ...
            error_info = (
                traverse_obj(e.cause.response.read(), ({json.loads}, 0, {dict})) or {}
            )
            if error_info.get("error_code") in (
                ...
                if self._get_cookies("https://gql.twitch.tv").get("auth-token"):
                    raise ExtractorError(
                        f"Your account does not have {common_msg}", expected=True
                    )
                self.raise_login_required(
                    f"You must be logged into an account that has {common_msg}"
                )

            if error_msg := join_nonempty(
                "error_code", "error", from_dict=error_info, delim=": "
            ):
        ...
        for fmt in formats:
            if fmt.get("vcodec") and fmt["vcodec"].startswith("av01"):
                ...
                fmt.setdefault("downloader_options", {}).update(
                    {"ffmpeg_args_out": ["-f", "mp4"]}
                )
            if live_from_start:
                fmt.setdefault("downloader_options", {}).update(
                    {"ffmpeg_args": ["-live_start_index", "0"]}
                )
    ...
    def _extract_chapters(self, info, item_id):
        ...
        for moment in info["moments"]:
            ...
            if start_time is None or duration is None:
                self.report_warning(
                    f"Important chapter information missing for chapter {name}", item_id
                )
                continue
            yield {
...
class TwitchVideosIE(TwitchVideosBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:(?:www|go|m)\.)?twitch\.tv/(?P<id>[^/]+)/(?:videos|profile)"
    )
    _TESTS = [
        {
...
class TwitchStreamIE(TwitchVideosBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if self.get_param("live_from_start"):
            self.to_screen(
                f"{channel_name}: Extracting VOD to download live from start"
            )
            entry = next(self._entries(channel_name, None, "time"), None)
            if entry and entry.pop("timestamp") >= (timestamp or float("inf")):
                return entry
            self.report_warning(
                "Unable to extract the VOD associated with this livestream",
        ...
        thumbnail = url_or_none(
            try_get(
                gql, lambda x: x[2]["data"]["user"]["stream"]["previewImageURL"], str
            )
        )
...
class TwitchClipsIE(TwitchBaseIE):
    ...
    def _real_extract(self, url):
        ...
        for source in traverse_obj(
            asset_portrait, ("videoQualities", lambda _, v: url_or_none(v["sourceURL"]))
        ):
            formats.append(
                {
        ...
        old_id = self._search_regex(
            r"%7C(\d+)(?:-\d+)?.mp4", formats[-1]["url"], "old id", default=None
        )
