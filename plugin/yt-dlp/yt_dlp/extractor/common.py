...
class InfoExtractor:
    ...
    def _match_valid_url(cls, url):
        ...
        return next(
            filter(None, (regex.match(url) for regex in cls._VALID_URL_RE)), None
        )
    ...
    def _create_request(
        self, url_or_request, data=None, headers=None, query=None, extensions=None
    ):
    ...
    def _request_webpage(
        ...
        available_target, requested_targets = (
            self._downloader._parse_impersonate_targets(impersonate)
        )
        if available_target:
            ...
            if require_impersonation:
                raise ExtractorError(
                    self._downloader._unavailable_targets_message(
                        requested_targets, note=msg, is_error=True
                    ),
                    expected=True,
                )
            self.report_warning(
                self._downloader._unavailable_targets_message(
                    requested_targets, note=msg
                ),
                only_once=True,
            )
        ...
        try:
            return self._downloader.urlopen(
                self._create_request(url_or_request, data, headers, query, extensions)
            )
    ...
    def _guess_encoding_from_content(content_type, webpage_bytes):
        ...
        if m:
            ...
            m = re.search(
                rb'<meta[^>]+charset=[\'"]?([^\'")]+)[ /\'">]', webpage_bytes[:1024]
            )
            if m:
    ...
    def _webpage_read_content(
        ...
        if self.get_param("write_pages"):
            ...
            filename = _request_dump_filename(
                urlh.url, video_id, data, trim_length=self.get_param("trim_file_name")
            )
    ...
    def __create_download_methods(name, parser, note, errnote, return_value):
        ...
        impersonate(
            download_handle, f"_download_{name}_handle", f"({return_value}, URL handle)"
        )
    ...
    def _get_login_info(
        self, username_option="username", password_option="password", netrc_machine=None
    ):
        """
    ...
    def _html_extract_title(self, html, name="title", *, fatal=False, **kwargs):
        return self._html_search_regex(
            r"(?s)<title\b[^>]*>([^<]+)</title>", html, name, fatal=fatal, **kwargs
        )
    ...
    def _search_json_ld(
        self, html, video_id, expected_type=None, *, fatal=True, default=NO_DEFAULT
    ):
    ...
    def _json_ld(self, json_ld, video_id, fatal=True, expected_type=None):
        ...
        def extract_chapter_information(e):
            ...
            for idx, (last_c, current_c, next_c) in enumerate(
                zip([{"end_time": 0}, *chapters], chapters, chapters[1:], strict=False)
            ):
        ...
        def traverse_json_ld(json_ld, at_top_level=True):
            for e in variadic(json_ld):
                ...
                if expected_type is not None and not is_type(e, expected_type):
                    continue
                rating = traverse_obj(
                    e, ("aggregateRating", "ratingValue"), expected_type=float_or_none
                )
                ...
                if is_type(e, "TVEpisode", "Episode", "PodcastEpisode"):
                    ...
                    if is_type(
                        part_of_season, "TVSeason", "Season", "CreativeWorkSeason"
                    ):
                        info.update(
                            {
                    ...
                    if is_type(
                        part_of_series, "TVSeries", "Series", "CreativeWorkSeries"
                    ):
    ...
    def _search_nextjs_v13_data(self, webpage, video_id, fatal=True):
        ...
        def flatten(flight_data):
            ...
            if len(flight_data) == 4 and flight_data[0] == "$":
                ...
                if (
                    ...
                    and re.fullmatch(r"\$L[0-9a-f]+", name)
                ):
        ...
        for flight_segment in re.findall(
            r"<script\b[^>]*>self\.__next_f\.push\((\[.+?\])\)</script>", webpage
        ):
            segment = self._parse_json(
                flight_segment, video_id, fatal=fatal, errnote=None if fatal else False
            )
            ...
            if not isinstance(segment, list) or len(segment) != 2:
                self.write_debug(
                    f"{video_id}: Unsupported next.js flight data structure detected",
                    only_once=True,
                )
                continue
            ...
            if bootstrap_info is None:
                ...
                manifest_url = (
                    ...
                    if media_url.startswith(("http://", "https://"))
                        ...
                        (manifest_base_url or "/".join(manifest_url.split("/")[:-1]))
                        + "/"
                        + media_url
                    )
                )
                        ...
                        f.update(
                            {
                                ...
                                "format_id": (
                                    f.get("format_id") if not tbr else format_id
                                ),
                                "vcodec": vcodec,
                            }
                        )
    ...
    def _parse_m3u8_formats_and_subtitles(
            ...
            formats = [
                {
                    ...
                    "format_index": idx,
                    "url": m3u8_url
                    or encode_data_uri(m3u8_doc.encode(), "application/x-mpegurl"),
                    "ext": ext,
        ...
        def extract_media(x_media_line):
            ...
            if media_url:
                ...
                is_alternate = (
                    media.get("DEFAULT") == "NO" or media.get("AUTOSELECT") == "NO"
                )
                formats.extend(
                    {
                        ...
                        "_audio_group_id": (
                            group_id if is_audio and not is_alternate else None
                        ),
                    }
        ...
        audio_groups_by_quality = orderedSet(
            ...
            for f in sorted(
                traverse_obj(
                    formats,
                    lambda _, v: v.get("vcodec") != "none" and v["_audio_group_id"],
                ),
    ...
    def _extract_smil_formats_and_subtitles(
        self, smil_url, video_id, fatal=True, f4m_params=None, transform_source=None
    ):
        if self.get_param("ignore_no_formats_error"):
            fatal = False
    ...
    def _parse_smil_formats_and_subtitles(
        ...
        for medium in media:
            ...
            bitrate = float_or_none(
                medium.get("system-bitrate") or medium.get("systemBitrate"), 1000
            )
            ...
            src_url = (
                src if src.startswith("http") else urllib.parse.urljoin(f"{base}/", src)
            )
    ...
    def _extract_xspf_playlist(self, xspf_url, playlist_id, fatal=True):
        ...
        return self._parse_xspf(
            xspf, playlist_id, xspf_url=xspf_url, xspf_base_url=base_url(xspf_url)
        )
            ...
            for adaptation_set in period.findall(_add_ns("AdaptationSet")):
                ...
                for representation in adaptation_set.findall(_add_ns("Representation")):
                    ...
                    def prepare_template(template_name, identifiers):
                        ...
                        t = re.sub(
                            r"\$({})\$".format("|".join(identifiers)), r"%(\1)d", t
                        )
                        t = re.sub(
                    ...
                    if (
                        ...
                        if (
                            ...
                            if (
                                ...
                                representation_ms_info["total_number"] = math.ceil(
                                    float_or_none(
                                        period_duration, segment_duration, default=0
                                    )
                                )
                        ...
                        for s in representation_ms_info["s"]:
                            ...
                            for _ in range(s.get("r", 0) + 1):
                                segment_uri = representation_ms_info["segment_urls"][
                                    segment_index
                                ]
                                fragments.append(
                                    {
        ...
        for stream in ism_doc.findall("StreamIndex"):
            ...
            for track in stream.findall("QualityLevel"):
                ...
                for stream_fragment_index, stream_fragment in enumerate(
                    ...
                    fragment_ctx["time"] = (
                        int_or_none(stream_fragment.get("t")) or fragment_ctx["time"]
                    )
                    ...
                    if not fragment_ctx["duration"]:
                        try:
                            next_fragment_time = int(
                                stream_fragment[stream_fragment_index + 1].attrib["t"]
                            )
                        except IndexError:
        ...
        for media_tag, _, media_type, media_content in media_tags:
            ...
            if media_content:
                for source_tag in re.findall(r"<source[^>]+>", media_content):
                    ...
                    src = strip_or_none(
                        dict_get(
                            s_attr, ("src", "data-video-src", "data-src", "data-source")
                        )
                    )
                    if not src:
                        continue
                    ...
                    if is_plain_url:
                        ...
                        height = int_or_none(s_attr.get("height")) or int_or_none(
                            s_attr.get("res")
                        )
    ...
    def _extract_akamai_formats_and_subtitles(self, manifest_url, video_id, hosts={}):
        ...
        if not signed:
            ...
            manifest_url = re.sub(
                r"(?:b=[\d,-]+|(?:__a__|attributes)=off|__b__=\d+)&?", "", manifest_url
            ).strip("?")
        ...
        f4m_url = re.sub(r"(https?://[^/]+)/i/", r"\1/z/", manifest_url).replace(
            "/master.m3u8", "/manifest.f4m"
        )
        ...
        m3u8_url = re.sub(r"(https?://[^/]+)/z/", r"\1/i/", manifest_url).replace(
                ...
                for f in m3u8_formats:
                    if f["vcodec"] != "none":
                        for protocol in ("http", "https"):
                            ...
                            http_url = re.sub(
                                REPL_REGEX,
                                protocol + rf"://{http_host}/\g<1>{qualities[i]}\3",
                                f["url"],
                            )
                            http_f.update(
                                {
    ...
    def _extract_wowza_formats(
        self, url, video_id, m3u8_entry_protocol="m3u8_native", skip_protocols=[]
    ):
        ...
        if "f4m" not in skip_protocols:
            formats.extend(
                self._extract_f4m_formats(
                    manifest_url("manifest.f4m"), video_id, f4m_id="hds", fatal=False
                )
            )
        if "dash" not in skip_protocols:
            formats.extend(
                self._extract_mpd_formats(
                    manifest_url("manifest.mpd"), video_id, mpd_id="dash", fatal=False
                )
            )
    ...
    def _parse_jwplayer_data(
        ...
        for video_data in playlist_items:
            ...
            if len(formats) == 1 and re.search(
                r"^(?:http|//).*(?:youtube\.com|youtu\.be)/.+", formats[0]["url"]
            ):
                entry.update(
                    {
    ...
    def _parse_jwplayer_formats(
        ...
        for source in jwplayer_sources_data:
            ...
            if (
                ...
                formats.extend(
                    self._extract_smil_formats(source_url, video_id, fatal=False)
                )
    ...
    def description(cls, *, markdown=True, search_examples=None):
        ...
        name = (
            (" - **{}**".format(re.sub(r":(\w+:)", ":\u200b\\g<1>", cls.IE_NAME)))
            if markdown
            else cls.IE_NAME
        )
    ...
    def _extract_chapters_helper(
        self, chapter_list, start_function, title_function, duration, strict=True
    ):
        if not duration:
            return
        chapter_list = [
    ...
    def RetryManager(self, **kwargs):
        return RetryManager(
            self.get_param("extractor_retries", 3), self._error_or_warning, **kwargs
        )

    def _extract_generic_embeds(
        self, url, *args, info_dict={}, note="Extracting generic embeds", **kwargs
    ):
