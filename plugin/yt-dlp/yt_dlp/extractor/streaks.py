...
class StreaksBaseIE(InfoExtractor):
    ...
    def _extract_from_streaks_api(
        self, project_id, media_id, headers=None, query=None, ssai=False
    ):
        try:
            response = self._download_json(
                self._API_URL_TEMPLATE.format("playback", project_id, media_id, ""),
                media_id,
                "Downloading STREAKS playback API JSON",
                headers={
                ...
                if code == "REQUEST_FAILED":
                    if error_id == 124:
                        ...
                        raise ExtractorError(
                            "Access is denied (possibly due to invalid/missing API key)"
                        )
                if code == "MEDIA_NOT_FOUND":
                    raise ExtractorError(
                        join_nonempty(code, message, delim=": "), expected=True
                    )
                if code or message:
                    raise ExtractorError(
                        join_nonempty(code, error_id, message, delim=": ")
                    )
            raise
                ...
                session_params = traverse_obj(
                    self._download_json(
                        self._API_URL_TEMPLATE.format(
                            "ssai", project_id, streaks_id, "/ssai/session"
                        ),
                        media_id,
                        "Downloading session parameters",
                        headers={
        ...
        for subs in traverse_obj(
            ...
            (
                "tracks",
                lambda _, v: v["kind"] in ("captions", "subtitles")
                and url_or_none(v["src"]),
            ),
        ):
