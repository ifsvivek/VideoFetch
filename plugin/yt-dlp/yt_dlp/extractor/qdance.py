...
class QDanceIE(InfoExtractor):
    ...
    def _get_auth(self):
        if (try_call(lambda: jwt_decode_hs256(self._access_token)["exp"]) or 0) <= int(
            time.time() - 120
        ):
            ...
            self._call_login_api(
                {"refreshToken": self._refresh_token}, note="Refreshing access token"
            )
    ...
    def _real_extract(self, url):
        ...
        formats = (
            self._extract_m3u8_formats(m3u8_url, video_id, fatal=False, live=True)
            if m3u8_url
            else []
        )
        if not formats:
