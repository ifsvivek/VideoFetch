    ...
    def _real_extract(self, url):
        ...
        def call_playback_api(item, query=None):
            try:
                return self._call_api(
                    f"playback/{item}/program/{video_id}", video_id, item, query=query
                )
        ...
        info = {
            ...
            "timestamp": parse_iso8601(
                try_get(manifest, lambda x: x["availability"]["onDemand"]["from"], str)
            ),
        }
...
class NRKTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(f"nrk:{video_id}", ie=NRKIE.ie_key(), video_id=video_id)
    ...
    def _real_extract(self, url):
        ...
        if NRKTVIE.suitable(urlh.url):
            ...
            nrk_id = self._search_json(
                r'<script\b[^>]+\bid="pageData"[^>]*>', webpage, "page data", display_id
            )["initialState"]["selectedEpisodePrfId"]
    ...
    def _extract_entries(self, entry_list):
        ...
        for episode in entry_list:
            ...
            if not nrk_id or not isinstance(nrk_id, str):
                continue
            entries.append(
                self.url_result(f"nrk:{nrk_id}", ie=NRKIE.ie_key(), video_id=nrk_id)
            )
        return entries
...
class NRKSkoleIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        nrk_id = self._download_json(
            f"https://nrkno-skole-prod.kube.nrk.no/skole/api/media/{video_id}", video_id
        )["psId"]
