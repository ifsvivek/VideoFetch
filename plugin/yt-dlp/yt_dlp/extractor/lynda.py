...
class LyndaBaseIE(InfoExtractor):
    ...
    def _perform_login_step(
        self, form_html, fallback_action_url, extra_form_data, note, referrer_url
    ):
...
class LyndaIE(LyndaBaseIE):
    ...
    def _raise_unavailable(self, video_id):
        self.raise_login_required(f"Video {video_id} is only available for members")
                ...
                [
                    {
                        ...
                        "format_id": (
                            str(f.get("Resolution")) if f.get("Resolution") else None
                        ),
                    }
                    for f in fmts
                    if f.get("Url")
...
class LyndaCourseIE(LyndaBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if not course:
            ...
            entries = [
                self.url_result(
                    item_template % video_id, ie=LyndaIE.ie_key(), video_id=video_id
                )
        ...
        if course.get("Status") == "NotFound":
            raise ExtractorError(f"Course {course_id} does not exist", expected=True)
