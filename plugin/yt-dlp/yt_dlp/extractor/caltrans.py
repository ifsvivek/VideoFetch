...
class CaltransIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        route_place = self._search_regex(
            r'routePlace\s*=\s*"([^"]+)"', global_vars, "Route Place", fatal=False
        )
        location_name = self._search_regex(
            r'locationName\s*=\s*"([^"]+)"', global_vars, "Location Name", fatal=False
        )
        poster_url = self._search_regex(
            r'posterURL\s*=\s*"([^"]+)"', global_vars, "Poster Url", fatal=False
        )
