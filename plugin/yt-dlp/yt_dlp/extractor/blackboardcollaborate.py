...
class BlackboardCollaborateIE(InfoExtractor):
    ...
    def _call_api(
        self, region, video_id, path=None, token=None, note=None, fatal=False
    ):
        # Ref: https://github.com/blackboard/BBDN-Collab-Postman-REST
        return self._download_json(
            join_nonempty(
                f"https://{region}.bbcollab.com/collab/api/csa/recordings",
                video_id,
                path,
                delim="/",
            ),
    ...
    def _real_extract(self, url):
        ...
        video_info = self._call_api(
            region, video_id, path="data/secure", token=token, note="Trying auth token"
        )
        if video_info:
            video_extra = self._call_api(
                region, video_id, token=token, note="Retrieving extra attributes"
            )
        else:
            video_info = self._call_api(
                region, video_id, path="data", note="Trying fallback", fatal=True
            )
            video_extra = {}
        ...
        for subs in traverse_obj(
            video_info, ("subtitles", lambda _, v: url_or_none(v["url"]))
        ):
        ...
        for live_chat_url in traverse_obj(
            video_info, ("chats", ..., "url", {url_or_none})
        ):
