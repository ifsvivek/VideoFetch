...
class NiconicoChannelPlusBaseIE(InfoExtractor):
    ...
    def _find_fanclub_site_id(self, channel_name):
        ...
        fanclub_id = traverse_obj(
            fanclub_list_json,
            (
                lambda _, v: v["domain"] == f"{self._WEBPAGE_BASE_URL}/{channel_name}",
                "id",
            ),
            get_all=False,
        )
...
class NiconicoChannelPlusIE(NiconicoChannelPlusBaseIE):
    ...
    _VALID_URL = (
        r"https?://nicochannel\.jp/(?P<channel>[\w.-]+)/(?:video|live)/(?P<code>sm\w+)"
    )
    _TESTS = [
        {
    ...
    def _get_comments(self, content_code, comment_group_id):
        ...
        comment_list = self._download_json(
            ...
            query={
                "sort_direction": "asc",
                "limit": int_or_none(self._configuration_arg("max_comments", [""])[0])
                or 120,
            },
            data=json.dumps(
                {
    ...
    def _get_live_status_and_session_id(self, content_code, data_json):
        ...
        if video_type == "vod":
            ...
            raise ExtractorError(
                f"Unknown type: {video_type}", video_id=content_code, expected=False
            )
