...
class IqiyiIE(InfoExtractor):
    ...
    def _extract_playlist(self, webpage):
        ...
        album_id = self._search_regex(r"albumId\s*:\s*(\d+),", webpage, "album ID")
...
class IqIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if traverse_obj(
            initial_format_data, ("boss_ts", "data", "prv"), expected_type=int_or_none
        ):
            self.report_warning(
            ...
            if video_format.get("m3u8Url"):
                extracted_formats.extend(
                    self._extract_m3u8_formats(
                        urljoin(
                            format_data.get("dm3u8", "https://cache-m.iq.com/dc/dt/"),
            ...
            if video_format.get("m3u8"):
                ...
                if ff == "ts":
                    ...
                    mpd_data = traverse_obj(
                        self._parse_json(video_format["m3u8"], video_id, fatal=False),
                        ("payload", ..., "data"),
                        expected_type=str,
                    )
                    if not mpd_data:
        ...
        for sub_format in traverse_obj(
            ...
            subtitles.setdefault(lang, []).extend(
                [
                    ...
                    for format_key, format_ext in [("srt", "srt"), ("webvtt", "vtt")]
        ...
        return {
            ...
            "average_rating": traverse_obj(
                page_data, ("playScoreInfo", "score"), expected_type=float_or_none
            ),
            "timestamp": parse_iso8601(video_info.get("isoUploadDate")),
            "categories": traverse_obj(
                extra_metadata, ("videoTagMap", ..., ..., "name"), expected_type=str
            ),
            "cast": traverse_obj(
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(
                ...
                album_id,
                traverse_obj(
                    next_data, ("props", "initialProps", "pageProps", "modeCode")
                ),
                traverse_obj(
                    next_data, ("props", "initialProps", "pageProps", "langCode")
                ),
            ),
            album_id,
            album_data.get("name"),
