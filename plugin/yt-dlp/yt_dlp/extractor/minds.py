...
class MindsIE(MindsBaseIE):
    IE_NAME = "minds"
    _VALID_URL = (
        MindsBaseIE._VALID_URL_BASE + r"(?:media|newsfeed|archive/view)/(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        entity = self._call_api("v1/entities/entity/" + entity_id, entity_id, "entity")[
            "entity"
        ]
...
class MindsFeedBaseIE(MindsBaseIE):
    ...
    def _entries(self, feed_id):
        ...
        while True:
            data = self._call_api(
                f"v2/feeds/container/{feed_id}/videos", feed_id, f"page {i}", query
            )
            ...
            for entity in entities:
                ...
                yield self.url_result(
                    "https://www.minds.com/newsfeed/" + guid, MindsIE.ie_key(), guid
                )
