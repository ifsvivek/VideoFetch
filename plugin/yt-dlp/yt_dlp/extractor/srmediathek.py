    ...
    def _real_extract(self, url):
        ...
        player_url = traverse_obj(
            ...
            (
                ...
                {extract_attributes},
                "data-mediacollection-ardplayer",
        ...
        return {
            **self._extract_media_info(player_url, webpage, video_id),
            "id": video_id,
            "title": traverse_obj(
                webpage, ({find_element(cls="ardplayer-title")}, {clean_html})
            ),
            ...
            "duration": parse_duration(
                self._search_regex(r"(\d{2}:\d{2}:\d{2})", article, "duration")
            ),
            "release_date": unified_strdate(
                self._search_regex(r"(\d{2}\.\d{2}\.\d{4})", article, "release_date")
            ),
            "series": traverse_obj(
                webpage,
