...
class TV4IE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        fmts, subs = self._extract_m3u8_formats_and_subtitles(
            manifest_url, video_id, "mp4", "m3u8_native", m3u8_id="hls", fatal=False
        )
        formats.extend(fmts)
        subtitles = self._merge_subtitles(subtitles, subs)

        fmts, subs = self._extract_mpd_formats_and_subtitles(
            manifest_url.replace(".m3u8", ".mpd"), video_id, mpd_id="dash", fatal=False
        )
        formats.extend(fmts)
        subtitles = self._merge_subtitles(subtitles, subs)

        fmts = self._extract_f4m_formats(
            manifest_url.replace(".m3u8", ".f4m"), video_id, f4m_id="hds", fatal=False
        )
        formats.extend(fmts)
