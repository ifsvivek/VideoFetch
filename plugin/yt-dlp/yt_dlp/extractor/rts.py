    ...
    def _real_extract(self, url):
        ...
        def download_json(internal_id):
            return self._download_json(
                f"http://www.rts.ch/a/{internal_id}.html?f=json/article", display_id
            )
        ...
        if "video" not in all_info and "audio" not in all_info:
            ...
            internal_id = self._html_search_regex(
                r'<(?:video|audio) data-id="([0-9]+)"', page, "internal video id"
            )
        ...
        info = (
            all_info["video"]["JSONinfo"] if "video" in all_info else all_info["audio"]
        )
        ...
        def extract_bitrate(url):
            return int_or_none(
                self._search_regex(r"-([0-9]+)k\.", url, "bitrate", default=None)
            )
