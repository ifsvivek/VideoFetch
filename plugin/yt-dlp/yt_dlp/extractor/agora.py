...
class WyborczaVideoIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if meta["files"].get("dash"):
            formats.extend(
                self._extract_mpd_formats(base_url + meta["files"]["dash"], video_id)
            )
    ...
    def _real_extract(self, url):
        ...
        if not podcast_id:  # playlist
            ...
            return self.url_result(
                TokFMAuditionIE._create_url(podcast_id), TokFMAuditionIE, podcast_id
            )
...
class TokFMAuditionIE(InfoExtractor):
    _VALID_URL = (
        r"(?:https?://audycje\.tokfm\.pl/audycja/|tokfm:audition:)(?P<id>\d+),?"
    )
    ...
    def _real_extract(self, url):
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, audition_id, data), self._PAGE_SIZE
        )
