...
class ThePlatformBaseIE(AdobePassIE):
    ...
    def _download_theplatform_metadata(self, path, video_id, fatal=True):
        return (
            self._download_json(
                f"https://link.theplatform.{self._TP_TLD}/s/{path}",
                video_id,
                fatal=fatal,
    ...
    def _parse_theplatform_metadata(tp_metadata):
        ...
        info = traverse_obj(
            tp_metadata,
            {
                ...
                "season_number": (
                    site_specific_filter("seasonNumber"),
                    {int_or_none},
                    any,
                ),
                "episode_number": (
                    site_specific_filter("episodeNumber", "airOrder"),
                    {int_or_none},
                    any,
                ),
                "series": (
                    site_specific_filter("show", "seriesTitle", "seriesShortTitle"),
                    (None, ...),
                    {str},
                    any,
                ),
                ...
                "media_type": (
                    site_specific_filter("programmingType", "type"),
                    {str},
                    any,
                ),
            },
        ...
        for caption in traverse_obj(
            tp_metadata, ("captions", lambda _, v: url_or_none(v["src"]))
        ):
...
class ThePlatformFeedIE(ThePlatformBaseIE):
    ...
    def _extract_feed_info(
        ...
        for item in entry["media$content"]:
            ...
            file_asset_types = (
                item.get("plfile$assetTypes") or parse_qs(smil_url)["assetTypes"]
            )
