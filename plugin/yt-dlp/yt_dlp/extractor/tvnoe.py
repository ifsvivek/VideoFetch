...
class TVNoeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for source in traverse_obj(
            player, ("tracks", ..., lambda _, v: url_or_none(v["src"]))
        ):
            src_url = source["src"]
        ...
        return {
            ...
            **traverse_obj(
                webpage,
                {
                    ...
                    "release_date": (
                        {clean_html},
                        {re.compile(r"Premiéra:\s*(\d{1,2}\.\d{1,2}\.\d{4})").findall},
                        ...,
                        {str},
                        {unified_strdate},
                        any,
                    ...
                    "thumbnail": (
                        {find_element(id="player-live", html=True)},
                        {extract_attributes},
                        "poster",
                        {urljoin("https://www.tvnoe.cz/")},
                    ),
                },
            ),
        }
