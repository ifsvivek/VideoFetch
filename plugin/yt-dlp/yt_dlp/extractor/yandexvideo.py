...
class ZenYandexBaseIE(InfoExtractor):
    def _fetch_ssr_data(self, url, video_id):
        ...
        redirect = self._search_json(
            r"(?:var|let|const)\s+it\s*=", webpage, "redirect", video_id, default={}
        ).get("retpath")
        if redirect:
        ...
        return (
            video_id,
            self._search_json(
                r"(?:var|let|const)\s+_params\s*=\s*\(",
                webpage,
                "metadata",
                video_id,
                contains_pattern=r'{["\']ssrData.+}',
            )["ssrData"],
        )
        ...
        stream_urls = set(
            traverse_obj(
                ...
                (
                    ...
                    {url_or_none},
                    {update_url_query(query={"dzen_dash": []})},
                ),
            )
        )
...
class ZenYandexChannelIE(ZenYandexBaseIE):
    ...
    _VALID_URL = (
        r"https?://(zen\.yandex|dzen)\.ru/(?!media|video)(?:id/)?(?P<id>[a-z0-9-_]+)"
    )
    _TESTS = [
        {
        ...
        for page in itertools.count(1):
            for item in traverse_obj(
                ...
                yield self.url_result(
                    item["link"], ZenYandexIE, item.get("id"), title=item.get("title")
                )
