    ...
    def _real_extract(self, url):
        ...
        for source_type, source in metadata["streams"].items():
            if source_type == "smooth_Streaming":
                ...
                formats.extend(
                    self._extract_mpd_formats(source["url"], video_id, fatal=False)
                )
            else:
                formats.append(
                    {
        ...
        return {
            ...
            "age_limit": int_or_none(traverse_obj(metadata, ("snippet", "minimumAge")))
            or 0,
            "formats": formats,
            "subtitles": subtitles,
    ...
    def _extract_subtitle(self, webpage, video_id):
        captions = traverse_obj(
            ...
            for url in re.findall(
                r"var\s+file\s+=\s+\{[^}]+\'(https://[^\']+\.vtt\?[^\']+)", webpage
            )
        ]
...
class MicrosoftLearnEpisodeIE(MicrosoftMediusBaseIE):
    _VALID_URL = (
        r"https?://learn\.microsoft\.com/(?:[\w-]+/)?shows/[\w-]+/(?P<id>[^?#/]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if ism_url := traverse_obj(
            video_info, ("publicVideo", "adaptiveVideoUrl", {url_or_none})
        ):
            formats.extend(self._extract_ism(ism_url, video_id, fatal=False))
        if hls_url := traverse_obj(
            video_info, ("publicVideo", "adaptiveVideoHLSUrl", {url_or_none})
        ):
            formats.extend(
                self._extract_m3u8_formats(
        ...
        if mpd_url := traverse_obj(
            video_info, ("publicVideo", "adaptiveVideoDashUrl", {url_or_none})
        ):
            formats.extend(
                self._extract_mpd_formats(mpd_url, video_id, mpd_id="dash", fatal=False)
            )
        for key in ("low", "medium", "high"):
            if video_url := traverse_obj(
                video_info, ("publicVideo", f"{key}QualityVideoUrl", {url_or_none})
            ):
                formats.append(
                    {
                        "url": video_url,
...
class MicrosoftLearnSessionIE(InfoExtractor):
    _VALID_URL = (
        r"https?://learn\.microsoft\.com/(?:[\w-]+/)?events/[\w-]+/(?P<id>[^?#/]+)"
    )
    _TESTS = [
        {
