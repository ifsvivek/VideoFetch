...
class ZDFBaseIE(InfoExtractor):
    ...
    def _get_api_token(self):
        ...
        if not self._token_cache:
            self._token_cache.update(
                self.cache.load(*self._TOKEN_CACHE_PARAMS, default={})
            )

        if traverse_obj(self._token_cache, ("expires", {int_or_none}), default=0) < int(
            time.time()
        ):
            self._token_cache.update(
                self._download_json(
        ...
        for ptmd_url in variadic(ptmd_urls):
            ...
            basename = (
                ...
                or self._search_regex(
                    r"/vod/ptmd/[^/?#]+/(\w+)", ptmd_url, "content ID", default=None
                )
            )
            ...
            if not duration:
                duration = traverse_obj(
                    ptmd,
                    ("attributes", "duration", "value", {float_or_none(scale=1000)}),
                )
            ...
            for stream in traverse_obj(
                ...
                for quality in traverse_obj(stream, ("qualities", ..., {dict})):
                    for variant in traverse_obj(
                        quality, ("audio", "tracks", lambda _, v: url_or_none(v["uri"]))
                    ):
                        ...
                        if ext == "m3u8":
                            ...
                            width = (
                                round(aspect_ratio * height)
                                if aspect_ratio and height
                                else None
                            )
                            fmts = [
                                {
                            ...
                            self.report_warning(
                                f'Skipping unsupported extension "{ext}"',
                                video_id=video_id,
                            )
                            fmts = []
                        ...
                        for f in fmts:
                            f_lang = ISO639Utils.short2long(
                                (
                                    f.get("language") or variant.get("language") or ""
                                ).lower()
                            )
                                    ...
                                    **f,
                                    ...
                                    "language_preference": (
                                        ...
                                        if (
                                            ...
                                            10
                                            if f_lang == "deu" and f_class == "main"
                                            else (
                                                5
                                                if f_lang == "deu"
                                                else 1 if f_class == "main" else -1
                                            )
                                        )
                                    ),
                                }
    ...
    def _download_graphql(self, item_id, data_desc, query=None, body=None):
        ...
        return self._download_json(
            ...
            item_id,
            f"Downloading {data_desc}",
            f"Failed to download {data_desc}",
            query=query,
        ...
        return [
            {
                ...
                **traverse_obj(
                    re.search(
                        r"(?P<width>\d+|auto)[Xx](?P<height>\d+|auto)", str(format_id)
                    ),
                    {
            ...
            for format_id, url in traverse_obj(
                source, ({dict.items}, lambda _, v: url_or_none(v[1]))
            )
        ]
  ...
  videoByCanonical(canonical: $canonical) {
    ...
    def _extract_fallback(self, document_id):
        video = self._download_json(
            ...
            document_id,
            note="Downloading fallback metadata",
            errnote="Failed to download fallback metadata",
        )
        document = video["document"]

        ptmd_url = traverse_obj(
            document,
            (
    ...
    def _real_extract(self, url):
        ...
        video_data = self._download_graphql(
            video_id,
            "video metadata",
            body={
        ...
        for node in traverse_obj(
            video_data, ("currentMedia", "nodes", lambda _, v: v["ptmdTemplate"])
        ):
        ...
        return {
            **traverse_obj(
                video_data,
                {
                    "title": ("title", {str}),
                    "description": (
                        ("leadParagraph", ("teaser", "description")),
                        any,
                        {str},
                    ),
            ...
            **self._extract_ptmd(
                ptmd_urls, video_id, self._get_api_token(), aspect_ratio
            ),
            "id": video_id,
        }
...
class ZDFChannelIE(ZDFBaseIE):
    ...
    def _fetch_page(
        ...
        return self._download_graphql(
            playlist_id,
            f"season {season_number} page {page_number} JSON",
            query={
    ...
    def _entries(
        self, playlist_id, canonical_id, season_numbers, requested_season_number
    ):
        for season_idx, season_number in enumerate(season_numbers):
            if (
                requested_season_number is not None
                and requested_season_number != season_number
            ):
                continue
            ...
            for page_number in itertools.count(1):
                ...
                for episode in traverse_obj(
                    ...
                    (
                        ...
                        "episodes",
                        "nodes",
                        lambda _, v: url_or_none(v["sharingUrl"]),
                    ),
                ):
                ...
                page_info = (
                    traverse_obj(nodes, (-1, "episodes", "pageInfo", {dict})) or {}
                )
    ...
    def _real_extract(self, url):
        ...
        if not self._yes_playlist(
            ...
            return self.url_result(
                video_data["sharingUrl"], ZDFIE, video_data["canonical"]
            )
        ...
        return self.playlist_result(
            ...
            join_nonempty(
                ...
                season_number and f"Season {season_number}",
                delim=" - ",
            ),
