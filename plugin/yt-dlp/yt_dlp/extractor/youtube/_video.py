...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    def _prepare_live_from_start_formats(
        ...
        def refetch_manifest(format_id, delay):
            ...
            _, live_status, formats, _ = self._list_formats(
    ...
    def _live_dash_fragments(
        self, video_id, format_id, live_start_time, mpd_feed, manifestless_orig_fmt, ctx
    ):
        ...
        def _extract_sequence_from_mpd(refresh_sequence, immediate):
            ...
            if manifestless_orig_fmt:
                ...
                if not fmts:
                    ...
                    return False, last_seq
                fmt_info = next(
                    x for x in fmts if x["manifest_stream_number"] == stream_number
                )
        ...
        while is_live:
            ...
            if lack_early_segments:
                known_idx = max(
                    known_idx, last_seq - int(MAX_DURATION // fragments[-1]["duration"])
                )
            try:
    ...
    def _get_player_js_version(self):
        player_js_version = (
            self._configuration_arg("player_js_version", [""])[0]
            or self._DEFAULT_PLAYER_JS_VERSION
        )
    ...
    def _construct_player_url(self, *, player_id=None, player_url=None):
        ...
        if player_id_override and player_id_override != player_id:
            force_player_id = (
                f"Forcing player {player_id_override} in place of player {player_id}"
            )
            player_id = player_id_override

        variant = (
            self._configuration_arg("player_js_variant", [""])[0]
            or self._DEFAULT_PLAYER_JS_VARIANT
        )
        ...
        if not player_url:
            ...
            return urljoin(
                ...
                f"/s/player/{player_id}/{self._PLAYER_JS_VARIANT_MAP[variant]}",
            )
        ...
        self.write_debug(
            join_nonempty(
                force_player_id,
                variant != actual_variant
                and f'Forcing "{variant}" player JS variant for player {player_id}',
    ...
    def _get_player_id_variant_and_path(self, player_url):
        ...
        player_path = remove_start(
            urllib.parse.urlparse(player_url).path, f"/s/player/{player_id}/"
        )
        variant = self._INVERSE_PLAYER_JS_VARIANT_MAP.get(player_path) or next(
            (
                ...
                if re.fullmatch(
                    re.escape(k).replace("en_US", r"[a-zA-Z0-9_]+"), player_path
                )
            ),
            None,
        )
    ...
    def _player_js_cache_key(self, player_url):
        player_id, variant, player_path = self._get_player_id_variant_and_path(
            player_url
        )
        if not variant:
    ...
    def _extract_signature_timestamp(
        ...
        """
        CACHE_ENABLED = (
            False  # TODO: enable when preprocessed player JS cache is solved/enabled
        )
        ...
        return (
            traverse_obj(
                ...
                (
                    ...
                    "entityBatchUpdate",
                    "mutations",
                    lambda _, v: v["payload"]["macroMarkersListEntity"]["markersList"][
                        "markerType"
                    ]
                    == "MARKER_TYPE_HEATMAP",
                    "payload",
                    ...
                    ...,
                    {
                        ...
                        "end_time": {
                            lambda x: (int(x["startMillis"]) + int(x["durationMillis"]))
                            / 1000
                        },
    ...
    def _extract_comment(self, entities, parent=None):
        ...
        toolbar_entity_payload = get_first(
            entities, ("payload", "engagementToolbarStateEntityPayload", {dict})
        )
        time_text = (
            traverse_obj(comment_entity_payload, ("properties", "publishedTime", {str}))
            or ""
        )
    ...
    def _comment_entries(
        self, root_continuation_data, ytcfg, video_id, parent=None, tracker=None
    ):
        ...
        def extract_header(contents):
            ...
            for content in contents:
                ...
                if not sort_text:
                    sort_text = (
                        "top comments" if comment_sort_index == 0 else "newest first"
                    )
            ...
            for content in contents:
                ...
                # new comment format
                    ...
                    entities = traverse_obj(
                        entity_payloads, lambda _, v: v["entityKey"] in comment_keys
                    )
                    comment = self._extract_comment(entities, parent)
                    if comment:
                        comment["is_pinned"] = (
                            traverse_obj(view_model, ("pinnedText", {str})) is not None
                        )
                ...
                if comment_id in tracker["seen_comment_ids"]:
                    if comment_id in tracker["pinned_comment_ids"] and not comment.get(
                        "is_pinned"
                    ):
                ...
                tracker[
                    "total_reply_comments" if parent else "total_parent_comments"
                ] += 1
                yield comment
                ...
                if comment_replies_renderer:
                    ...
                    yield from itertools.islice(
                        ...
                        min(
                            ...
                            max(0, max_replies - tracker["total_reply_comments"]),
                        ),
                    )
        ...
        for page_num in itertools.count(0):
            ...
            for continuation_items in traverse_obj(
                response, continuation_items_path, expected_type=list, default=[]
            ):
                if is_first_continuation:
    ...
    def fetch_po_token(
        ...
        **kwargs,
        ...
        if "true" in traverse_obj(
            experiments, (..., "html5_generate_content_po_token", -1)
        ):
            self.write_debug(
                f"{video_id}: Detected experiment to bind GVS PO Token to video id.",
                only_once=True,
            )
    ...
    def _fetch_po_token(self, client, **kwargs):
        ...
        fetch_pot_policy = self._configuration_arg("fetch_pot", [""], ie_key=YoutubeIE)[
            0
        ]
        if fetch_pot_policy not in ("never", "auto", "always"):
            fetch_pot_policy = "auto"
        if fetch_pot_policy == "never" or (
            fetch_pot_policy == "auto" and not kwargs.get("required", False)
        ):
            return None
    ...
    def _is_agegated(player_response):
        if traverse_obj(
            player_response, ("playabilityStatus", "desktopLegacyAgeGateReason")
        ):
            return True
        ...
        return any(
            expected in reason for expected in AGE_GATE_REASONS for reason in reasons
        )
    ...
    def _extract_player_response(
        ...
        headers = self.generate_api_headers(
            ...
            delegated_session_id=(
                ...
                or self._extract_delegated_session_id(
                    webpage_ytcfg, initial_pr, player_ytcfg
                )
            ),
            user_session_id=(
        ...
        sts = (
            self._extract_signature_timestamp(
                video_id, player_url, webpage_ytcfg, fatal=False
            )
            if player_url
            else None
        )
    ...
    def _get_requested_clients(self, url, smuggled_data, is_premium_subscriber):
        requested_clients = []
        excluded_clients = []
        js_runtime_available = any(
            p.is_available() for p in self._jsc_director.providers.values()
        )
        default_clients = (
        ...
        if (
            not (requested_clients or excluded_clients)
            and default_clients == self._DEFAULT_JSLESS_CLIENTS
        ):
            self.report_warning(
                ...
                f'you can use  --extractor-args "youtube:player_client=default"',
                only_once=True,
            )
    ...
    def _extract_player_responses(
        ...
        while clients:
            ...
            if (
                ...
                and client != webpage_client
            ):
                player_ytcfg = self._download_ytcfg(client, video_id) or player_ytcfg

            player_url = player_url or self._extract_player_url(
                webpage_ytcfg, player_ytcfg, webpage=webpage
            )
            ...
            player_pot_policy: PlayerPoTokenPolicy = self._get_default_ytcfg(client)[
            ...
            player_po_token = (
                ...
                if pr
                    ...
                    **fetch_po_token_args,
                    required=player_pot_policy.required
                    or player_pot_policy.recommended,
                )
            )
            ...
            if pr_id := self._invalid_player_response(pr, video_id):
                ...
                innertube_context = traverse_obj(
                    player_ytcfg or self._get_default_ytcfg(client), "INNERTUBE_CONTEXT"
                )
            ...
            embedding_is_disabled = variant == "web_embedded" and self._is_unplayable(
                pr
            )
            if self.is_authenticated and (
                self._is_agegated(pr) or embedding_is_disabled
            ):
                self.to_screen(
            ...
            if status not in (
                ...
                self.write_debug(
                    f"{video_id}: {client} player response playability status: {status}"
                )
    ...
    def _needs_live_processing(self, live_status, duration):
        if (live_status == "is_live" and self.get_param("live_from_start")) or (
            live_status == "post_live" and (duration or 0) > 2 * 3600
        ):
            return live_status
    ...
    def _report_pot_subtitles_skipped(self, video_id, client_name, msg=None):
        ...
        # Only raise a warning for non-default clients, to not confuse users.
        if not subs_wanted or client_name in (
    ...
    def _extract_formats_and_subtitles(
        self, video_id, player_responses, player_url, live_status, duration
    ):
        CHUNK_SIZE = 10 << 20
        ...
        def is_super_resolution(f_url):
            return "1" in traverse_obj(
                f_url, ({parse_qs}, "xtags", ..., {urllib.parse.parse_qs}, "sr", ...)
            )
        ...
        playback_wait = int_or_none(
            self._configuration_arg("playback_wait", [None])[0], default=6
        )
        ...
        for pr in player_responses:
            ...
            available_at = (
                streaming_data[STREAMING_DATA_FETCHED_TIMESTAMP] + playback_wait
            )
            ...
            def process_format_stream(
                fmt_stream, proto, missing_pot, super_resolution=False
            ):
                ...
                tbr = float_or_none(
                    fmt_stream.get("averageBitrate") or fmt_stream.get("bitrate"), 1000
                )
                format_duration = traverse_obj(
                    fmt_stream, ("approxDurationMs", {float_or_none(scale=1000)})
                )
                ...
                if missing_pot and "missing_pot" not in self._configuration_arg(
                    "formats"
                ):
                ...
                dct = {
                    ...
                    "format_id": join_nonempty(
                        ...
                        (
                            "drc"
                            if fmt_stream.get("isDrc")
                            else "sr" if super_resolution else None
                        ),
                    ),
            ...
            def process_https_formats():
                ...
                for fmt_stream in streaming_formats:
                    ...
                    if fmt_stream.get("type") == "FORMAT_STREAM_TYPE_OTF" and not bool(
                        fmt_stream.get("drmFamilies")
                    ):
                        continue
                    ...
                    pot_policy: GvsPoTokenPolicy = self._get_default_ytcfg(client_name)[
                        "GVS_PO_TOKEN_POLICY"
                    ][StreamingProtocol.HTTPS]

                    require_po_token = stream_id[0] not in ["18"] and gvs_pot_required(
                        pot_policy, is_premium_subscriber, player_token_provided
                    )
                    ...
                    if encrypted_sig:
                        spec_cache_id = self._sig_spec_cache_id(
                            player_url, len(encrypted_sig)
                        )
                        spec = self._load_sig_spec_from_cache(spec_cache_id)
                        if spec:
                            self.write_debug(
                                f"Using cached signature function {spec_cache_id}",
                                only_once=True,
                            )
                            fmt_url += "&{}={}".format(
                    ...
                    if query.get("n"):
                        ...
                        if n_challenge in self._player_cache:
                            fmt_url = update_url_query(
                                fmt_url, {"n": self._player_cache[n_challenge]}
                            )
                        else:
                ...
                for fmt in https_fmts:
                    ...
                    if s_challenge is not None:
                        s_challenges.setdefault(len(s_challenge), {}).setdefault(
                            s_challenge, []
                        ).append(fmt)
                    ...
                    challenge_requests.append(
                        JsChallengeRequest(
                            ...
                            input=NChallengeInput(
                                challenges=list(n_challenges.keys()),
                                player_url=player_url,
                            ),
                        )
                    )
                ...
                if challenge_requests:
                    for (
                        _challenge_request,
                        challenge_response,
                    ) in self._jsc_director.bulk_solve(challenge_requests):
                        if challenge_response.type == JsChallengeType.SIG:
                            for (
                                challenge,
                                result,
                            ) in challenge_response.output.results.items():
                                spec_id = len(challenge)
                                spec = [ord(c) for c in result]
                                self._store_sig_spec_to_cache(
                                    self._sig_spec_cache_id(player_url, spec_id), spec
                                )
                    ...
                    for cfmts in list(s_challenges.values()) + list(
                        n_challenges.values()
                    ):
                        for fmt in cfmts:
            ...
            def process_manifest_format(f, proto, client_name, itag, missing_pot):
                ...
                if lang_code and lang_code == language_map[ORIGINAL_LANG_VALUE]:
                    f["format_note"] = join_nonempty(
                        f.get("format_note"), "(original)", delim=" "
                    )
            ...
            hls_manifest_url = "hls" not in skip_manifests and streaming_data.get(
                "hlsManifestUrl"
            )
            if hls_manifest_url:
                pot_policy: GvsPoTokenPolicy = self._get_default_ytcfg(client_name)[
                ...
                require_po_token = gvs_pot_required(
                    pot_policy, is_premium_subscriber, player_token_provided
                )
            ...
            dash_manifest_url = "dash" not in skip_manifests and streaming_data.get(
                "dashManifestUrl"
            )
                ...
                "fragments": [
                    {
                        ...
                        "duration": min(
                            fragment_duration, duration - (j * fragment_duration)
                        ),
                    }
            ...
            if post_live
                ...
                if is_live
                    ...
                    if is_upcoming
                        ...
                        "was_live"
                        if live_content
                        else "not_live" if False in (is_live, live_content) else None
                    )
                )
            )
        )
        *formats, subtitles = self._extract_formats_and_subtitles(
            video_id, player_responses, player_url, live_status, duration
        )
    ...
    def _download_initial_data(self, video_id, webpage, webpage_client, webpage_ytcfg):
        ...
        if webpage and "initial_data" not in self._configuration_arg("webpage_skip"):
            ...
            if not traverse_obj(initial_data, "contents"):
                ...
                initial_data = None
        if not initial_data and "initial_data" not in self._configuration_arg(
            "player_skip"
        ):
    ...
    def _is_premium_subscriber(self, initial_data):
        ...
        tlr = traverse_obj(
            initial_data,
            ("topbar", "desktopTopbarRenderer", "logo", "topbarLogoRenderer"),
        )
        return (
    ...
    def _initial_extract(
        self, url, smuggled_data, webpage_url, webpage_client, video_id
    ):
        ...
        webpage_ytcfg = self.extract_ytcfg(
            video_id, webpage
        ) or self._get_default_ytcfg(webpage_client)

        initial_data = self._download_initial_data(
    ...
    def _real_extract(self, url):
        ...
        initial_vdhr = (
            traverse_obj(
                initial_sdcr,
                ("items", ..., "videoDescriptionHeaderRenderer", {dict}, any),
            )
            or {}
        )
        ...
        initial_title = self._get_text(initial_vdhr, "title") or self._get_text(
            initial_video_details_renderer, "title"
        )
        ...
        if not formats:
            ...
            if subreason:
                if subreason.startswith(
                    ...
                    if not countries:
                        ...
                        countries = (
                            regions_allowed.split(",") if regions_allowed else None
                        )
            ...
            if reason:
                if "sign in" in reason.lower():
                    ...
                    reason = (
                        f'{remove_end(reason.strip(), ".")}. {self._youtube_login_hint}'
                    )
                elif get_first(
                    playability_statuses,
                    ("errorScreen", "playerCaptchaViewModel", {dict}),
                ):
                    reason += (
        ...
        info = {
            ...
            "channel_url": format_field(
                channel_id, None, "https://www.youtube.com/channel/%s", default=None
            ),
            "duration": duration,
            ...
            "media_type": (
                ...
                else "short" if get_first(microformats, "isShortsEligible") else "video"
            ),
            ...
            "_format_sort_fields": (  # source_preference is lower for potentially damaged formats
                ...
                (
                    "vcodec:vp9.2"
                    if "prefer-vp9-sort" in self.get_param("compat_opts", [])
                    else "vcodec"
                ),
                "channels",
                "acodec",
        ...
        def process_language(
            container, base_url, lang_code, sub_name, client_name, query
        ):
                ...
                lang_subs.append(
                    {
                        "ext": fmt,
                        "url": urljoin(
                            "https://www.youtube.com", update_url_query(base_url, query)
                        ),
                        "name": sub_name,
                        "impersonate": True,
        ...
        get_translated_subs = "translated_subs" not in self._configuration_arg(
            "skip"
        ) and (
        ...
        prs = traverse_obj(
            player_responses,
            (
                lambda _, v: v["streamingData"]
                and v["captions"]["playerCaptionsTracklistRenderer"]
            ),
        )
        ...
        need_subs_langs = {
            get_lang_code(sub) for sub in all_captions if sub.get("kind") != "asr"
        }
        need_caps_langs = {
        ...
        for pr in prs:
            ...
            innertube_client_name = pr["streamingData"][
                STREAMING_DATA_INNERTUBE_CONTEXT
            ]["client"]["clientName"]
            pot_policy: GvsPoTokenPolicy = self._get_default_ytcfg(client_name)[
                "SUBS_PO_TOKEN_POLICY"
            ]
            fetch_subs_po_token_func = pr["streamingData"][
            ...
            for caption_track in traverse_obj(
                pctr, ("captionTracks", lambda _, v: v["baseUrl"])
            ):
                ...
                if not already_fetched_pot:
                    ...
                    if subs_po_token := fetch_subs_po_token_func(
                        required=requires_pot or pot_policy.recommended
                    ):
                        pot_params.update(
                            {
                                "pot": subs_po_token,
                ...
                for trans_code, trans_name in translation_languages.items():
                    ...
                    process_language(
                        ...
                        (
                            ...
                            else {"tlang": trans_code, **pot_params}
                        ),
                    )
        ...
        if skipped_subs_clients and (need_subs_langs or need_caps_langs):
            self._report_pot_subtitles_skipped(
                ...
                msg=join_nonempty(
                    ...
                    need_subs_langs
                    and f'Subtitles for these languages are missing: {", ".join(need_subs_langs)}.',
                    need_caps_langs
                    and f"Automatic captions for {len(need_caps_langs)} languages are missing.",
                    delim=" ",
                ),
            )
        ...
        info["comment_count"] = traverse_obj(
            ...
            (
                "engagementPanels",
                lambda _, v: v["engagementPanelSectionListRenderer"]["panelIdentifier"]
        ...
        try:  # This will error if there is no livechat
            initial_data["contents"]["twoColumnWatchNextResults"]["conversationBar"][
                "liveChatRenderer"
                ...
                for tbr in tbrs:
                    for getter, regex in [
                        ...
                        if label:
                            ...
                            if mobj:
                                info[mobj.group("type") + "_count"] = str_to_int(
                                    mobj.group("count")
                                )
                                break
        ...
        if vsir:
            ...
            info.update(
                {
                    ...
                    or (collaborators[0] if collaborators else None),
            ...
            rows = (
                try_get(
                    vsir,
                    lambda x: x["metadataRowContainer"]["metadataRowContainerRenderer"][
                        "rows"
                    ],
                    list,
                )
            ...
            for row in rows:
                if (
                    try_get(row, lambda x: x["metadataRowRenderer"]["hasDividerLine"])
                    is True
                ):
                    multiple_songs = True
                    break
        ...
        info["__post_extractor"] = self.extract_comments(
            webpage_ytcfg, video_id, contents, webpage
        )
