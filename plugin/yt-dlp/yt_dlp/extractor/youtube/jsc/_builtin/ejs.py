...
class ScriptSource(enum.Enum):
    ...
    BUILTIN = (
        "builtin"  # vendored (full core script; import-only lib script + NPM cache)
    )
...
class EJSBaseJCP(JsChallengeProvider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ejs_settings = self.ie.get_param("extractor_args", {}).get(
            "youtube-ejs", {}
        )
        ...
        if custom_repo:
            self.report_dev_option(
                f"You have set a custom GitHub repository for EJS JCP Providers ({custom_repo})."
            )
    ...
    def _real_bulk_solve(self, /, requests: list[JsChallengeRequest]):
        ...
        for player_url, grouped_requests in grouped.items():
            ...
            if player:
                ...
                cached = False
                video_id = next(
                    (request.video_id for request in grouped_requests), None
                )
            ...
            if self._ENABLE_PREPROCESSED_PLAYER_CACHE and (
                ...
                self.ie.cache.store(
                    self._CACHE_SECTION, f"player:{player_url}", preprocessed
                )

            for request, response_data in zip(
                grouped_requests, output["responses"], strict=True
            ):
                if response_data["type"] == "error":
                    yield JsChallengeProviderResponse(
                        request, None, response_data["error"]
                    )
                else:
                    yield JsChallengeProviderResponse(
                        request,
    ...
    def _construct_stdin(
        self, player: str, preprocessed: bool, requests: list[JsChallengeRequest], /
    ) -> str:
        json_requests = [
            {
            ...
            if not self.is_dev:
                if script.version != self._SCRIPT_VERSION:
                    ...
                    if script.source is ScriptSource.CACHE:
                        ...
                        self.ie.cache.store(
                            self._CACHE_SECTION, script_type.value, None
                        )
                    continue
                script_hashes = self._ALLOWED_HASHES[script.type].get(
                    script.variant, []
                )
    ...
    def _iter_script_sources(
        self,
    ) -> Generator[tuple[ScriptSource, Callable[[ScriptType], Script | None]]]:
        yield from [
