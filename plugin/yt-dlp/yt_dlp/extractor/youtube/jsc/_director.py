...
if typing.TYPE_CHECKING:
    ...
    from yt_dlp.extractor.youtube.jsc.provider import (
        Preference as JsChallengePreference,
    )
...
class JsChallengeRequestDirector:
    ...
    def _get_providers(
        ...
        if self.logger.log_level <= self.logger.LogLevel.TRACE:
            ...
            self.logger.trace(
                f"JS Challenge Providers: {provider_display_list(self.providers.values())}"
            )
            self.logger.trace(
                "JS Challenge Provider preferences for this request: {}".format(
                    ", ".join(
                        f"{provider.PROVIDER_NAME}={pref}"
                        for provider, pref in preferences.items()
                    )
                )
            )
    ...
    def bulk_solve(
        self, requests: list[JsChallengeRequest]
    ) -> list[tuple[JsChallengeRequest, JsChallengeResponse]]:
        ...
        for provider in self._get_providers(next_requests):
            ...
            try:
                for response in provider.bulk_solve(
                    [dataclasses.replace(request) for request in next_requests]
                ):
                ...
                if (
                    isinstance(e, JsChallengeProviderRejectedRequest)
                    and e._skipped_components
                ):
    ...
    def __report_skipped_components(self, components: list[_SkippedComponent], /):
        ...
        self.logger.warning(
            f"{msg}. For more information and alternatives, refer to  {_EJS_WIKI_URL}"
        )
...
def initialize_jsc_director(ie):
    ...
    enable_trace = (
        ie._configuration_arg(
            "jsc_trace", ["false"], ie_key="youtube", casesense=False
        )[0]
        == "true"
    )
    ...
    if director.logger.log_level <= director.logger.LogLevel.DEBUG:
        ...
        director.logger.debug(
            f"JS Challenge Providers: {provider_display_list(director.providers.values())}"
        )
        director.logger.trace(
            f"Registered {len(director.preferences)} JS Challenge provider preferences"
        )
...
def validate_response(
    response: JsChallengeResponse, request: JsChallengeRequest
) -> bool | str:
...
def validate_nsig_challenge_output(
    challenge_output: NChallengeOutput, challenge_input: NChallengeInput
) -> bool | str:
    if not (
        ...
        and all(
            ...
            for k, v in challenge_output.results.items()
        )
        and all(
            challenge in challenge_output.results
            for challenge in challenge_input.challenges
        )
    ):
...
def validate_sig_challenge_output(
    challenge_output: SigChallengeOutput, challenge_input: SigChallengeInput
) -> bool:
    return (
