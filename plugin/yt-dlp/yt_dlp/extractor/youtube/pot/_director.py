...
class YoutubeIEContentProviderLogger(IEContentProviderLogger):
    def __init__(
        self, ie, prefix, log_level: IEContentProviderLogger.LogLevel | None = None
    ):
        self.__ie = ie
        self.prefix = prefix
...
class PoTokenCache:
    ...
    def __init__(
        ...
        self.cache_provider_preferences: list[CacheProviderPreference] = (
            cache_provider_preferences or []
        )
    ...
    def _get_cache_providers(
        self, request: PoTokenRequest
    ) -> Iterable[PoTokenCacheProvider]:
        """Sorts available cache providers by preference, given a request"""
        preferences = {
            provider: sum(
                pref(provider, request) for pref in self.cache_provider_preferences
            )
        ...
        return (
            ...
            for provider in sorted(
                self.cache_providers.values(), key=preferences.get, reverse=True
            )
    ...
    def store(
        ...
        default_expires_at = (
            int(dt.datetime.now(dt.timezone.utc).timestamp()) + spec.default_ttl
        )
        cache_response = dataclasses.replace(
            response, expires_at=response.expires_at or default_expires_at
        )
...
def initialize_pot_director(ie):
    ...
    enable_trace = (
        ie._configuration_arg(
            "pot_trace", ["false"], ie_key="youtube", casesense=False
        )[0]
        == "true"
    )
    ...
    for cache_spec_provider in _pot_pcs_providers.value.values():
        logger, settings = get_provider_logger_and_settings(
            cache_spec_provider, "pot:cache:spec"
        )
    ...
    if director.logger.log_level <= director.logger.LogLevel.DEBUG:
        ...
        director.logger.trace(
            f"Registered {len(cache.cache_provider_preferences)} cache provider preferences"
        )
...
def provider_display_list(providers: Iterable[IEContentProvider]):
    ...
    return (
        ", ".join(provider_display_name(provider) for provider in providers) or "none"
    )
...
def validate_response(response: PoTokenResponse | None):
    ...
    return response.expires_at <= 0 or response.expires_at > int(
