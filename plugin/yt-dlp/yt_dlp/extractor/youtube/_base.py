...
class YoutubeBaseInfoExtractor(InfoExtractor):
    ...
    _PLAYLIST_ID_RE = (
        r"(?:(?:PL|LL|EC|UU|FL|RD|UL|TL|PU|OLAK5uy_)[0-9A-Za-z-_]{10,}|RDMM|WL|LL|LM)"
    )
    ...
    _YT_HANDLE_RE = (
        r"@[\w.-]{3,30}"  # https://support.google.com/youtube/answer/11585688?hl=en
    )
    ...
    def _get_default_ytcfg(self, client="web"):
        ...
        if ytcfg.get("AUTHENTICATED_USER_AGENT") and self.is_authenticated:
            client_context = ytcfg.setdefault("INNERTUBE_CONTEXT", {}).setdefault(
                "client", {}
            )
    ...
    def _select_api_hostname(self, req_api_hostname, default_client=None):
        return (
            self._configuration_arg(
                "innertube_host", [""], ie_key=CONFIGURATION_ARG_KEY
            )[0]
            or req_api_hostname
        ...
        for scheme, sid in (
            ...
            if sid:
                authorizations.append(
                    self._make_sid_authorization(scheme, sid, origin, additional_parts)
                )
    ...
    def _call_api(
        ...
        data = (
            {"context": context}
            if context
            else {"context": self._extract_context(default_client=default_client)}
        )
        data.update(query)
    ...
    def extract_yt_initial_data(self, item_id, webpage, fatal=True):
        return self._search_json(
            self._YT_INITIAL_DATA_RE, webpage, "yt initial data", item_id, fatal=fatal
        )
    ...
    def _extract_data_sync_id(self, *args):
        ...
        if data_sync_id := self._configuration_arg(
            "data_sync_id", [None], ie_key=CONFIGURATION_ARG_KEY, casesense=True
        )[0]:
            return data_sync_id
    ...
    def _extract_visitor_data(self, *args):
        ...
        if visitor_data := self._configuration_arg(
            "visitor_data", [None], ie_key=CONFIGURATION_ARG_KEY, casesense=True
        )[0]:
            return visitor_data
        return get_first(
            args,
    ...
    def _download_ytcfg(self, client, video_id):
        ...
        if client == "tv" and not self.is_authenticated:
            config_info = (
                traverse_obj(
                    ytcfg, ("INNERTUBE_CONTEXT", "client", "configInfo", {dict})
                )
                or {}
            )
    ...
    def _report_alerts(self, alerts, expected=True, fatal=True, only_once=False):
        ...
        for alert_type, alert_message in warnings + errors[:-1]:
            self.report_warning(
                f"YouTube said: {alert_type} - {alert_message}", only_once=only_once
            )
        if errors:
    ...
    def _extract_badges(self, badge_list: list):
        ...
        for badge in traverse_obj(
            badge_list, (..., lambda key, _: re.search(r"[bB]adgeRenderer$", key))
        ):
    ...
    def _get_count(self, data, *path_list):
        ...
        if count is None:
            count = str_to_int(
                self._search_regex(
                    r"^([\d,]+)", re.sub(r"\s", "", count_text), "count", default=None
                )
            )
        return count
    ...
    def _parse_time_text(self, text, report_failure=True):
        ...
        if (
            ...
            and self._preferred_lang in (None, "en")
        ):
            self.report_warning(
    ...
    def _extract_response(
        ...
        raise_for_incomplete = bool(
            self._configuration_arg(
                "raise_incomplete_data", ie_key=CONFIGURATION_ARG_KEY
            )
        )
                    ...
                    yt_error = try_get(
                        self._parse_json(
                            self._webpage_read_content(
                                e.cause.response, None, item_id, prefix=first_bytes
                            )
                            or "{}",
                            item_id,
                            fatal=False,
