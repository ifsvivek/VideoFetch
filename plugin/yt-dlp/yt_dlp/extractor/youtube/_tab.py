...
class YoutubeTabBaseInfoExtractor(YoutubeBaseInfoExtractor):
        ...
        def wrapper(self, url):
            ...
            if smuggled_data:
                ...
                if info_dict.get("entries"):
                    info_dict["entries"] = (
                        _smuggle(i, smuggled_data.copy()) for i in info_dict["entries"]
                    )
            return info_dict
    ...
    def _extract_video(self, renderer):
        ...
        title = self._get_text(renderer, "title", "headline") or self._get_text(
        ...
        scheduled_timestamp = str_to_int(
            traverse_obj(renderer, ("upcomingEventData", "startTime"), get_all=False)
        )
        ...
        view_count_text = (
            self._get_text(renderer, "viewCountText", "shortViewCountText", "videoInfo")
            or ""
        )
        view_count = (
            0
        ...
        return {
            ...
            "availability": (
                ...
                if self._has_badge(badges, BadgeType.AVAILABILITY_PUBLIC)
                    ...
                    or None,
                    is_unlisted=self._has_badge(badges, BadgeType.AVAILABILITY_UNLISTED)
                    or None,
                )
            ),
            ...
            "channel_is_verified": (
                True if self._has_badge(owner_badges, BadgeType.VERIFIED) else None
            ),
        }
    ...
    def _extract_channel_renderer(self, renderer):
        ...
        return {
            ...
            "channel_is_verified": (
                True
                if self._has_badge(
                    self._extract_badges(traverse_obj(renderer, "ownerBadges")),
                    BadgeType.VERIFIED,
                )
                else None
            ),
    ...
    def _extract_lockup_view_model(self, view_model):
        ...
        return self.url_result(
            ...
            thumbnails=self._extract_thumbnails(
                view_model,
                ("contentImage", *thumb_keys, "thumbnailViewModel", "image"),
                final_key="sources",
            ),
            ...
            timestamp=(
                traverse_obj(
                    ...
                    (
                        ...
                        "text",
                        "content",
                        {lambda t: self._parse_time_text(t, report_failure=False)},
                        any,
                    ),
                )
                if self._configuration_arg("approximate_date", ie_key=YoutubeTabIE)
            ...
            response = self._extract_response(
                ...
                check_get_keys=(
                    ...
                    (
                        ...
                        ...,
                        lambda k, v: k == "key"
                        and v == "GetRecommendedMusicPlaylists_rid",
                    ),
                ),
            )
    ...
    def _extract_metadata_from_tabs(self, item_id, data):
        ...
        primary_thumbnails = self._extract_thumbnails(
            ...
            (
                "thumbnailRenderer",
                ("playlistVideoThumbnailRenderer", "playlistCustomThumbnailRenderer"),
                "thumbnail",
            ),
        )
        ...
        if not info.get("channel_id"):
            ...
            browse_ep = (
                traverse_obj(owner, ("runs", 0, "navigationEndpoint", "browseEndpoint"))
                or {}
            )
            info.update(
                {
                    ...
                    "uploader_id": self.handle_from_url(
                        urljoin(
                            "https://www.youtube.com", browse_ep.get("canonicalBaseUrl")
                        )
                    ),
                }
            )
    ...
    def _extract_inline_playlist(self, playlist, playlist_id, data, ytcfg):
        ...
        for page_num in itertools.count(1):
            ...
            if not videos:
                return
            start = (
                next((i for i, v in enumerate(videos) if v["id"] == last_id), -1) + 1
            )
            if start >= len(videos):
                return
            ...
            playlist = try_get(
                response,
                lambda x: x["contents"]["twoColumnWatchNextResults"]["playlist"][
                    "playlist"
                ],
                dict,
            )
    ...
    def _extract_availability(self, data):
        ...
        return (
            ...
            if (
                ...
                or None,
                needs_premium=self._has_badge(badges, BadgeType.AVAILABILITY_PREMIUM)
                or None,
                needs_auth=False,
            )
        )
    ...
    def _extract_webpage(self, url, item_id, fatal=True):
        ...
        for retry in self.RetryManager(fatal=fatal):
            try:
                ...
                data = (
                    self.extract_yt_initial_data(item_id, webpage or "", fatal=fatal)
                    or {}
                )
            ...
            if not traverse_obj(
                data, "contents", "currentVideoEndpoint", "onResponseReceivedActions"
            ):
    ...
    def _report_playlist_authcheck(self, ytcfg, fatal=True):
        ...
        if not ytcfg and self.is_authenticated:
            ...
            if (
                "authcheck"
                not in self._configuration_arg("skip", ie_key=YoutubeTabIE.ie_key())
                and fatal
            ):
...
class YoutubeTabIE(YoutubeTabBaseInfoExtractor):
    ...
    def _extract_tab_id_and_name(self, tab, base_url="https://www.youtube.com"):
        ...
        tab_id = (tab_url and self._get_url_mobj(tab_url)["tab"][1:]) or traverse_obj(
            tab, "tabIdentifier", expected_type=str
        )
        if tab_id:
            return {
...
class YoutubePlaylistIE(YoutubeBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        url = update_url_query(
            "https://www.youtube.com/playlist", parse_qs(url) or {"list": playlist_id}
        )
        if is_music_url:
