...
query BootstrapPlayer {
  rpc {
    ...
    def _download_course_rpc(self, course_id, url, display_id):
        ...
        course = try_get(
            response, lambda x: x["data"]["rpc"]["bootstrapPlayer"]["course"], dict
        )
        if course:
            return course
  ...
  viewClip(input: {
    ...
    def _perform_login(self, username, password):
        ...
        if all(
            ...
            if BLOCKED in response:
                raise ExtractorError(f"Unable to login: {BLOCKED}", expected=True)
    ...
    def _convert_subtitles(duration, subs):
        ...
        for num, current in enumerate(subs):
            ...
            start, text = (
                float_or_none(
                    dict_get(current, TIME_OFFSET_KEYS, skip_false_values=False)
                ),
