...
class XHamsterIE(InfoExtractor):
    ...
    def _decipher_format_url(self, format_url, format_id):
        ...
        if cipher_type == "xor":
            return bytes(
                a ^ b
                for a, b in zip(ciphertext.encode(), itertools.cycle(self._XOR_KEY))
            ).decode()
    ...
    def _fixup_formats(self, formats):
        for f in formats:
            ...
            for vcodec in ("av1", "h264"):
                if any(
                    f".{vcodec}." in f_url
                    for f_url in (f["url"], f.get("manifest_url", ""))
                ):
                    f["vcodec"] = vcodec
                    break
        return formats
    ...
    def _real_extract(self, url):
        ...
        categories = (
            [
                clean_html(category)
                for category in re.findall(r"<a[^>]+>(.+?)</a>", categories_html)
            ]
            if categories_html
            else None
        )
...
class XHamsterEmbedIE(InfoExtractor):
    _VALID_URL = (
        rf"https?://(?:[^/?#]+\.)?{XHamsterIE._DOMAINS}/xembed\.php\?video=(?P<id>\d+)"
    )
    _EMBED_REGEX = [
