...
class NBCUniversalBaseIE(ThePlatformBaseIE):
    ...
    def _download_nbcu_smil_and_extract_m3u8_url(self, tp_path, video_id, query):
        smil = self._download_xml(
            f"https://link.theplatform.com/s/{tp_path}",
            video_id,
            "Downloading SMIL manifest",
            "Failed to download SMIL manifest",
            query={
                **query,
        ...
        ns = f"//{{{default_ns}}}"
        if url := traverse_obj(
            smil, (f"{ns}video/@src", lambda _, v: determine_ext(v) == "m3u8", any)
        ):
            return url

        exc = traverse_obj(
            smil,
            (f"{ns}param", lambda _, v: v.get("name") == "exception", "@value", any),
        )
    ...
    def _extract_nbcu_formats_and_subtitles(self, tp_path, video_id, query):
        ...
        orig_m3u8_url = m3u8_url = self._download_nbcu_smil_and_extract_m3u8_url(
            tp_path, video_id, query
        )

        if mobj := re.fullmatch(self._M3U8_RE, m3u8_url):
            query["formats"] = "mpeg4"
            m3u8_tmpl = self._download_nbcu_smil_and_extract_m3u8_url(
                tp_path, video_id, query
            )
    ...
    def _extract_nbcu_video(self, url, display_id, old_ie_key=None):
        ...
        if tve:
            account_pid = (
                tve.get("data-mpx-media-account-pid") or tve["data-mpx-account-pid"]
            )
            account_id = tve["data-mpx-media-account-id"]
            metadata = self._parse_json(
                tve.get("data-normalized-video") or "",
                display_id,
                fatal=False,
            ...
            if tve.get("data-entitlement") == "auth":
                ...
                query["auth"] = self._extract_mvpd_auth(
                    url,
                    release_pid,
                    auth["adobePassRequestorId"],
                    resource,
                    auth["adobePassSoftwareStatement"],
                )
        else:
            ls_playlist = traverse_obj(
                settings,
                (
                    "ls_playlist",
                    lambda _, v: v["defaultGuid"],
                    any,
                    {require("LS playlist")},
                ),
            )
            ...
            account_pid = (
                ls_playlist.get("mpxMediaAccountPid") or ls_playlist["mpxAccountPid"]
            )
            account_id = ls_playlist["mpxMediaAccountId"]
            metadata = (
                traverse_obj(
                    ls_playlist, ("videos", lambda _, v: v["guid"] == video_id, any)
                )
                or {}
            )

        tp_path = f"{account_pid}/media/guid/{account_id}/{video_id}"
        formats, subtitles = self._extract_nbcu_formats_and_subtitles(
            tp_path, video_id, query
        )
        tp_metadata = self._download_theplatform_metadata(
            tp_path, video_id, fatal=False
        )
        ...
        return {
            ...
            **traverse_obj(
            ...
            "subtitles": subtitles,
            "_old_archive_ids": (
                [make_archive_id(old_ie_key, video_id)] if old_ie_key else None
            ),
        }
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "info_dict": {
                ...
                "_old_archive_ids": [
                    "theplatform para24_sww_alitruwittodayshow_240823"
                ],
            },
            "params": {
                "skip_download": "m3u8",
      ...
      ... on VideoPageData {
        ...
        if not video_data:
            ...
            video_data = self._search_json(
                r"<script>\s*PRELOAD\s*=", webpage, "video data", video_id
            )["pages"][urllib.parse.urlparse(url).path]["base"]["metadata"]
...
class NBCSportsVPlayerIE(InfoExtractor):
    _WORKING = False
    _VALID_URL_BASE = (
        r"https?://(?:vplayer\.nbcsports\.com|(?:www\.)?nbcsports\.com/vplayer)/"
    )
...
class NBCSportsIE(InfoExtractor):
    _WORKING = False
    _VALID_URL = (
        r"https?://(?:www\.)?nbcsports\.com//?(?!vplayer/)(?:[^/]+/)+(?P<id>[0-9a-z-]+)"
    )
...
class NBCSportsStreamIE(AdobePassIE):
    ...
    def _real_extract(self, url):
        ...
        live_source = self._download_json(
            f"http://stream.nbcsports.com/data/live_sources_{video_id}.json", video_id
        )
        ...
        is_live = (
            video_source.get("type") == "live" or video_source.get("status") == "Live"
        )
        resource = self._get_mvpd_resource("nbcsports", title, video_id, "")
        token = self._extract_mvpd_auth(
            url, video_id, "nbcsports", resource, None
        )  # XXX: None arg needs to be software_statement
...
class NBCStationsIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        nbc_data = self._search_json(
            r"(?:<script>\s*var\s+nbc\s*=|Object\.assign\(nbc,)",
            webpage,
            "NBC JSON data",
            video_id,
        )
        ...
        if is_live:
            ...
            player_id = traverse_obj(
                video_data,
                ((None, ("video", "meta")), ("pid_streaming_web_high", "mpx_pid")),
                get_all=False,
            )
            ...
            video_url = traverse_obj(
                video_data, ((None, ("video", "meta")), "mp4_url"), get_all=False
            )
            if video_url:
                ext = determine_ext(video_url)
                height = self._search_regex(
                    r"\d+-(\d+)p", url_basename(video_url), "height", default=None
                )
                formats.append(
                    {
                        "url": video_url,
        ...
        if player_id and fw_ssid:
            ...
            if not isinstance(smil, xml.etree.ElementTree.Element):
                smil = None
        subtitles = (
            self._parse_smil_subtitles(smil, default_ns) if smil is not None else {}
        )
        for video in (
...
class BravoTVIE(NBCUniversalBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?(?:bravotv|oxygen)\.com/(?:[^/?#]+/)+(?P<id>[^/?#]+)"
    )
    _TESTS = [
        {
            ...
            "info_dict": {
                ...
                "chapters": [
                    ...
                    },
                    {
                        "start_time": 1870.0,
                        "end_time": 2496.967,
                        "title": "<Untitled Chapter 5>",
                    },
                    {
                        "start_time": 2496.967,
                        "end_time": 2599,
                        "title": "<Untitled Chapter 6>",
                    },
                ],
                "series": "Face Off",
                "season": "Season 13",
