        ...
        if item.get("type") != "video":
            ...
            if items.get("markup"):
                entries.extend(
                    ...
                    for yt_url in YoutubeIE._extract_embed_urls(url, items["markup"])
                )

            return self.playlist_result(
                entries, item.get("uuid"), item.get("title"), item.get("summary")
            )
...
class YahooJapanNewsIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://news\.yahoo\.co\.jp/(?:articles|feature)/(?P<id>[a-zA-Z0-9]+)"
    )
    _GEO_COUNTRIES = ["JP"]
    _TESTS = [
        {
    ...
    def _extract_formats(self, json_data, content_id):
        ...
        for vid in (
            traverse_obj(
                json_data, ("ResultSet", "Result", ..., "VideoUrlSet", "VideoUrl", ...)
            )
            or []
        ):
    ...
    def _real_extract(self, url):
        ...
        json_data = self._download_json(
            ...
            query={
                ...
                "domain": HOST,
                "ak": (
                    hashlib.md5("_".join((space_id, HOST)).encode()).hexdigest()
                    if space_id
                    else ""
                ),
                "device_type": "1100",
