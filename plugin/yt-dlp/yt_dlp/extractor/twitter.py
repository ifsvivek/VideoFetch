...
class TwitterBaseIE(InfoExtractor):
    ...
    def _extract_variant_formats(self, variant, video_id):
        ...
        if not variant_url:
            ...
            fmts, subs = self._extract_m3u8_formats_and_subtitles(
                variant_url, video_id, "mp4", "m3u8_native", m3u8_id="hls", fatal=False
            )
            for f in traverse_obj(
                fmts, lambda _, v: v["vcodec"] == "none" and v.get("tbr") is None
            ):
                if mobj := re.match(
    ...
    def _extract_formats_from_vmap_url(self, vmap_url, video_id):
        ...
        for video_variant in vmap_data.findall(
            ...
            fmts, subs = self._extract_variant_formats(video_variant.attrib, video_id)
            formats.extend(fmts)
    ...
    def _set_base_headers(self, legacy=False):
        bearer_token = (
            self._LEGACY_AUTH if legacy and not self.is_logged_in else self._AUTH
        )
        return filter_dict(
            {
    ...
    def _call_graphql_api(self, endpoint, media_id):
        ...
        return traverse_obj(
            self._call_api(endpoint, media_id, query=query, graphql=True), "data"
        )
...
class TwitterCardIE(InfoExtractor):
    ...
    _VALID_URL = (
        TwitterBaseIE._BASE_REGEX + r"i/(?:cards/tfw/v1|videos(?:/tweet)?)/(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            "https://twitter.com/statuses/" + status_id, TwitterIE.ie_key(), status_id
        )
...
class TwitterIE(TwitterBaseIE):
    ...
    def _extract_status(self, twid):
        if self._selected_api not in ("graphql", "legacy", "syndication"):
            raise ExtractorError(
                f"{self._selected_api!r} is not a valid API selection", expected=True
            )
        ...
        def extract_from_video_info(media):
            ...
            return {
                ...
                "duration": float_or_none(
                    traverse_obj(media, ("video_info", "duration_millis")), 1000
                ),
...
class TwitterAmplifyIE(TwitterBaseIE):
    ...
    def _real_extract(self, url):
        ...
        vmap_url = self._html_search_meta("twitter:amplify:vmap", webpage, "vmap url")
        ...
        def _find_dimension(target):
            w = int_or_none(
                self._html_search_meta(f"twitter:{target}:width", webpage, fatal=False)
            )
            h = int_or_none(
                self._html_search_meta(f"twitter:{target}:height", webpage, fatal=False)
            )
            return w, h
...
class TwitterBroadcastIE(TwitterBaseIE, PeriscopeBaseIE):
    ...
    _VALID_URL = (
        TwitterBaseIE._BASE_REGEX + r"i/(?P<type>broadcasts|events)/(?P<id>\w+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        info.update(
            {
                ...
                "uploader_id": broadcast.get("twitter_username")
        ...
        if info["live_status"] == "is_upcoming":
            self.raise_no_formats(
                "This live broadcast has not yet started", expected=True
            )
            return info

        media_key = broadcast["media_key"]
        source = self._call_api(f"live_video_stream/status/{media_key}", media_key)[
            "source"
        ]
        m3u8_url = source.get("noRedirectPlaybackUrl") or source["location"]
        if "/live_video_stream/geoblocked/" in m3u8_url:
            self.raise_geo_restricted()
        m3u8_id = urllib.parse.parse_qs(urllib.parse.urlparse(m3u8_url).query).get(
            "type", [None]
        )[0]
    ...
    def _real_extract(self, url):
        ...
        if live_status == "is_upcoming":
            ...
            source = traverse_obj(
                ...
                ("source", ("noRedirectPlaybackUrl", "location"), {url_or_none}),
                get_all=False,
            )
        ...
        participants = (
            ", ".join(
                traverse_obj(
                    space_data, ("participants", "speakers", ..., "display_name")
                )
            )
            or "nobody yet"
        )
                ...
                (
                    ...
                    "legacy",
                    {
                        ...
                        "thumbnail": (
                            ...
                            {lambda x: x.replace("_normal", "_400x400")},
                            {url_or_none},
                        ),
                    },
                ),
