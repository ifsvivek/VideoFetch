...
class MSNIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        locale, display_id, page_id = self._match_valid_url(url).group(
            "locale", "display_id", "id"
        )
        ...
        common_metadata = traverse_obj(
            json_data,
            {
                "title": ("title", {str}),
                "description": (
                    ("abstract", ("body", {clean_html})),
                    {str},
                    filter,
                    any,
                ),
        ...
        if page_type == "video":
            if (
                traverse_obj(json_data, ("thirdPartyVideoPlayer", "enabled"))
                and source_url
            ):
