...
class BitmovinIE(InfoExtractor):
    ...
    _EMBED_REGEX = [
        r'<iframe\b[^>]+\bsrc=["\'](?P<url>(?:https?:)?//streams\.bitmovin\.com/(?P<id>\w+)[^"\']+)'
    ]
    _TESTS = [
        {
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        for stream_id in re.findall(
            r'<bitmovin-stream\b[^>]*\bstream-id=["\'](?P<id>\w+)', webpage
        ):
    ...
    def _real_extract(self, url):
        ...
        player_config = self._download_json(
            f"https://streams.bitmovin.com/{video_id}/config", video_id
        )["sources"]
