    ...
    def _vuejs_entries(self, url, website_data, page_id):
        ...
        if website_data.get("items_total_count") > website_data.get("items_per_page"):
            for page in itertools.count(2):
                ...
                if not page_website_data.get("videos") and not page_website_data.get(
                    "items"
                ):
                    break
...
class TVPStreamIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        channel = (
            traverse_obj(
                channels, (lambda _, v: channel_id == str(v["id"])), get_all=False
            )
            if channel_id
            else channels[0]
        )
            ...
            if ext == "m3u8":
                formats.extend(
                    self._extract_m3u8_formats(
                        video_url, video_id, m3u8_id="hls", fatal=False, live=is_live
                    )
                )
            elif ext == "mpd":
                if is_live:
                ...
                formats.append(
                    {
                        ...
                        "tbr": int_or_none(
                            traverse_obj(file, ("quality", "bitrate")), scale=1000
                        ),
...
class TVPVODVideoIE(TVPVODBaseIE):
    ...
    def _real_extract(self, url):
        ...
        playlist = self._call_api(
            f"{video_id}/videos/playlist", video_id, query={"videoType": "MOVIE"}
        )
...
class TVPVODSeriesIE(TVPVODBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            ...
            clean_html(
                traverse_obj(
                    metadata, ("description", "lead"), expected_type=strip_or_none
                )
            ),
