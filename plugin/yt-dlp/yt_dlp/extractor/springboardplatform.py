...
class SpringboardPlatformIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        content = xpath_element(
            item, "./{http://search.yahoo.com/mrss/}content", "content", fatal=True
        )
        ...
        if "error_video.mp4" in video_url:
            raise ExtractorError(f"Video {video_id} no longer exists", expected=True)
        ...
        thumbnail = xpath_attr(
            item, "./{http://search.yahoo.com/mrss/}thumbnail", "url", "thumbnail"
        )
