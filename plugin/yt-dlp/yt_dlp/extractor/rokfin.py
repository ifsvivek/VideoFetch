...
class RokfinIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_url = traverse_obj(
            metadata, "url", ("content", "contentUrl"), expected_type=url_or_none
        )
        ...
        return {
            ...
            "thumbnail": url_or_none(
                traverse_obj(metadata, "thumbnail", ("content", "thumbnailUrl1"))
            ),
            "description": str_or_none(
                traverse_obj(metadata, "description", ("content", "contentDescription"))
            ),
            "like_count": int_or_none(metadata.get("likeCount")),
            "dislike_count": int_or_none(metadata.get("dislikeCount")),
            "channel": str_or_none(
                traverse_obj(metadata, ("createdBy", "name"), ("creator", "name"))
            ),
            "channel_id": str_or_none(
                traverse_obj(metadata, ("createdBy", "id"), ("creator", "id"))
            ),
            "channel_url": (
                url_or_none(f"https://rokfin.com/{uploader}") if uploader else None
            ),
            "timestamp": timestamp,
...
class RokfinPlaylistBaseIE(InfoExtractor):
    ...
    def _get_video_data(self, metadata):
        for content in metadata.get("content") or []:
            ...
            video_id = (
                content.get("id") if media_type == "post" else content.get("mediaId")
            )
...
class RokfinChannelIE(RokfinPlaylistBaseIE):
    ...
    def _entries(self, channel_id, channel_name, tab):
        ...
        for page_n in itertools.count(0):
            if tab in ("posts", "top"):
                data_url = (
                    f"{_API_BASE_URL}user/{channel_name}/{tab}?page={page_n}&size=50"
                )
            else:
...
class RokfinSearchIE(SearchInfoExtractor):
    ...
    def _get_db_access_credentials(self, video_id=None):
        ...
        for js_file_path in re.findall(
            r'<script\b[^>]*\ssrc\s*=\s*"(/static/js/[^">]+)"', notfound_err_page
        ):
