...
class HuyaLiveIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.|m\.)?huya\.com/(?!(?:video/play/))(?P<id>[^/#?&]+)(?:\D|$)"
    )
    IE_NAME = "huya:live"
    IE_DESC = "虎牙直播"
    _TESTS = [
    ...
    def _real_extract(self, url):
        ...
        for stream_info in stream_info_list:
            ...
            params = dict(
                urllib.parse.parse_qsl(unescapeHTML(stream_info["sFlvAntiCode"]))
            )
            fm, ss = "", ""
            if re_secret:
