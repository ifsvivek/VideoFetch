...
class VideoKenBaseIE(InfoExtractor):
    ...
    def _create_slideslive_url(self, video_url, video_id, referer):
        if not video_url and not video_id:
            ...
            video_url = (
                f'https://slideslive.com/embed/{remove_start(video_id, "slideslive-")}'
            )
        ...
        for video in traverse_obj(videos, (("videos", "results"), ...)):
            ...
            if traverse_obj(video, "type", "source") == "youtube":
                ...
                video_url = traverse_obj(
                    video, "embed_url", "embeddableurl", expected_type=url_or_none
                )
                if not video_url:
                    continue
    ...
    def _real_extract(self, url):
        ...
        total_pages = int_or_none(
            self._get_topic_page(
                topic, org_id, search_id, api_key, note="Downloading topic info"
            )["total_no_of_pages"]
        )
        return self.playlist_result(
            InAdvancePagedList(
