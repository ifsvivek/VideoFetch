                ...
                info["comments"].append(
                    {
                        ...
                        "text": join_nonempty(
                            "reviewtitle", "reviewbody", from_dict=review, delim="\n\n"
                        ),
...
class YoutubeWebArchiveIE(InfoExtractor):
    ...
    def _extract_metadata(self, video_id, webpage):
        ...
        if not player_response:
            ...
            player_response = self._parse_json(
                traverse_obj(player_config, ("args", "player_response")) or "{}",
                video_id,
                fatal=False,
            )
        ...
        _CHANNEL_URL_HREF_RE = (
            r'href="[^"]*(?P<url>https?://www\.youtube\.com/(?:user|channel)/[^"]+)"'
        )
        ...
        uploader_url = (
            f"https://www.youtube.com/user/{uploader_id}" if uploader_id else None
        )
    ...
    def _real_extract(self, url):
        ...
        video_info = self._download_json(
            "https://web.archive.org/__wb/videoinfo",
            video_id,
        ...
        for fmt in traverse_obj(
            video_info, ("formats", lambda _, v: url_or_none(v["url"]))
        ):
            ...
            formats.append(
                {
                    ...
                    **traverse_obj(
                        fmt,
                        {
                            "url": (
                                "url",
                                {lambda x: f"https://web.archive.org/web/2id_/{x}"},
                            ),
                            "ext": ("ext", {str}),
