...
class TencentBaseIE(InfoExtractor):
    ...
    def _extract_all_video_formats_and_subtitles(self, url, video_id, series_id):
        api_responses = [
            self._get_video_api_response(url, video_id, series_id, "srt", "hls", "hd")
        ]
        self._check_api_response(api_responses[0])
        qualities = traverse_obj(api_responses, (0, "fl", "fi", ..., "name")) or (
            "shd",
            "fhd",
        )
        for q in qualities:
...
class VQQVideoIE(VQQBaseIE):
    IE_NAME = "vqq:video"
    _VALID_URL = (
        VQQBaseIE._VALID_URL_BASE + r"/x/(?:page|cover/(?P<series_id>\w+))/(?P<id>\w+)"
    )
...
class WeTvBaseIE(TencentBaseIE):
    ...
    def _extract_series(self, url, ie):
        ...
        episode_paths = [
            ...
            r'<a[^>]+class="play-video__link"[^>]+href="(?P<path>[^"]+)', webpage
        )
