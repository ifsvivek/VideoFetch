...
class VKUserVideosIE(VKBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(page_id, section), f"{page_id}_{section}", playlist_title
        )
            ...
            entries.append(
                {
                    ...
                    "thumbnails": try_call(
                        lambda: [{"url": u} for u in audio["coverUrl"].split(",")]
                    ),
...
class VKPlayIE(VKPlayBaseIE):
    _VALID_URL = (
        rf"{VKPlayBaseIE._BASE_URL_RE}(?P<username>[^/#?]+)/record/(?P<id>[\da-f-]+)"
    )
    _TESTS = [
        {
