...
class GeniusIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?genius\.com/(?:videos|(?P<article>a))/(?P<id>[^?/#]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        if not video_id:
            ...
            raise ExtractorError(
                "Brightcove video ID not found in webpage", expected=bool(is_article)
            )
...
class GeniusLyricsIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        title = traverse_obj(
            song_info,
            ("songPage", "trackingData", lambda _, x: x["key"] == "Title", "value"),
            get_all=False,
            default="untitled",
        )
        artist = traverse_obj(
