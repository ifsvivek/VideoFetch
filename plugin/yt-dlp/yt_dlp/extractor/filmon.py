    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            if isinstance(e.cause, HTTPError):
                errmsg = self._parse_json(e.cause.response.read().decode(), video_id)[
                    "reason"
                ]
    ...
    def _real_extract(self, url):
        ...
        try:
            channel_data = self._download_json(
                "http://www.filmon.com/api-v2/channel/" + channel_id, channel_id
            )["data"]
        except ExtractorError as e:
            if isinstance(e.cause, HTTPError):
                errmsg = self._parse_json(e.cause.response.read().decode(), channel_id)[
                    "message"
                ]
                raise ExtractorError(f"{self.IE_NAME} said: {errmsg}", expected=True)
            raise
