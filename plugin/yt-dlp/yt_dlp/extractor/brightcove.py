...
class BrightcoveNewIE(BrightcoveNewBaseIE):
    ...
    def _real_extract(self, url):
        ...
        account_id, player_id, embed, content_type, video_id = self._match_valid_url(
            url
        ).groups()
        ...
        def extract_policy_key():
            base_url = (
                f"https://players.brightcove.net/{account_id}/{player_id}_{embed}/"
            )
            config = (
                self._download_json(base_url + "config.json", video_id, fatal=False)
                or {}
            )
        ...
        if errors and errors[0].get("error_subcode") == "TVE_AUTH":
            ...
            missing_fields = ", ".join(
                key
                for key in ("source_url", "software_statement")
                if not smuggled_data.get(key)
            )
            if missing_fields:
                raise ExtractorError(
                    ...
                    f"Pass the URL of the embedding webpage instead of the Brightcove URL",
                    expected=True,
                )
