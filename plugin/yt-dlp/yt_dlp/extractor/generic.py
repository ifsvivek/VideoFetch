...
class GenericIE(InfoExtractor):
    ...
    def _extra_manifest_info(self, info, manifest_url):
        ...
        def hex_or_none(value):
            return (
                value if re.fullmatch(r"(0x)?[\da-f]+", value, re.IGNORECASE) else None
            )
            ...
            entries.append(
                {
                    ...
                    "thumbnail": url_or_none(
                        xpath_attr(it, xpath_with_ns("./itunes:image", NS_MAP), "href")
                    ),
                    "episode": itunes("title"),
                    "episode_number": int_or_none(itunes("episode")),
                    "season_number": int_or_none(itunes("season")),
                    "age_limit": {"true": 18, "yes": 18, "false": 0, "no": 0}.get(
                        (itunes("explicit") or "").lower()
                    ),
                }
            )
    ...
    def _kvs_get_real_url(cls, video_url, license_code):
        ...
        urlparts[3] = (
            "".join(hash_[index] for index in indices) + urlparts[3][HASH_LENGTH:]
        )
    ...
    def _extract_kvs(self, url, webpage, video_id):
        ...
        url_keys = list(
            filter(re.compile(r"^video_(?:url|alt_url\d*)$").match, flashvars.keys())
        )
        formats = []
        for key in url_keys:
        ...
        try:
            ...
            if doc.tag == "rss":
                ...
                return self.playlist_result(
                    self._parse_xspf(
                        doc, video_id, xspf_url=url, xspf_base_url=new_url
                    ),
                    video_id,
                )
                ...
                info_dict["formats"], info_dict["subtitles"] = (
                    self._parse_mpd_formats_and_subtitles(
                        ...
                        mpd_base_url=update_url(
                            new_url, query=None, fragment=None
                        ).rpartition("/")[0],
                        mpd_url=url,
                    )
                )
    ...
    def _extract_embeds(self, url, webpage, *, urlh=None, info_dict={}):
        ...
        if mobj is not None:
            ...
            sources = variadic(
                self._parse_json(
                    mobj.group(2), video_id, transform_source=js_to_json, fatal=False
                )
                or []
            )
