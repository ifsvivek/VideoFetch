...
class TedTalkIE(TedBaseIE):
    ...
    def _real_extract(self, url):
        ...
        talk_info = self._search_nextjs_data(webpage, display_id)["props"]["pageProps"][
            "videoData"
        ]
...
class TedPlaylistIE(TedBaseIE):
    ...
    def _real_extract(self, url):
        display_id = self._match_id(url)
        webpage = self._download_webpage(url, display_id)
        playlist = self._search_nextjs_data(webpage, display_id)["props"]["pageProps"][
            "playlist"
        ]
        ...
        return self.playlist_result(
            ...
            playlist.get("id"),
            playlist.get("title")
            or self._og_search_title(webpage, default="").replace(" | TED Talks", "")
            or None,
            self._og_search_description(webpage),
        )
