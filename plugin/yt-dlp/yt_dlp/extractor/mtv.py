...
class MTVServicesBaseIE(InfoExtractor):
    ...
    def _call_auth_api(
        ...
        return self._download_json(
            f"https://auth.mtvnservices.com/{path}",
            display_id,
            note=note or "Calling authentication API",
            data=data,
            headers=headers,
            query={**self._get_auth_suite_data(config), **(query or {})},
        )
    ...
    def _get_fresh_access_token(self, config, display_id=None, force_refresh=False):
        ...
        if (
            not force_refresh
            and access_token
            and not self._jwt_is_expired(access_token)
        ):
            return access_token
    ...
    def _get_media_token(self, video_config, config, display_id=None):
        ...
        if resource_id in self._token_cache:
            ...
            tokens = self._token_cache[resource_id] = (
                self.cache.load(self._CACHE_SECTION, resource_id) or {}
            )
        ...
        if not jwt_decode_hs256(access_token).get("accessMethods"):
            ...
            callback_url = update_url_query(
                config["callbackURL"],
                {
                    "authSuiteData": urllib.parse.quote(
                        base64.b64encode(auth_suite_data).decode()
                    ),
                    "mvpdCode": mso_id,
                },
            )
            ...
            self._call_auth_api(
                f"access/mvpd/{mso_id}",
                config,
                display_id,
                "Submitting authorization code to MTVNServices",
                query={"authorizationCode": authorization_code},
                data=b"",
                headers={"Authorization": f"Bearer {access_token}"},
            )
        ...
        tokens[self._MEDIA_TOKEN_KEY] = self._call_auth_api(
            ...
            data={
                "content": {
                    ("id" if k == "videoId" else k): v for k, v in video_config.items()
                },
    ...
    def _real_extract(self, url):
        ...
        try:
            data = self._download_json(
                update_url(url, query=None), display_id, query={"json": "true"}
            )
        except ExtractorError as e:
            if (
                isinstance(e.cause, HTTPError)
                and e.cause.status == 404
                and not self.suitable(e.cause.response.url)
            ):
                self.raise_geo_restricted(countries=self._GEO_COUNTRIES)
            raise

        flex_wrapper = traverse_obj(
            data,
            (
                "children",
                lambda _, v: v["type"] == "MainContainer",
                (None, ("children", lambda _, v: v["type"] == "AviaWrapper")),
                "children",
                lambda _, v: v["type"] == "FlexWrapper",
                {dict},
                any,
            ),
        )
        ...
        if not video_detail:
            video_detail = traverse_obj(
                ...
                (
                    "children",
                    ...,
                    ("handleTVEAuthRedirection", None),
                    "videoDetail",
                    {dict},
                    any,
                    {require("video detail")},
                ),
            )

        mgid = video_detail["mgid"]
        video_id = mgid.rpartition(":")[2]
        service_url = traverse_obj(
            video_detail, ("videoServiceUrl", {url_or_none}, {update_url(query=None)})
        )
        if not service_url:
        ...
        if video_detail.get("authRequired"):
            ...
            video_config = traverse_obj(
                flex_wrapper,
                (
                    "children",
                    lambda _, v: v["type"] == "AuthSuiteWrapper",
                    "props",
                    "videoConfig",
                    {dict},
                    any,
                ...
                video_detail,
                {
                    ...
                    "timestamp": (
                        (
                            ...
                            ("publishDate", "timestamp", {int_or_none}),
                        ),
                        any,
                    ),
                    "release_timestamp": (
...
class MTVIE(MTVServicesBaseIE):
    IE_NAME = "mtv"
    _VALID_URL = (
        r"https?://(?:www\.)?mtv\.com/(?:video-clips|episodes)/(?P<id>[\da-z]{6})"
    )
    _TESTS = [
        {
