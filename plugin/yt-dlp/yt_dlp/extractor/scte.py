...
class SCTEBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        def is_logged(webpage):
            return any(
                re.search(p, webpage) for p in (r'class=["\']welcome\b', r">Sign Out<")
            )
        ...
        response = self._download_webpage(
            self._LOGIN_URL, None, "Logging in", data=urlencode_postdata(login_form)
        )
...
class SCTEIE(SCTEBaseIE):
    ...
    def _real_extract(self, url):
        ...
        data = self._parse_xml(
            self._search_regex(r'CreateData\(\s*"(.+?)"', context, "data").replace(
                r"\'", "'"
            ),
            video_id,
        )
...
class SCTECourseIE(SCTEBaseIE):
    _WORKING = False
    _VALID_URL = (
        r"https?://learning\.scte\.org/(?:mod/sub)?course/view\.php?.*?\bid=(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        title = self._search_regex(r"<h1>(.+?)</h1>", webpage, "title", default=None)
