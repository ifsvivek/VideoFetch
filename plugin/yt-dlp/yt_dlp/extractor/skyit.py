...
class SkyItVideoLiveIE(SkyItBaseIE):
    ...
    def _real_extract(self, url):
        ...
        asset_id = str(
            self._search_nextjs_data(webpage, display_id)["props"]["initialState"][
                "livePage"
            ]["content"]["asset_id"]
        )
        livestream = self._download_json(
            "https://apid.sky.it/vdp/v1/getLivestream", asset_id, query={"id": asset_id}
        )
...
class TV8ItPlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        data = self._search_nextjs_data(webpage, playlist_id)["props"]["pageProps"][
            "data"
        ]
        entries = [
            self.url_result(
                urljoin("https://tv8.it", card["href"]),
                ie=TV8ItIE,
                **traverse_obj(
                    card,
                    {
            ...
            for card in traverse_obj(
                data, ("lastContent", "cards", lambda _, v: v["href"])
            )
        ]
