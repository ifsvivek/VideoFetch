...
class VimeoBaseInfoExtractor(InfoExtractor):
    ...
    def _parse_config(self, config, video_id):
            ...
            for cdn_name, cdn_data in (
                ...
                for f_id, m_url in sep_manifest_urls:
                    if files_type == "hls":
                        ...
                        if "json=1" in m_url:
                            real_m_url = (
                                self._download_json(m_url, video_id, fatal=False) or {}
                            ).get("url")
                            if real_m_url:
                                m_url = real_m_url
        ...
        if not is_live:
            ...
            thumbnails.extend(
                traverse_obj(
                    video_data, (("thumbnail", "thumbnail_url"), {"url": {url_or_none}})
                )
            )
    ...
    def _fetch_oauth_token(self, client):
        ...
        if not self._oauth_tokens.get(cache_key):
            self._oauth_tokens[cache_key] = self.cache.load(
                self._NETRC_MACHINE, cache_key
            )
        ...
        if not self._oauth_tokens.get(cache_key):
            if client_config["CACHE_ONLY"]:
                raise ExtractorError(
                    ...
                    f"and is only intended for use with previously cached tokens",
                    expected=True,
                )
            ...
            self._oauth_tokens[cache_key] = self._download_json(
                ...
                None,
                f"Fetching {client} OAuth token",
                f"Failed to fetch {client} OAuth token",
                headers={
            ...
            self.cache.store(
                self._NETRC_MACHINE, cache_key, self._oauth_tokens[cache_key]
            )
        ...
        if client := self._configuration_arg("client", [None], ie_key=VimeoIE)[0]:
            if client not in self._CLIENT_CONFIGS:
                raise ExtractorError(
                    ...
                    f'Supported clients are: {", ".join(self._CLIENT_CONFIGS)}',
                    expected=True,
                )
            self.write_debug(
                f"Using {client} API client as specified by extractor argument",
                only_once=True,
            )
            return client
    ...
    def _call_videos_api(
        ...
        **kwargs,
        ...
        return self._download_json(
            ...
            video_id,
            f"Downloading {client} API JSON",
            f"Unable to download {client} API JSON",
            headers=filter_dict(
                {
    ...
    def _extract_original_format(self, url, video_id, unlisted_hash=None):
        ...
        policy = self._configuration_arg(
            "original_format_policy", ["auto"], ie_key=VimeoIE
        )[0]
        if policy == "never":
            return None
        ...
        if not traverse_obj(privacy_info, ("privacy", "download", {bool})):
            ...
            if policy != "always":
                ...
                return None
            self.write_debug(
                f"{msg}; attempting to extract original/source format anyways"
            )
    ...
    def _get_embed_params(is_embed, referer):
        return {
            ...
            "referrer": (
                urllib.parse.urlparse(referer).hostname if referer and is_embed else ""
            ),
        }
...
class VimeoIE(VimeoBaseInfoExtractor):
    ...
    def _get_subtitles(self, video_id, unlisted_hash):
        ...
        for tt in traverse_obj(
            text_tracks, ("data", lambda _, v: url_or_none(v["link"]))
        ):
    ...
    def _parse_api_response(self, video, video_id, unlisted_hash=None):
        ...
        for file in traverse_obj(
            video,
            (
                (("play", (None, "progressive")), "files", "download"),
                ...
                fmt.update(
                    {
                        ...
                        "format_id": join_nonempty(
                            "http",
                            traverse_obj(file, "public_name", "rendition"),
                            quality,
                        ),
        ...
        if traverse_obj(
            video, ("metadata", "connections", "texttracks", "total", {int})
        ):
            self._merge_subtitles(
                self.extract_subtitles(video_id, unlisted_hash), target=subtitles
            )
    ...
    def _extract_from_api(self, video_id, unlisted_hash=None):
        for retry in (False, True):
            try:
                ...
                if (
                    ...
                    and "password"
                    in traverse_obj(response, ("invalid_parameters", ..., "field"))
                ):
        ...
        if config_url := traverse_obj(video, ("config_url", {url_or_none})):
            info = self._parse_config(
                self._download_json(config_url, video_id), video_id
            )
        else:
        ...
        connections = try_get(video, lambda x: x["metadata"]["connections"], dict) or {}
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            if not isinstance(error.cause, HTTPError) or error.cause.status not in (
                403,
                429,
            ):
                raise
        ...
        def is_rented():
            ...
            for purchase_option in vod.get("purchase_options") or []:
                ...
                if label and (
                    label.startswith("You rented this") or label.endswith(" remaining")
                ):
                    return True
            return False
        ...
        view_count = int_or_none(
            self._search_regex(r"UserPlays:(\d+)", webpage, "view count", default=None)
        )
        like_count = int_or_none(
            self._search_regex(r"UserLikes:(\d+)", webpage, "like count", default=None)
        )
...
class VimeoAlbumIE(VimeoBaseInfoExtractor):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "playlist_mincount": 9,
            "params": {
                "http_headers": {"Referer": "https://www.riccardomutimusic.com/"}
            },
        },
        {
    ...
    def _fetch_page(self, album_id, hashed_pass, is_embed, referer, page):
        ...
        for video in videos:
            ...
            uri = video.get("uri")
            video_id = (
                self._search_regex(r"/videos/(\d+)", uri, "id", default=None)
                if uri
                else None
            )
            if is_embed:
    ...
    def _real_extract(self, url):
        ...
        try:
            album, hashed_pass = self._get_album_data_and_hashed_pass(
                album_id, is_embed, referer
            )
        except ExtractorError as e:
            if (
                is_embed
        ...
        entries = OnDemandPagedList(
            functools.partial(
                self._fetch_page, album_id, hashed_pass, is_embed, referer
            ),
            self._PAGE_SIZE,
        )
...
class VHXEmbedIE(VimeoBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        config_url = self._parse_json(
            self._search_regex(r"window\.OTTData\s*=\s*({.+})", webpage, "ott data"),
            video_id,
            js_to_json,
        )["config_url"]
        config = self._download_json(config_url, video_id)
...
class VimeoProIE(VimeoBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            vimeo_url, VimeoIE, video_id, url_transparent=True, description=description
        )
...
class VimeoEventIE(VimeoBaseInfoExtractor):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "params": {
                "skip_download": "m3u8",
                "http_headers": {
                    "Referer": "https://www.media.mit.edu/events/aha-symposium/"
                },
            },
            "expected_warnings": ["Failed to parse XML: not well-formed"],
        },
    ...
    def _call_events_api(
        ...
        return self._download_json(
            join_nonempty(
                ...
                join_nonempty(event_id, unlisted_hash, delim=":"),
                ep,
                delim="/",
            ),
            event_id,
    ...
    def _vimeo_url_result(self, video_id, unlisted_hash=None, event_id=None):
        ...
        return self.url_result(
            join_nonempty("https://vimeo.com", video_id, unlisted_hash, delim="/"),
            VimeoIE,
            video_id,
            display_id=event_id,
            live_status="was_live",
    ...
    def _real_extract(self, url):
        ...
        event_id, unlisted_hash, video_id = self._match_valid_url(url).group(
            "id", "unlisted_hash", "video_id"
        )
        ...
        for retry in (False, True):
            try:
                live_event_data = self._call_events_api(
                    ...
                    referrer=referrer,
                    query={"clip_to_play_id": video_id or "0"},
                ...
                if (
                    retry
                    or not isinstance(e.cause, HTTPError)
                    or e.cause.status not in (400, 403)
                ):
                    raise
                response = (
                    traverse_obj(e.cause.response.read(), ({json.loads}, {dict})) or {}
                )
        ...
        if view_policy == "nobody":
            raise ExtractorError(
                "This event has not been made available to anyone", expected=True
            )
        ...
        if (
            ...
            self.raise_no_formats(
                f"This live event is scheduled for {start_time}", expected=True
            )
            ...
            if video_id:
                ...
                if (
                    video_id == clip_id
                    and clip_status == "done"
                    and (clip_hash or view_policy != "unlisted")
                ):
                    return self._vimeo_url_result(clip_id, clip_hash, event_id)

                video_filter = (
                    lambda _, v: self._extract_video_id_and_unlisted_hash(v)[0]
                    == video_id
                )
            else:
                video_filter = (
                    lambda _, v: traverse_obj(v, ("live", "status")) != "unavailable"
                )
            ...
            if (
                not video
            ):  # requested video_id is unavailable or no videos are available
        ...
        if config_url:  # view_policy == 'embed_only' or live_status == 'is_live'
            info = filter_dict(
                self._parse_config(
                    self._download_json(
                        config_url, event_id, "Downloading config JSON"
                    ),
                    event_id,
                )
            )
