...
class HotStarBaseIE(InfoExtractor):
            ...
            headers=filter_dict(
                {
                    **(
                        self._SUB_HEADERS
                        if self._has_active_subscription(cookies, st)
                        else self._FREE_HEADERS
                    ),
                    "hotstarauth": auth,
                    "x-hs-usertoken": traverse_obj(
                        cookies, (self._TOKEN_NAME, "value")
                    ),
                    "x-hs-device-id": traverse_obj(cookies, ("deviceId", "value"))
                    or str(uuid.uuid4()),
    ...
    def _fetch_page(self, path, item_id, name, query, root, page):
        results = self._call_api_v1(
            path,
            item_id,
            note=f"Downloading {name} page {page + 1} JSON",
            query={
                **query,
...
class HotStarIE(HotStarBaseIE):
    ...
    def _real_extract(self, url):
        ...
        content_type = (
            traverse_obj(video_data, ("contentType", {str}))
            or self._CONTENT_TYPE[video_type]
        )

        # See https://github.com/yt-dlp/yt-dlp/issues/396
        st = self._request_webpage(
            f"{self._BASE_URL}/in", video_id, "Fetching server time"
        ).get_header("x-origin-date")
        watch = self._call_api_v2("pages/watch", video_id, content_type, cookies, st)
        player_config = traverse_obj(
            watch,
            (
                "page",
                "spaces",
                "player",
                "widget_wrappers",
                lambda _, v: v["template"] == "PlayerWidget",
                "widget",
                "data",
                "player_config",
                {dict},
        ...
        for playback_set in traverse_obj(
            player_config,
            (
                ("media_asset", "media_asset_v2"),
                ("primary", "fallback"),
                all,
                lambda _, v: url_or_none(v["content_url"]),
            ),
        ):
            ...
            format_url = re.sub(
                r"(?<=//staragvod)(\d)", r"web\1", playback_set["content_url"]
            )
            ...
            for f in current_formats:
                ...
                if f.get("acodec") != "none" and not f.get("audio_channels"):
                    ...
                    if "Audio_Description" in f["format_id"] or "Audio Description" in (
