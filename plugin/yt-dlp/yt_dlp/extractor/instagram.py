...
class InstagramBaseIE(InfoExtractor):
    ...
    def _extract_product(self, product_info):
        ...
        info_dict = {
            "id": _pk_to_id(
                traverse_obj(product_info, "pk", "id", expected_type=str_or_none)[:19]
            ),
    ...
    def _get_comments(self, video_id):
        ...
        comment_data = traverse_obj(
            comments_info, ("edge_media_to_parent_comment", "edges"), "comments"
        )
        for comment_dict in comment_data or []:
            yield {
                "author": traverse_obj(
                    comment_dict, ("node", "owner", "username"), ("user", "username")
                ),
...
class InstagramIE(InstagramBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if not general_info:
            ...
            xdt_shortcode_media = (
                traverse_obj(general_info, ("data", "xdt_shortcode_media", {dict}))
                or {}
            )
        ...
        if dash:
            formats.extend(
                self._parse_mpd_formats(self._parse_xml(dash, video_id), mpd_id="dash")
            )
            ...
            [
                {
                    ...
                    "timestamp": traverse_obj(
                        comment_dict, ("node", "created_at"), expected_type=int_or_none
                    ),
                }
        ...
        return {
            ...
            "comment_count": self._get_count(
                media, "comments", "preview_comment", "to_comment", "to_parent_comment"
            ),
            "comments": comments,
            "thumbnails": thumbnails,
            "http_headers": {
...
class InstagramPlaylistBaseIE(InstagramBaseIE):
    ...
    def _extract_graphql(self, data, url):
        ...
        for page_num in itertools.count(1):
            ...
            nodes = (
                traverse_obj(media, ("edges", ..., "node"), expected_type=dict) or []
            )
            if not nodes:
                break
