    ...
    def _real_extract(self, url):
        ...
        for video_json in traverse_obj(
            ...
            if video_json.get("url"):
                # external video host
                entries.append(
                    self.url_result(video_json["url"], self._providers.get(provider))
                )
            ...
            entries.insert(
                0,
                {
                    **info_dict,
                    ...
                    "duration": duration,
                    "thumbnail": (
                        traverse_obj(video_json, ("poster", 0, "url", {url_or_none}))
        ...
        if not entries:
            self.raise_no_formats(
                "No video could be found in this post", expected=True, video_id=video_id
            )
        if len(entries) == 1:
            return {
                **info_dict,
