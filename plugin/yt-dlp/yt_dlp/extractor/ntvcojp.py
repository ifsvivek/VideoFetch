...
class NTVCoJpCUIE(StreaksBaseIE):
    ...
    IE_DESC = "日テレ無料TADA!"
    _VALID_URL = (
        r"https?://cu\.ntv\.co\.jp/(?!program-list|search)(?P<id>[\w-]+)/?(?:[?#]|$)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        info = self._search_json(r"window\.app\s*=", webpage, "video info", display_id)[
            "falcorCache"
        ]["catalog"]["episode"][display_id]["value"]
