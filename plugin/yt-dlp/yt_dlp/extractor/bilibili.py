    ...
    def _get_episodes_from_season(self, ss_id, url):
        ...
        for entry in traverse_obj(
            ...
            yield self.url_result(
                entry["share_url"], BiliBiliBangumiIE, str_or_none(entry.get("id"))
            )
    ...
    def _real_extract(self, url):
        ...
        if not initial_state:
            if self._search_json(
                r"\bwindow\._riskdata_\s*=", webpage, "risk", video_id, default={}
            ).get("v_voucher"):
                raise ExtractorError(
                    "You have exceeded the rate limit. Try again later", expected=True
                )
            query = {"platform": "web"}
            prefix = prefix.upper()
            if prefix == "BV":
        ...
        if video_data.get("is_upower_exclusive"):
            high_level = (
                traverse_obj(
                    initial_state, ("elecFullInfo", "show_info", "high_level", {dict})
                )
                or {}
            )
        ...
        if not traverse_obj(play_info, "dash"):
            ...
            for qn in traverse_obj(
                play_info, ("accept_quality", lambda _, v: not has_qn(v), {int})
            ):
                formats.extend(
                    traverse_obj(
                ...
                "entries": [
                    {
                        ...
                        "subtitles": (
                            self.extract_subtitles(video_id, cid) if idx == 0 else None
                        ),
                        "__post_extractor": (
        ...
        return {
            ...
            **traverse_obj(
                episode_info,
                {
                    ...
                    "title": {
                        lambda v: v
                        and join_nonempty("title", "long_title", delim=" ", from_dict=v)
                    },
                },
            ),
...
class BilibiliCheeseBaseIE(BilibiliBaseIE):
    def _extract_episode(self, season_info, ep_id):
        episode_info = traverse_obj(
            season_info, ("episodes", lambda _, v: v["id"] == int(ep_id)), get_all=False
        )
        ...
        return {
            ...
            **traverse_obj(
                episode_info,
                {
                    ...
                    "title": {
                        lambda v: v
                        and join_nonempty("index", "title", delim=" - ", from_dict=v)
                    },
        ...
        def fetch_page(page_idx):
            query = {
                ...
                "dm_img_str": base64.b64encode(
                    "".join(
                        random.choices(string.printable, k=random.randint(16, 64))
                    ).encode()
                )[:-2].decode(),
                "dm_cover_img_str": base64.b64encode(
                    "".join(
                        random.choices(string.printable, k=random.randint(32, 128))
                    ).encode()
                )[:-2].decode(),
...
class BilibiliSpaceListBaseIE(BilibiliSpaceBaseIE):
    def _get_entries(self, page_data, bvid_keys, ending_key="bvid"):
        for bvid in traverse_obj(
            page_data,
            (*variadic(bvid_keys, (str, bytes, dict, set)), ..., ending_key, {str}),
        ):
            yield self.url_result(
    ...
    def _get_uploader(self, uid, playlist_id):
        ...
        return self._search_regex(
            r"(?s)<title\b[^>]*>([^<]+)的个人空间-", webpage, "uploader", fatal=False
        )
...
class BilibiliCategoryIE(InfoExtractor):
    ...
    def _entries(self, category, subcategory, query):
        ...
        return OnDemandPagedList(
            functools.partial(self._fetch_page, api_url, num_pages, query), size
        )
    ...
    def json2srt(self, json):
        return "\n\n".join(
            ...
            for i, line in enumerate(
                traverse_obj(
                    json, ("body", lambda _, l: l["content"] and l["from"] and l["to"])
                )
            )
        )
                ...
                chapters = [
                    {
                        "start_time": float_or_none(
                            traverse_obj(
                                intro_ending_json, ("skip", "opening_start_time")
                            ),
                            1000,
                        ),
                    ...
                    },
                    {
                        "start_time": float_or_none(
                            traverse_obj(
                                intro_ending_json, ("skip", "ending_start_time")
                            ),
                            1000,
                        ),
                        "end_time": float_or_none(
