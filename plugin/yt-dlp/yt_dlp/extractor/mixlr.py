...
class MixlrIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        format_url = traverse_obj(
            broadcast_info,
            ("included", 0, "attributes", "progressive_stream_url", {url_or_none}),
        )
        if format_url:
        ...
        if not formats and release_timestamp:
            self.raise_no_formats(
                f"This event will start at {release_timestamp}", expected=True
            )
...
class MixlrRecoringIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        recording_info = self._download_json(
            f"https://api.mixlr.com/v3/channels/{username}/recordings/{recording_id}",
            recording_id,
        )
