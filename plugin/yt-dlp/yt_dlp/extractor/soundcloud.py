    ...
    def _extract_info_dict(
        ...
        for t in traverse_obj(
            ...
            abr = int_or_none(
                ...
                or self._search_regex(
                    r"\.(\d+)\.(?:opus|mp3)[/?]", stream_url, "abr", default=None
                )
            ...
            formats.append(
                {
                    ...
                    "abr": abr,
                    "protocol": (
                        "m3u8_native" if protocol in ("hls", "hls-aes") else "http"
                    ),
            ...
            if mobj := re.search(self._IMAGE_REPL_RE, thumbnail):
                for image_id, size in self._ARTWORK_MAP.items():
                    ...
                    i = {
                        "id": image_id,
                        "url": re.sub(
                            self._IMAGE_REPL_RE, f"-{image_id}.{ext}", thumbnail
                        ),
                    }
        ...
        return {
            ...
            "artists": traverse_obj(
                info, ("publisher_metadata", "artist", {str}, filter, all, filter)
            ),
    ...
    def _real_extract(self, url):
        ...
        if track.get("errors"):
            raise ExtractorError(
                ...
                % ",".join(str(err["error_message"]) for err in track["errors"]),
                expected=True,
            )
