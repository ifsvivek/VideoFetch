...
class KuwoIE(KuwoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if (
            ...
            raise ExtractorError(
                "this song has been offline because of copyright issues", expected=True
            )
...
class KuwoAlbumIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        album_name = self._html_search_regex(
            r'<div[^>]+class="comm"[^<]+<h1[^>]+title="([^"]+)"', webpage, "album name"
        )
        album_intro = remove_start(
            clean_html(get_element_by_id("intro", webpage)), f"{album_name}简介："
        )
...
class KuwoSingerIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        page_count = int(
            self._html_search_regex(r'data-page="(\d+)"', webpage, "page count")
        )
        ...
        def page_func(page_num):
            ...
            return [
                ...
                for song_url in re.findall(
                    r'<div[^>]+class="name"><a[^>]+href="(/yinyue/\d+)', webpage
                )
            ]
...
class KuwoCategoryIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        category_desc = remove_start(
            get_element_by_id("intro", webpage).strip(), f"{category_name}简介："
        )
        if category_desc == "暂无":
            category_desc = None
        ...
        entries = [
            self.url_result(
                "http://www.kuwo.cn/yinyue/{}/".format(song["musicrid"]), "Kuwo"
            )
