...
class RTBFIE(RedBeeBaseIE):
    ...
    def _perform_login(self, username, password):
        ...
        self._set_cookie(
            ".rtbf.be", "gmid", "gmid.ver4", secure=True, expire_time=time.time() + 3600
        )
    ...
    def _real_extract(self, url):
        ...
        if not media_data:
            if re.search(
                r'<div[^>]+id="js-error-expired"[^>]+class="(?![^"]*hidden)', embed_page
            ):
                raise ExtractorError("Livestream has ended.", expected=True)
            if re.search(
                r'<div[^>]+id="js-sso-connect"[^>]+class="(?![^"]*hidden)', embed_page
            ):
                self.raise_login_required()
        ...
        m3u8_url = (
            None if data.get("isLive") else traverse_obj(data, "urlHlsAes128", "urlHls")
        )
        if m3u8_url:
            fmts, subs = self._extract_m3u8_formats_and_subtitles(
        ...
        if formats and http_url and re.search(height_re, http_url):
            ...
            for key, format_id in self._QUALITIES:
                ...
                if not format_url:
                    continue
                height = int_or_none(
                    self._search_regex(height_re, format_url, "height", default=None)
                )
                formats.append(
                    {
                        "format_id": format_id,
