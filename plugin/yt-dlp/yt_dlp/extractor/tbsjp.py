...
class TBSJPBaseIE(StreaksBaseIE):
    def _search_window_app_json(self, webpage, name, item_id, **kwargs):
        return self._search_json(
            r"window\.app\s*=", webpage, f"{name} info", item_id, **kwargs
        )
...
class TBSJPProgramIE(TBSJPBaseIE):
    ...
    def _real_extract(self, url):
        ...
        programme = traverse_obj(
            meta, ("falcorCache", "catalog", "program", programme_id, "false", "value")
        )
        ...
        return {
            ...
            "entries": [
                self.url_result(
                    f"https://cu.tbs.co.jp/episode/{video_id}", TBSJPEpisodeIE, video_id
                )
                for video_id in traverse_obj(
                    programme, ("custom_data", "seriesList", "episodeCode", ...)
                )
            ],
            "id": programme_id,
            **traverse_obj(
...
class TBSJPPlaylistIE(TBSJPBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            entries(), playlist_id, traverse_obj(playlist, ("display_name", "value"))
        )
