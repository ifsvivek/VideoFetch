...
class CSpanIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        jwsetup = self._parse_json(
            self._search_regex(
                r"(?s)jwsetup\s*=\s*({.+?})\s*;", webpage, "jwsetup", default="{}"
            ),
            video_id,
        ...
        if jwsetup:
            info = self._parse_jwplayer_data(
                jwsetup, video_id, require_title=False, m3u8_id="hls", base_url=url
            )
            ...
            return merge_dicts(
                ...
                ld_info,
                {
                    ...
                    "timestamp": parse_iso8601(
                        get_element_by_attribute("itemprop", "uploadDate", webpage)
                    ),
        ...
        if data["@status"] != "Success":
            raise ExtractorError(
                "{} said: {}".format(self.IE_NAME, get_text_attr(data, "error")),
                expected=True,
            )
            ...
            entries.append(
                {
                    ...
                    "title": (
                        title if len(files) == 1 else f"{title} part {partnum + 1}"
                    ),
                    "formats": formats,
                    "description": description,
                    "thumbnail": thumbnail,
