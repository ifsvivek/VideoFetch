...
class KooIE(InfoExtractor):
    ...
    _TESTS = [
        {  # Test for video in the comments
    ...
    def _real_extract(self, url):
        ...
        data_json = self._download_json(
            f"https://www.kooapp.com/apiV1/ku/{video_id}?limit=20&offset=0&showSimilarKoos=true",
            video_id,
        )["parentContent"]
        item_json = next(
            content["items"][0]
        ...
        if video_m3u8_url:
            formats.extend(
                self._extract_m3u8_formats(
                    video_m3u8_url, video_id, fatal=False, ext="mp4"
                )
            )
        if not formats:
