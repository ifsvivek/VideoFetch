...
class TurnerBaseIE(AdobePassIE):
    ...
    def _extract_cvp_info(
        ...
        for video_file in video_data.findall(".//file"):
            ...
            if ext in ("scc", "srt", "vtt"):
                ...
                formats.extend(
                    self._extract_smil_formats(video_url, video_id, fatal=False)
                )
                ...
                m3u8_formats = self._extract_m3u8_formats(
                    video_url, video_id, "mp4", m3u8_id=format_id or "hls", fatal=False
                )
        ...
        return {
            ...
            "duration": parse_duration(
                xpath_text(video_data, "length") or xpath_text(video_data, "trt")
            ),
    ...
    def _extract_ngtv_info(
        self, media_id, tokenizer_query, software_statement, ap_data=None
    ):
        if not isinstance(ap_data, dict):
            ap_data = {}
        is_live = ap_data.get("is_live")
        streams_data = self._download_json(
            f"https://medium.ngtv.io/media/{media_id}/tv", media_id
        )["media"]["tv"]
        duration = None
        chapters = []
        formats = []
