...
class RadioFranceIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        formats_str = self._html_search_regex(
            r'class="jp-jplayer[^"]*" data-source="([^"]+)">', webpage, "audio URLs"
        )
        formats = [
            {
                "format_id": fm[0],
...
class RadioFranceLiveIE(RadioFranceBaseIE):
    ...
    def _real_extract(self, url):
        ...
        for media_source in traverse_obj(
            api_response, (("now", None), "media", "sources", lambda _, v: v["url"])
        ):
...
class RadioFrancePlaylistBaseIE(RadioFranceBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._generate_playlist_entries(content_id, metadata[self._METADATA_KEY]),
            content_id,
            display_id=display_id,
            **{
                **traverse_obj(
