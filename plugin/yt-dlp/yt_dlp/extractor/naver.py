...
class NaverBaseIE(InfoExtractor):
    ...
    def process_subtitles(vod_data, process_url):
        ...
        for caption in traverse_obj(vod_data, ("captions", "list", ...)):
            ...
            if not caption_url:
                continue
            type_ = (
                "automatic_captions" if caption.get("type") == "auto" else "subtitles"
            )
            lang = (
                caption.get("locale")
    ...
    def _extract_video_info(self, video_id, vid, key):
        ...
        formats = []
        get_list = (
            lambda x: try_get(video_data, lambda y: y[x + "s"]["list"], list) or []
        )
    ...
    def _call_api(self, path, video_id):
        ...
        md = base64.b64encode(
            hmac.HMAC(
                key, f"{api_endpoint[:255]}{msgpad}".encode(), digestmod=hashlib.sha1
            ).digest()
        ).decode()
...
class NaverLiveIE(NaverBaseIE):
    ...
    def _real_extract(self, url):
        video_id = self._match_id(url)
        data = self._call_api(
            f"/live-end/normal/{video_id}/play-info?renewLastPlayDate=true", video_id
        )
        ...
        return {
            ...
            "formats": self._extract_m3u8_formats(
                traverse_obj(data, ("playbackBody", {json.loads}, "media", 0, "path")),
                video_id,
                live=True,
            ),
            **traverse_obj(
...
class NaverNowIE(NaverBaseIE):
    ...
    def _extract_replay(self, show_id, replay_id):
        ...
        in_key = self._download_json(
            f"{self._API_URL}/shows/now.{show_id}/vod/{replay_id}/inkey", replay_id
        )["inKey"]
        return merge_dicts(
            {
