...
class VrtNUIE(VRTBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?vrt\.be/(?:vrtnu|vrtmax)/a-z/(?:[^/]+/){2}(?P<id>[^/?#&]+)"
    )
    _TESTS = [
        {
    ...
    def _fetch_tokens(self):
        ...
        if (
            access_token
            and not self._is_jwt_token_expired(access_token)
            and video_token
            and not self._is_jwt_token_expired(video_token)
        ):
        ...
        if has_credentials:
            self.cache.store(
                self._NETRC_MACHINE, "token_data", (access_token, video_token)
            )
    ...
    def _get_vrt_cookie(self, cookie_name):
        ...
        return try_call(
            lambda: self._get_cookies("https://www.vrt.be/vrtmax/sso")[
                cookie_name
            ].value
        )
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            **traverse_obj(
            ...
            "subtitles": subtitles,
            "_old_archive_ids": [
                make_archive_id("Canvas", video_id),
