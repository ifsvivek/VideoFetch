...
class VVVVIDIE(InfoExtractor):
    ...
    _VALID_URL = (
        rf"{_VALID_URL_BASE}(?P<show_id>\d+)/[^/]+/(?P<season_id>\d+)/(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _download_info(self, show_id, path, video_id, fatal=True, query=None):
        ...
        if response.get("result") == "error":
            raise ExtractorError(
                "{} said: {}".format(self.IE_NAME, response["message"]), expected=True
            )
    ...
    def _real_extract(self, url):
        ...
        response = self._download_info(
            show_id, f"season/{season_id}", video_id, query={"video_id": video_id}
        )
