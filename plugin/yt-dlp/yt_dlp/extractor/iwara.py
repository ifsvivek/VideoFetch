...
class IwaraIE(IwaraBaseIE):
    ...
    def _extract_formats(self, video_id, fileurl):
        ...
        x_version = hashlib.sha1(
            "_".join((paths[-1], q["expires"][0], "5nFp9kmbNnHdAFhaqMvt")).encode()
        ).hexdigest()
...
class IwaraUserIE(IwaraBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            OnDemandPagedList(
                functools.partial(self._entries, playlist_id, user_id), self._PER_PAGE
            ),
            playlist_id,
...
class IwaraPlaylistIE(IwaraBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            OnDemandPagedList(
                functools.partial(self._entries, playlist_id, page_0), self._PER_PAGE
            ),
            playlist_id,
