        ...
        try:
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 403:
                raise ExtractorError(
                    self._parse_json(e.cause.response.read().decode(), media_id)[
                        "messages"
                    ][0],
                    expected=True,
                )
    ...
    def _real_extract(self, url):
        ...
        if media_type == "playlist":
            ...
            for video in try_get(playlist, lambda x: x["videos"]["vods"]) or []:
                ...
                entries.append(
                    self.url_result(
                        f"https://{domain}/video/{video_id}", self.ie_key(), video_id
                    )
                )
            return self.playlist_result(
                entries, media_id, playlist.get("title"), playlist.get("description")
            )
