...
class GameJoltBaseIE(InfoExtractor):
    ...
    def _get_comments(self, post_num_id, post_hash_id):
        ...
        for page in itertools.count(1):
            ...
            for comment in traverse_obj(
                comments_data, (("comments", "childComments"), ...), expected_type=dict
            ):
                yield {
                    "id": comment["id"],
                    ...
                    "author_id": traverse_obj(
                        comment, ("user", "username"), expected_type=str_or_none
                    ),
                    "author_thumbnail": traverse_obj(
                        comment, ("user", "image_avatar"), expected_type=str_or_none
                    ),
    ...
    def _parse_post(self, post_data):
        ...
        info_dict = {
            ...
            "categories": [
                try_get(
                    ...
                    lambda x: "{} - {}".format(
                        x["community"]["name"],
                        x["channel"].get("display_title") or x["channel"]["title"],
                    ),
                )
        ...
        for media in post_data.get("media", []):
            if determine_ext(media["img_url"]) != "gif" or "gif" not in media.get(
                "filetype", ""
            ):
                continue
            gif_entries.append(
                    ...
                    "formats": [
                        {
                            ...
                            "url": media[url_key],
                            "width": (
                                media.get("width") if url_key == "img_url" else None
                            ),
                            "height": (
                                media.get("height") if url_key == "img_url" else None
                            ),
                            "filesize": (
                                media.get("filesize") if url_key == "img_url" else None
                            ),
                            "acodec": "none",
                        }
                        for url_key in (
        ...
        embed_url = traverse_obj(
            post_data, ("embeds", ..., "url"), expected_type=str_or_none, get_all=False
        )
        if embed_url:
...
class GameJoltPostListBaseIE(GameJoltBaseIE):
    def _entries(
        ...
        items = (
            initial_items
            or self._call_api(endpoint, list_id, note=note, errnote=errnote)["items"]
        )
        while items:
            for item in items:
...
class GameJoltUserIE(GameJoltPostListBaseIE):
    ...
    def _real_extract(self, url):
        ...
        bio = self._parse_content_as_text(
            self._parse_json(user_data.get("bio_content", "{}"), user_id, fatal=False)
            or {}
        )
...
class GameJoltGameIE(GameJoltPostListBaseIE):
    ...
    def _real_extract(self, url):
        ...
        description = self._parse_content_as_text(
            self._parse_json(
                game_data.get("description_content", "{}"), game_id, fatal=False
            )
            or {}
        )
        return self.playlist_result(
