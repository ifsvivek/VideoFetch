...
class OnsenIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        sign_in = self._download_json(
            f"{self._BASE_URL}/web_api/signin",
            None,
            "Logging in",
            headers={
    ...
    def _real_extract(self, url):
        ...
        if "c" not in query:
            entries = [
                self.url_result(
                    update_url_query(url, {"c": self._get_encoded_id(program)}), OnsenIE
                )
                for program in traverse_obj(
                    programs, ("contents", lambda _, v: v["id"])
                )
            ]
            ...
            return self.playlist_result(
                entries,
                program_id,
                traverse_obj(programs, ("program_info", "title", {clean_html})),
            )
        ...
        return {
            ...
            "formats": self._extract_m3u8_formats(
                m3u8_url, raw_id, headers=self._HEADERS
            ),
