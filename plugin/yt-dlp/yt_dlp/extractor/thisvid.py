...
class ThisVidIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?thisvid\.com/(?P<type>videos|embed)/(?P<id>[A-Za-z0-9-]+)"
    )
    _TESTS = [
        {
        ...
        title = (
            re.split(
                ...
                or self._html_search_regex(
                    r"(?s)<title\b[^>]*>(.+?)</title", webpage, "title", fatal=False
                )
                or "",
                maxsplit=1,
            )[0]
...
class ThisVidPlaylistIE(ThisVidPlaylistBaseIE):
    _VALID_URL = (
        r"https?://thisvid\.com/playlist/(?P<id>\d+)/video/(?P<video_id>[A-Za-z0-9-]+)"
    )
    _TESTS = [
        {
