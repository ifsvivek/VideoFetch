...
class NDRIE(NDRBaseIE):
    ...
    def _extract_embed(self, webpage, display_id, url):
        embed_url = (
            self._html_search_meta("embedURL", webpage, "embed URL", default=None)
        ...
        if re.match(r"^[a-z]+\d+$", embed_url):
            ...
            path = self._search_regex(
                rf"(.+/){display_id}", parsed_url.path or "", "embed URL", default=""
            )
            # find tell-tale image with the actual ID
            ndr_id = self._search_regex(
                rf"{path}([a-z]+\d+)(?!\.)\b", webpage, "embed URL", default=None
            )
