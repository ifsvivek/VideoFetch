                    ...
                    **traverse_obj(
                        file_element,
                        {
                            ...
                            "timestamp": (
                                ...
                                {
                                    parse_iso8601(
                                        delimiter=" ", timezone=dt.timedelta(hours=9)
                                    )
                                },
                            ),
                        },
...
class AfreecaTVLiveIE(AfreecaTVBaseIE):
    ...
    def _extract_formats(self, channel_info, broadcast_no, aid):
        stream_base_url = (
            channel_info.get("RMD") or "https://livestream-manager.sooplive.co.kr"
        )
    ...
    def _real_extract(self, url):
        ...
        if not aid:
            ...
            if result == 0:
                ...
                self.raise_login_required(
                    "This livestream is for subscribers only", method="password"
                )
...
class AfreecaTVUserIE(AfreecaTVBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = OnDemandPagedList(
            functools.partial(self._fetch_page, user_id, user_type), self._PER_PAGE
        )
