    ...
    def _real_extract(self, url):
        ...
        if embed_url and (
            ...
            meta_description = clean_html(
                self._html_search_meta(
                    "description",
                    self._webpage_read_content(urlh, embed_url, video_id, fatal=False),
                    default=None,
                )
            )
        ...
        if not entries and can_view_post is False:
            self.raise_no_formats(
                "You do not have access to this post", video_id=video_id, expected=True
            )
        elif not entries:
            self.raise_no_formats(
    ...
    def _real_extract(self, url):
        ...
        if campaign_id is None:
            ...
            campaign_id = traverse_obj(
                self._search_nextjs_data(webpage, vanity, default=None),
                (
                    "props",
                    "pageProps",
                    "bootstrapEnvelope",
            ...
            if not campaign_id:
                campaign_id = traverse_obj(
                    self._search_nextjs_v13_data(webpage, vanity),
                    (
                        lambda _, v: v["type"] == "campaign",
                        "id",
                        {str},
                        any,
                        {require("campaign ID")},
        ...
        return {
            ...
            "age_limit": (
                18 if traverse_obj(campaign_info, ("attributes", "is_nsfw")) else 0
            ),
