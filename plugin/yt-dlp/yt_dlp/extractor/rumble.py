...
class RumbleEmbedIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if video.get("live") == 0:
            live_status = (
                "not_live" if video.get("livestream_has_dvr") is None else "was_live"
            )
        elif video.get("live") == 1:
            live_status = (
                "is_upcoming" if video.get("livestream_has_dvr") else "was_live"
            )
                ...
                formats.append(
                    {
                        ...
                        "format_id": join_nonempty(
                            format_type, format_field(meta, "h", "%sp")
                        ),
...
class RumbleIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        url_info = next(
            RumbleEmbedIE.extract_from_webpage(self._downloader, url, webpage), None
        )
        if not url_info:
        ...
        for page in itertools.count(1):
            try:
                webpage = self._download_webpage(
                    f"{url}?page={page}", playlist_id, note=f"Downloading page {page}"
                )
