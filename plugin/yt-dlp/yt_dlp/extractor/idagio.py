...
class IdagioTrackIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        content_info = self._download_json(
            f"https://api.idagio.com/v1.8/content/track/{track_id}",
            track_id,
            query={
                "quality": "0",
                "format": "2",
...
class IdagioPlaylistBaseIE(InfoExtractor):
    ...
    def _entries(self, playlist_info):
        for track_data in traverse_obj(
            playlist_info, ("tracks", lambda _, v: v["id"] and v["recording"]["id"])
        ):
    ...
    def _real_extract(self, url):
        ...
        playlist_info = self._download_json(
            self._API_URL_TMPL.format(playlist_id), playlist_id
        )["result"]
...
class IdagioAlbumIE(IdagioPlaylistBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?app\.idagio\.com(?:/[a-z]{2})?/albums/(?P<id>[\w-]+)"
    )
    _TESTS = [
        {
