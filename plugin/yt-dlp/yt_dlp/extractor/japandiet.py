...
class ShugiinItvVodIE(ShugiinItvBaseIE):
    ...
    def _real_extract(self, url):
        ...
        for chp in re.finditer(
            r'(?i)<A\s+HREF="([^"]+?)"\s*class="play_vod">(?!<img)(.+)</[Aa]>', webpage
        ):
            chapters.append(
                {
                    "title": clean_html(chp.group(2)).strip(),
                    "start_time": try_call(
                        lambda: float(parse_qs(chp.group(1))["time"][0].strip())
                    ),
                }
            )
...
class SangiinIE(InfoExtractor):
    _VALID_URL = (
        r"https?://www\.webtv\.sangiin\.go\.jp/webtv/detail\.php\?sid=(?P<id>\d+)"
    )
    ...
    def _real_extract(self, url):
        ...
        m3u8_url = self._search_regex(
            r'var\s+videopath\s*=\s*(["\'])([^"\']+)\1', webpage, "m3u8 url", group=2
        )
