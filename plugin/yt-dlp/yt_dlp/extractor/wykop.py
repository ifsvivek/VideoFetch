...
class WykopBaseIE(InfoExtractor):
    ...
    def _do_call_api(
        self, path, video_id, note="Downloading JSON metadata", data=None, headers={}
    ):
        if data:
        ...
        for retrying in range(2):
            try:
                return self._do_call_api(
                    path, video_id, note, headers={"Authorization": f"Bearer {token}"}
                )
...
class WykopDigCommentIE(WykopBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?wykop\.pl/link/(?P<dig_id>\d+)/[^/]+/komentarz/(?P<id>\d+)"
    )
    ...
    def _real_extract(self, url):
        ...
        data = self._call_api(f"links/{dig_id}/comments/{comment_id}", comment_id)[
            "data"
        ]
...
class WykopPostCommentIE(WykopBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?wykop\.pl/wpis/(?P<post_id>\d+)/[^/#]+#(?P<id>\d+)"
    )
    ...
    def _real_extract(self, url):
        post_id, comment_id = self._search_regex(
            self._VALID_URL, url, "post and comment ids", group=("post_id", "id")
        )
        data = self._call_api(f"entries/{post_id}/comments/{comment_id}", comment_id)[
            "data"
        ]
