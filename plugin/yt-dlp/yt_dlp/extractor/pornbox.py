...
class PornboxIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?pornbox\.com/application/watch-page/(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        metadata = {
            ...
            **traverse_obj(
            ...
            "timestamp": parse_iso8601(
                traverse_obj(public_data, ("studios", "release_date"), "publish_date")
            ),
        ...
        media_id = traverse_obj(
            public_data,
            ("medias", lambda _, v: v["title"] == "Full video", "media_id", {int}),
            get_all=False,
        )
        if not media_id:
