    ...
    def _extract_from_smotrim_api(self, typ, item_id):
        ...
        return {
            "age_limit": traverse_obj(
                data, ("data", "age_restrictions", {int_or_none})
            ),
            "is_live": typ in ("audio-live", "live"),
            "tags": traverse_obj(
                webpage,
                (
                    {find_elements(cls="tags-list__link")},
                    ...,
                    {clean_html},
                    filter,
                    all,
...
class SmotrimIE(SmotrimBaseIE):
    ...
    _EMBED_REGEX = [rf'<iframe\b[^>]+\bsrc=["\'](?P<url>{_VALID_URL})']
    _TESTS = [
        {
        ...
        if typ == "channel":
            ...
            src_url = traverse_obj(
                ...
                (
                    (
                        (
                            {find_element(cls="main-player__frame", html=True)},
                            {extract_attributes},
                            "src",
                        ),
                        (
...
class SmotrimPlaylistIE(SmotrimBaseIE):
    ...
    _PAGE_SIZE = 15
    _VALID_URL = (
        r"https?://smotrim\.ru/(?P<type>brand|podcast)/(?P<id>\d+)/?(?P<season>[\w-]+)?"
    )
    _TESTS = [
        {
            # Video
    ...
    def _real_extract(self, url):
        playlist_type, playlist_id, season = self._match_valid_url(url).group(
            "type", "id", "season"
        )
        ...
        if season:
            return self.playlist_from_matches(
                ...
                season=traverse_obj(
                    webpage,
                    (
                        {find_element(cls="seasons__item seasons__item--selected")},
                        {clean_html},
                    ),
                ),
                ie=SmotrimIE,
        ...
        if traverse_obj(
            webpage,
            (
                {find_element(cls="brand-main-item__videos")},
                {clean_html},
                filter,
            ),
        ):
        ...
        return self.playlist_result(
            OnDemandPagedList(
                functools.partial(self._fetch_page, endpoint, key, playlist_id),
                self._PAGE_SIZE,
            ),
            playlist_id,
            playlist_title,
