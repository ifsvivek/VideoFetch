        ...
        for q in TP_SMIL_QUERY:
            ...
            try:
                tp_formats, tp_subtitles = self._extract_theplatform_smil(
                    m_url,
                    video_id,
                    "Downloading %s SMIL data" % (q.get("switch") or q["assetTypes"]),
                )
...
class AENetworksIE(AENetworksBaseIE):
    ...
    def _real_extract(self, url):
        domain, canonical, url_type, extra = self._match_valid_url(url).group(
            "domain", "id", "type", "extra"
        )
...
class AENetworksShowIE(AENetworksListBaseIE):
    ...
    _VALID_URL = (
        AENetworksBaseIE._BASE_URL_REGEX + r"shows/(?P<id>[^/?#&]+)/?(?:[?#&]|$)"
    )
    _TESTS = [
        {
...
class HistoryTopicIE(AENetworksBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            "http://www.history.com/videos/" + display_id, AENetworksIE.ie_key()
        )
...
class HistoryPlayerIE(AENetworksBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?(?P<domain>(?:history|biography)\.com)/player/(?P<id>\d+)"
    )
    _TESTS = []
