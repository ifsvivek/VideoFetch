...
class TuneInBaseIE(InfoExtractor):
    ...
    def _extract_formats_and_subtitles(self, content_id):
        ...
        for stream in traverse_obj(
            streams, ("body", lambda _, v: url_or_none(v["url"]))
        ):
...
class TuneInPodcastEpisodeIE(TuneInBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "formats": formats,
            "series": traverse_obj(
                self._call_api(series_id), ("Item", "Title", {clean_html})
            ),
            "series_id": series_id,
            "subtitles": subtitles,
            **traverse_obj(
                self._call_api(episode_id),
                (
                    "Item",
                    {
                        "title": ("Title", {clean_html}),
...
class TuneInEmbedIE(TuneInBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(f"https://tunein.com/{kind}/?{kind}id={embed_id[1:]}")
