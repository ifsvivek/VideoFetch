...
class DangalPlayBaseIE(InfoExtractor):
    ...
    def _call_api(
        self, path, display_id, note="Downloading JSON metadata", fatal=True, query={}
    ):
...
class DangalPlayIE(DangalPlayBaseIE):
    ...
    def _generate_api_data(self, data):
        ...
        unhashed = "".join(
            (catalog_id, content_id, self._OTV_USER_ID, timestamp, self._SECRET_KEY)
        )
    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 422:
                ...
                if error_code == "1016":
                    ...
                    self.raise_login_required(
                        f"Your login region is unspecified or incorrect. {self._LOGIN_HINT}",
                        method=None,
                    )
                raise ExtractorError(
                    join_nonempty(error_code, error_info.get("message"), delim=": ")
                )
            raise
        ...
        m3u8_url = traverse_obj(
            details,
            (
                ("adaptive_url", ("adaptive_urls", "hd", "hls", ..., "playback_url")),
                {url_or_none},
                any,
            ),
        )
        ...
        for subcategory in subcategories:
            ...
            for ep in traverse_obj(
                ...
                yield self.url_result(
                    ...
                    DangalPlayIE,
                    **self._extract_episode_info(ep, episode_slug, series_slug),
                )
