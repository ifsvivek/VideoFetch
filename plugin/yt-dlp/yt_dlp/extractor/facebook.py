...
class FacebookIE(InfoExtractor):
    ...
    _SUPPORTED_PAGLETS_REGEX = (
        r"(?:pagelet_group_mall|permalink_video_pagelet|hyperfeed_story_id_[0-9a-f]+)"
    )
    _api_config = {
    ...
    def _perform_login(self, username, password):
        ...
        lsd = self._search_regex(
            r'<input type="hidden" name="lsd" value="([^"]*)"', login_page, "lsd"
        )
    ...
    def _extract_from_url(self, url, video_id):
        ...
        def extract_metadata(webpage):
            ...
            uploader_data = (
                ...
                or get_first(
                    post, (..., "video", lambda k, v: k == "owner" and v["name"])
                )
        ...
        def extract_from_jsmods_instances(js_data):
            if js_data:
                return extract_video_data(
                    try_get(js_data, lambda x: x["jsmods"]["instances"], list) or []
                )
            ...
            return (
                traverse_obj(
                    ...
                    (
                        ...
                        (None, (..., ..., ..., "__bbox", "require")),
                        lambda _, v: any(
                            key.startswith("RelayPrefetchedStreamCache") for key in v
                        ),
                        ...,
                        ...,
                        "__bbox",
        ...
        if not video_data:
            ...
            if data:
                ...
                def parse_graphql_video(video):
                    ...
                    fmt_data = (
                        traverse_obj(video, ("videoDeliveryLegacyFields", {dict}))
                        or video
                    )
                    ...
                    mpd_urls = traverse_obj(
                        fmt_data,
                        ("dash_manifest_urls", ..., "manifest_url", {url_or_none}),
                    )
                    ...
                    for idx, dash_manifest in enumerate(dash_manifests):
                        extract_dash_manifest(
                            dash_manifest, formats, mpd_url=traverse_obj(mpd_urls, idx)
                        )
                    ...
                    if captions_url and not automatic_captions and not subtitles:
                        ...
                        (automatic_captions if is_broadcast else subtitles)[locale] = [
                            {"url": captions_url}
                        ]
                    ...
                    info = {
                        ...
                        "duration": (
                            float_or_none(video.get("playable_duration_in_ms"), 1000)
                ...
                edges = (
                    try_get(data, lambda x: x["mediaset"]["currMedia"]["edges"], list)
                    or []
                )
                for edge in edges:
        ...
        if not video_data:
            m_msg = re.search(
                r'class="[^"]*uiInterstitialContent[^"]*"><div>(.*?)</div>', webpage
            )
        ...
        if not video_data and "/watchparty/" in url:
            ...
            entries = []
            for edge in (
                try_get(living_room, lambda x: x["recap"]["watched_content"]["edges"])
                or []
            ):
        ...
        if len(video_data) > 1:
            ...
            for v in video_data:
                ...
                entries.append(
                    self.url_result(
                        urljoin(url, video_url), self.ie_key(), v[0].get("video_id")
                    )
                )
            return self.playlist_result(entries, video_id)
    ...
    def _real_extract(self, url):
        ...
        real_url = (
            self._VIDEO_PAGE_TEMPLATE % video_id if url.startswith("facebook:") else url
        )
...
class FacebookAdsIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:[\w-]+\.)?facebook\.com/ads/library/?\?(?:[^#]+&)?id=(?P<id>\d+)"
    )
    ...
    def _real_extract(self, url):
        ...
        post_data = traverse_obj(
            re.findall(r"data-sjs>({.*?ScheduledServerJS.*?})</script>", webpage),
            (..., {json.loads}),
        )
        data = get_first(
            post_data,
        ...
        info_dict = merge_dicts(
            ...
            traverse_obj(
                data,
                {
                    "description": (
                        "link_description",
                        {lambda x: x if not x.startswith("{{product.") else None},
                    ),
        ...
        for idx, entry in enumerate(
            traverse_obj(
                ...
                (
                    ("videos", "cards"),
                    lambda _, v: any(url_or_none(v.get(f)) for f in self._FORMATS_MAP),
                ),
            ),
            1,
        ):
            entries.append(
                {
                    ...
                    "description": traverse_obj(entry, "body", "link_description")
