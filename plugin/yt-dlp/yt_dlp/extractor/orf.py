...
class ORFRadioIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        station, station2, show_date, show_id = self._match_valid_url(url).group(
            "station", "station2", "date", "show"
        )
...
class ORFIPTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_id = self._search_regex(r'data-video(?:id)?="(\d+)"', webpage, "video id")
        ...
        for format_id, format_url in rendition["redirect"].items():
            if format_id == "rtmp":
                ...
                formats.extend(
                    self._extract_f4m_formats(format_url, video_id, f4m_id=format_id)
                )
