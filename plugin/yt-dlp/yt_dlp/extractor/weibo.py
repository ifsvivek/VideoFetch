...
class WeiboBaseIE(InfoExtractor):
    ...
    def _weibo_download_json(
        ...
        webpage, urlh = self._download_webpage_handle(
            url, video_id, note=note, data=data, headers=headers, query=query
        )
                    ...
                    formats.append(
                        {
                            ...
                            **traverse_obj(
                                ...
                                (
                                    "video_details",
                                    lambda _, v: v["label"].startswith(format_id),
                                    {
...
class WeiboVideoIE(WeiboBaseIE):
    ...
    _VALID_URL = [
        rf"https?://(?:www\.)?weibo\.com/tv/show/(?P<id>{_VIDEO_ID_RE})",
        rf"https?://video\.weibo\.com/show/?\?(?:[^#]+&)?fid=(?P<id>{_VIDEO_ID_RE})",
    ]
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        post_data = (
            f'data={{"Component_Play_Playinfo":{{"oid":"{video_id}"}}}}'.encode()
        )
...
class WeiboUserIE(WeiboBaseIE):
    ...
    def _real_extract(self, url):
        ...
        video_id = (
            params.get("layerid") if params.get("tabtype") == "newVideo" else None
        )
        if not self._yes_playlist(uid, video_id):
            return self.url_result(
                f"https://weibo.com/{uid}/{video_id}", WeiboIE, video_id
            )
