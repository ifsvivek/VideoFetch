...
class IvooxIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        direct_download = self._download_json(
            f"https://vcore-web.ivoox.com/v1/public/audios/{media_id}/download-url",
            media_id,
            fatal=False,
            note="Fetching direct download link",
            headers={"Referer": url},
        )
        ...
        return {
            ...
            "uploader": self._html_search_regex(
                r'data-prm-author="([^"]+)"', webpage, "author", default=None
            ),
            "timestamp": parse_iso8601(
                self._html_search_regex(
                    r'data-prm-pubdate="([^"]+)"', webpage, "timestamp", default=None
                )
            ),
            "channel": self._html_search_regex(
                r'data-prm-podname="([^"]+)"', webpage, "channel", default=None
            ),
            "title": self._html_search_regex(
                r'data-prm-title="([^"]+)"', webpage, "title", default=None
            ),
