...
class PornHubBaseIE(InfoExtractor):
    ...
    def _login(self, host):
        ...
        def is_logged(webpage):
            return any(
                re.search(p, webpage)
                for p in (r'id="profileMenuDropdown"', r'class="ph-icon-logout"')
            )
    ...
    def _real_extract(self, url):
        ...
        def extract_list(meta_key):
            ...
            if div:
                return [
                    clean_html(x).strip()
                    for x in re.findall(r"(?s)<a[^>]+\bhref=[^>]+>.+?</a>", div)
                ]
...
class PornHubPlaylistBaseIE(PornHubBaseIE):
    ...
    def _extract_entries(self, webpage, host):
        ...
        return [
            self.url_result(
                f"http://www.{host}/{video_url}", PornHubIE.ie_key(), video_title=title
            )
...
class PornHubPlaylistIE(PornHubPlaylistBaseIE):
    ...
    def _entries(self, url, host, item_id):
        ...
        video_count = int_or_none(
            self._search_regex(
                r"var\s+itemsCount\s*=\s*([0-9]+)\s*\|\|", webpage, "video_count"
            )
        )
