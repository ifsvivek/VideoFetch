    ...
    def _perform_login(self, username, password):
        ...
        user_check = traverse_obj(
            self._download_json(
                ...
                expected_status=400,
                headers=headers,
                data=json.dumps({"username": username}, separators=(",", ":")).encode(),
            ),
            "status",
        )
            ...
            **traverse_obj(
                video_info,
                {
                    ...
                    "creator": (
                        (
                            ("user"),
                            ("users", lambda _, v: str(v["user_id"]) == user_id),
                        ),
                        "name",
                    ),
...
class TrillerUserIE(TrillerBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if user_info.get("private") and user_info.get("followed_by_me") not in (
            True,
            "true",
        ):
            raise ExtractorError("This user profile is private", expected=True)
        elif traverse_obj(
            user_info, (("blocked_by_user", "blocking_user"), {bool}), get_all=False
        ):
