...
class GoogleDriveIE(InfoExtractor):
    ...
    def _get_subtitles(self, video_id, video_info):
        ...
        subtitle_formats = traverse_obj(
            subtitle_data,
            (lambda _, v: v.tag == "format", {lambda x: x.get("fmt_code")}, {str}),
        )
        for track in traverse_obj(
            subtitle_data, (lambda _, v: v.tag == "track" and v.get("lang_code"))
        ):
    ...
    def _real_extract(self, url):
        ...
        video_info = self._download_json(
            ...
            query={"key": "AIzaSyDVQw45DwoYh632gvsP5vPDqEKvb-Ywnb8"},
                    ...
                    **traverse_obj(
                        ...
                        (
                            "transcodeMetadata",
                            {
                                ...
                                "vcodec": (
                                    (("videoCodecString", {str}), {value("none")}),
                                    any,
                                ),
                                "acodec": (
                                    (("audioCodecString", {str}), {value("none")}),
                                    any,
                                ),
                            },
                        ),
            ...
            if urlh.headers.get("Content-Disposition"):
                ...
                if confirmation_webpage:
                    confirmed_source_url = extract_attributes(
                        get_element_html_by_id("download-form", confirmation_webpage)
                        or ""
                    ).get("action")
...
"""
    ...
    def _call_api(self, folder_id, key, data, **kwargs):
        ...
        return (
            self._search_json("", response, "api response", folder_id, **kwargs) or {}
        )
