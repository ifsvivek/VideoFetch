...
class CaracolTvPlayIE(InfoExtractor):
    ...
    def _extract_app_token(self, webpage):
        config_js_path = self._search_regex(
            r'<script[^>]+src\s*=\s*"([^"]+coreConfig.js[^"]+)',
            webpage,
            "config js url",
            fatal=False,
        )
        ...
        mediation_config = (
            ...
            else self._search_json(
                r"mediation\s*:",
                self._download_webpage(
                    urljoin("https://play.caracoltv.com/", config_js_path),
                    None,
                    fatal=False,
                    note="Extracting JS config",
                ),
                "mediation_config",
                None,
        ...
        key = (
            traverse_obj(mediation_config, ("live", "key"))
            or "795cd9c089a1fc48094524a5eba85a3fca1331817c802f601735907c8bbb4f50"
        )
        secret = (
            traverse_obj(mediation_config, ("live", "secret"))
            or "64dec00a6989ba83d087621465b5e5d38bdac22033b0613b659c442c78976fa0"
        )
    ...
    def _perform_login(self, email, password):
        webpage = self._download_webpage(
            "https://play.caracoltv.com/", None, fatal=False
        )
        app_token = self._extract_app_token(webpage)

        bearer_token = self._download_json(
            "https://eu-gateway.inmobly.com/applications/oauth",
            None,
            data=b"",
            note="Retrieving bearer token",
            headers={"Authorization": f"Basic {app_token}"},
        )["token"]

        self._USER_TOKEN = self._download_json(
            "https://eu-gateway.inmobly.com/user/login",
            None,
    ...
    def _extract_video(
        self, video_data, series_id=None, season_id=None, season_number=None
    ):
        formats, subtitles = self._extract_m3u8_formats_and_subtitles(
            video_data["stream_url"], series_id, "mp4"
        )
    ...
    def _extract_series_seasons(self, seasons, series_id):
        for season in seasons:
            api_response = self._download_json(
                "https://eu-gateway.inmobly.com/feed",
                series_id,
            ...
            for episode in api_response["items"]:
                yield self._extract_video(
                    episode, series_id, season["id"], season_number
                )
