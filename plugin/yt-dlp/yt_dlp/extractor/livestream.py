...
class LivestreamOriginalIE(InfoExtractor):
    ...
    def _extract_video_info(self, user, video_id):
        ...
        duration = float_or_none(
            xpath_attr(item, xpath_with_ns("media:content", media_ns), "duration")
        )
    ...
    def _real_extract(self, url):
        ...
        if url_type == "folder":
            ...
            stream_url = (
                f"http://x{user}x.api.channel.livestream.com/3.0/getstream.json"
            )
            info = {}
            if content_id:
                ...
                info = {
                    ...
                    "thumbnail": self._search_regex(
                        r'channelLogo\.src\s*=\s*"([^"]+)"', webpage, "thumbnail", None
                    ),
                }
