    ...
    def _call_api(self, url, video_id, note=None):
        ...
        if json.get("status", {}).get("code", 0) != 0:
            raise ExtractorError(
                f'{self.IE_NAME} said: {json["status"]["message_client"]}', cause=json
            )
    ...
    def _real_extract(self, url):
        ...
        if is_upcoming:
            self.raise_no_formats(
                (
                    "This event will start at {}.".format(video_json["live_label"])
...
class RCTIPlusSeriesIE(RCTIPlusBaseIE):
    ...
    def _entries(
        self, url, display_id=None, note="Downloading entries JSON", metadata={}
    ):
        total_pages = 0
        try:
            total_pages = self._call_api(f"{url}&length=20&page=0", display_id, note)[
                1
...
class RCTIPlusTVIE(RCTIPlusBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            f"https://www.rctiplus.com/{video_type}/{video_id}/{tv_id}", "RCTIPlus"
        )
