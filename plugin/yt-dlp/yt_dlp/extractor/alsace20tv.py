...
class Alsace20TVBaseIE(InfoExtractor):
    def _extract_video(self, video_id, url=None):
        ...
        thumbnail = url_or_none(
            dict_get(info, ("image", "preview")) or self._og_search_thumbnail(webpage)
        )
        upload_date = self._search_regex(
            r"/(\d{6})_", thumbnail, "upload_date", default=None
        )
        upload_date = (
            unified_strdate(f"20{upload_date[:2]}-{upload_date[2:4]}-{upload_date[4:]}")
            if upload_date
            else None
        )
        return {
            ...
            "duration": int_or_none(
                self._og_search_property("video:duration", webpage) if webpage else None
            ),
