...
class NetEaseMusicIE(NetEaseMusicBaseIE):
    ...
    IE_DESC = "网易云音乐"
    _VALID_URL = (
        r"https?://(?:y\.)?music\.163\.com/(?:[#m]/)?song\?.*?\bid=(?P<id>[0-9]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        lyric_data = (
            {
                "description": traverse_obj(
                    lyrics, (("lyrics_merged", "lyrics"), 0, "data"), get_all=False
                ),
                "subtitles": lyrics,
            }
            if lyrics
        ...
        return {
            "id": song_id,
            "formats": formats,
            "alt_title": "/".join(traverse_obj(info, (("transNames", "alias"), ...)))
            or None,
            "creators": traverse_obj(info, ("artists", ..., "name")) or None,
            "album_artists": traverse_obj(info, ("album", "artists", ..., "name"))
            or None,
            **lyric_data,
            **traverse_obj(
                info,
...
class NetEaseMusicSingerIE(NetEaseMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        name = join_nonempty(
            ...
            join_nonempty(
                *traverse_obj(info, ("artist", ("trans", ("alias", ...)), {str})),
                delim=";",
            ),
            delim=" - ",
        )
...
class NetEaseMusicListIE(NetEaseMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._get_entries(info, ("playlist", "tracks")), list_id, **metainfo
        )
