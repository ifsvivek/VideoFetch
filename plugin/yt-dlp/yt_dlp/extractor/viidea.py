    ...
    def _real_extract(self, url):
        ...
        try:
            lecture_data = self._download_json(
                f"{base_url}/site/api/lecture/{lecture_id}?format=json", lecture_id
            )["lecture"][0]
        ...
        if parts:
            ...
            def extract_part(part_id):
                ...
                info["id"] = (
                    lecture_id if not multipart else f"{lecture_id}_part{part_id}"
                )
                info["display_id"] = (
                    lecture_slug if not multipart else f"{lecture_slug}_part{part_id}"
                )
                if multipart:
