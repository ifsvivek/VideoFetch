...
class YleAreenaIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for sub in traverse_obj(
            video_data,
            ("ongoing_ondemand", "subtitles", lambda _, v: url_or_none(v["uri"])),
        ):
        ...
        # XXX: Has all externally-hosted Kaltura content been moved to native hosting?
            ...
            self.raise_no_formats(
                "The content is no longer available", expected=True, video_id=video_id
            )
        ...
        return {
            **traverse_obj(
            ...
            "season_number": (
                int_or_none(
                    self._search_regex(
                        r"Kausi (\d+)", description, "season number", default=None
                    )
                )
