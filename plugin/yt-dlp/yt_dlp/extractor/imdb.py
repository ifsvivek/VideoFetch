...
class ImdbIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_info = traverse_obj(
            info, ("props", "pageProps", "videoPlaybackData", "video"), default={}
        )
        title = (
            traverse_obj(
                video_info, ("name", "value"), ("primaryTitle", "titleText", "text")
            )
        ...
        for encoding in data:
            ...
            if not video_url:
                continue
            ext = mimetype2ext(encoding.get("mimeType")) or determine_ext(video_url)
            if ext == "m3u8":
...
class ImdbListIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?imdb\.com/list/ls(?P<id>\d{9})(?!/videoplayer/vi\d+)"
    )
    _TEST = {
