...
class YandexMusicTrackIE(YandexMusicBaseIE):
    ...
    def _real_extract(self, url):
        ...
        key = hashlib.md5(
            ("XGRlBW9FXlekgbPrRHuSiA" + fd_data["path"][1:] + fd_data["s"]).encode()
        ).hexdigest()
        ...
        def extract_artist(artist_list):
            if artist_list and isinstance(artist_list, list):
                artists_names = [
                    extract_artist_name(a) for a in artist_list if a.get("name")
                ]
                if artists_names:
...
class YandexMusicAlbumIE(YandexMusicPlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        album = self._call_api(
            "album", tld, url, album_id, "Downloading album JSON", {"album": album_id}
        )
