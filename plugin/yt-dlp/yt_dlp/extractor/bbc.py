...
class BBCCoUkIE(InfoExtractor):
    ...
    def _raise_extractor_error(self, media_selection_error):
        raise ExtractorError(
            f"{self.IE_NAME} returned error: {media_selection_error.id}", expected=True
        )
                ...
                for connection in self._extract_connections(media):
                    ...
                    if supplier == "asx":
                        for i, ref in enumerate(
                            self._extract_asx_playlist(connection, programme_id)
                        ):
                            formats.append(
                                {
                                    "url": ref,
...
class BBCIE(BBCCoUkIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        ...
        if programme_id:
            ...
            description = page_info.get("description") or self._og_search_description(
                webpage
            )
        ...
        if initial_data:
            init_data = (
                try_get(initial_data, lambda x: x["initData"]["items"][0], dict) or {}
            )
            ...
            if version_id:
                ...
                return {
                    ...
                    "thumbnail": (
                        image_url.replace("$recipe", "raw") if image_url else None
                    ),
        ...
        if morph_payload:
            for lead_media in traverse_obj(
                morph_payload, ("body", "components", ..., "props", "leadMedia", {dict})
            ):
                ...
                return {
                    ...
                    **traverse_obj(
                        lead_media,
                        {
                            ...
                            "duration": (
                                "duration",
                                ("rawDuration", "formattedDuration", "spokenDuration"),
                                {parse_duration},
                            ),
            ...
            body = self._parse_json(
                traverse_obj(morph_payload, ("body", "content", "article", "body")),
                playlist_id,
                fatal=False,
            )
            ...
            if entries:
                playlist_title = (
                    traverse_obj(
                        morph_payload, ("body", "content", "article", "headline", {str})
                    )
                    or playlist_title
                )
                return self.playlist_result(
        ...
        if programme_id and current_programme.get("type") == "playable_item":
            title = (
                traverse_obj(
                    current_programme, ("titles", ("tertiary", "secondary"), {str}, any)
                )
                or playlist_title
            )
            formats, subtitles = self._download_media_selector(programme_id)
                    ...
                    **traverse_obj(
                        ...
                        (
                            ...
                            asset_id,
                            {
                                "title": ("title", {str}),
                                "description": (
                                    ("synopsisLong", "synopsisMedium", "synopsisShort"),
                                    {str},
                                    any,
                                ),
            ...
            return {
                ...
                **traverse_obj(
                    model,
                    {
                        ...
                        "thumbnail": (
                            "imageUrl",
                            {lambda u: urljoin(url, u.replace("$recipe", "raw"))},
                        ),
                        "description": (
                            "synopses",
                            ("long", "medium", "short"),
        ...
        if initial_data:
            ...
            def parse_media(media):
                ...
                for item in try_get(media, lambda x: x["media"]["items"], list) or []:
                    ...
                    item_time = None
                    for meta in (
                        try_get(media, lambda x: x["metadata"]["items"], list) or []
                    ):
            ...
            for resp in traverse_obj(initial_data, ("data", lambda _, v: v["name"])):
                ...
                if name == "media-experience":
                    parse_media(
                        try_get(
                            resp, lambda x: x["data"]["initialItem"]["mediaItem"], dict
                        )
                    )
                elif name == "article":
                    ...
                    if formats:
                        entry = {
                            ...
                            "description": traverse_obj(
                                simorgh_data, ("pageData", "promo", "summary", {str})
                            ),
                            **traverse_obj(
                                model,
                                {
        ...
        EMBED_URL = (
            rf'https?://(?:www\.)?bbc\.co\.uk/(?:[^/]+/)+{self._ID_REGEX}(?:\b[^"]+)?'
        )
        entries = []
...
class BBCCoUkArticleIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?bbc\.co\.uk/programmes/articles/(?P<id>[a-zA-Z0-9]+)"
    )
...
class BBCCoUkPlaylistBaseIE(InfoExtractor):
    def _entries(self, webpage, url, playlist_id):
        single_page = "page" in urllib.parse.parse_qs(urllib.parse.urlparse(url).query)
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(webpage, url, playlist_id), playlist_id, title, description
        )


class BBCCoUkIPlayerPlaylistBaseIE(InfoExtractor):
    _VALID_URL_TMPL = (
        rf"https?://(?:www\.)?bbc\.co\.uk/iplayer/%s/(?P<id>{BBCCoUkIE._ID_REGEX})"
    )
