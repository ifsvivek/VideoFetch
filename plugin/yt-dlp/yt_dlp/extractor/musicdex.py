    ...
    def _return_info(self, track_json, album_json, video_id):
        return {
            ...
            "release_year": try_get(
                album_json,
                lambda x: date_from_str(unified_strdate(x["release_date"])).year,
            ),
...
class MusicdexArtistIE(MusicdexPageIE):
    ...
    def _real_extract(self, url):
        playlist_id = self._match_id(url)
        data_json = self._download_json(
            f"https://www.musicdex.org/secure/artists/{playlist_id}", playlist_id
        )["artist"]
        entries = []
...
class MusicdexPlaylistIE(MusicdexPageIE):
    ...
    def _real_extract(self, url):
        playlist_id = self._match_id(url)
        data_json = self._download_json(
            f"https://www.musicdex.org/secure/playlists/{playlist_id}", playlist_id
        )["playlist"]
        entries = [
