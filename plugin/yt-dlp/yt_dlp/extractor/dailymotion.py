...
class DailymotionBaseInfoExtractor(InfoExtractor):
    ...
    def _real_initialize(self):
        ...
        self._FAMILY_FILTER = (
            ff == "on" if ff else age_restricted(18, self.get_param("age_limit"))
        )
    ...
    ... on Live {
      ...
      isOnAir
        ...
        if error:
            ...
            raise ExtractorError(f"{self.IE_NAME} said: {title}", expected=True)
    ...
    def _fetch_page(self, playlist_id, page):
      ...
      edges {
        ...
        for edge in videos["edges"]:
            node = edge["node"]
            yield self.url_result(node["url"], DailymotionIE.ie_key(), node["xid"])
...
class DailymotionPlaylistIE(DailymotionPlaylistBaseIE):
    ...
    _VALID_URL = (
        r"(?:https?://)?(?:www\.)?dailymotion\.[a-z]{2,3}/playlist/(?P<id>x[0-9a-z]+)"
    )
    _TESTS = [
        {
...
class DailymotionSearchIE(DailymotionPlaylistBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?dailymotion\.[a-z]{2,3}/search/(?P<id>[^/?#]+)/videos"
    )
    _PAGE_SIZE = 20
    _TESTS = [
        {
