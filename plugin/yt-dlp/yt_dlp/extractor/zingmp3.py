...
class ZingMp3IE(ZingMp3BaseIE):
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "album": traverse_obj(
                item, ("album", ("name", "title")), ("genres", 0, "name"), get_all=False
            ),
...
class ZingMp3ChartMusicVideoIE(ZingMp3BaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._paged_list(song_id, url_type), song_id, f"{url_type}_{regions}"
        )
    ...
    def _real_extract(self, url):
        ...
        if alias == "new-release" and url_type in ("song", "album"):
            ...
            return self.playlist_result(
                self._parse_items(
                    self._call_api(
                        "new-release", params={"type": url_type}, display_id=_id
                    )
                ),
                _id,
            )
...
class ZingMp3HubIE(ZingMp3BaseIE):
    IE_NAME = "zingmp3:hub"
    _VALID_URL = (
        r"https?://(?:mp3\.zing|zingmp3)\.vn/(?P<type>hub)/[^/?#]+/(?P<id>[^./?#]+)"
    )
    _TESTS = [
        {
        ...
        return {
            ...
            **traverse_obj(
                info,
                {
                    "title": "title",
                    "thumbnail": (
                        ("thumbnail", "thumbnailM", "thumbnailV", "thumbnailH"),
                        {url_or_none},
                    ),
...
class ZingMp3PodcastIE(ZingMp3BaseIE):
    ...
    def _real_extract(self, url):
        ...
        if url_type == "podcast-new":
            params["type"] = "new"
        items = self._call_api("cgrs" if url_type == "cgr" else url_type, params)[
            "items"
        ]
        return self.playlist_result(self._parse_items(items), url_type)
