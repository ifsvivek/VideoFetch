...
class NuumTabIE(NuumBaseIE):
    IE_NAME = "nuum:tab"
    _VALID_URL = (
        r"https?://nuum\.ru/channel/(?P<id>[^/#?]+)/(?P<type>streams|videos|clips)"
    )
    _TESTS = [
        {
    ...
    def _fetch_page(self, channel_id, tab_type, tab_id, page):
        ...
        for container in traverse_obj(media_containers, (..., {dict})):
            ...
            yield self.url_result(
                f'https://nuum.ru/videos/{metadata["id"]}', NuumMediaIE, **metadata
            )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            OnDemandPagedList(
                functools.partial(
                    self._fetch_page, channel_data["channel_id"], tab_type, tab_id
                ),
                self._PAGE_SIZE,
            ),
            playlist_id=tab_id,
