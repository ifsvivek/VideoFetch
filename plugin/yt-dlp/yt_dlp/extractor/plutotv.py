...
class PlutoTVIE(InfoExtractor):
    ...
    def _to_ad_free_formats(self, video_id, formats, subtitles):
        ...
        for fmt in formats:
            res = self._download_webpage(
                fmt.get("url"), video_id, note="Downloading m3u8 playlist", fatal=False
            )
            if not res:
                continue
    ...
    def _real_extract(self, url):
        ...
        video_json = self._download_json(
            self._INFO_URL + info_slug, info_slug, query=self._INFO_QUERY_PARAMS
        )
            ...
            for season in video_json["seasons"]:
                ...
                for episode in season["episodes"]:
                    if episode_slug is not None and episode_slug != episode.get("slug"):
                        continue
                    videos.append(
                        self._get_video_info(episode, episode_slug, series_name)
                    )
            if not videos:
