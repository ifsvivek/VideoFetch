...
class RTPIE(InfoExtractor):
    ...
    _USER_AGENT = (
        "rtpplay/2.0.66 (pt.rtp.rtpplay; build:2066; iOS 15.8.3) Alamofire/5.9.1"
    )
    _AUTH_TOKEN = None
    ...
    def _extract_formats(self, media_urls, episode_id):
        ...
        for media_url in set(
            traverse_obj(media_urls, (..., {url_or_none}, {self._cleanup_media_url}))
        ):
    ...
    def _extract_from_api(self, program_id, episode_id):
        ...
        for sub_data in traverse_obj(
            asset_urls, ("subtitles", "vtt_list", lambda _, v: url_or_none(v["file"]))
        ):
    ...
    def _extract_from_html(self, url, episode_id):
        ...
        media_urls = traverse_obj(
            re.findall(
                r'(?:var\s+f\s*=|RTPPlayer\({[^}]+file:)\s*({[^}]+}|"[^"]+")', webpage
            ),
            (
                -1,
                (
