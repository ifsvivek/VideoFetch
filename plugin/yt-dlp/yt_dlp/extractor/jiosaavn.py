...
class JioSaavnBaseIE(InfoExtractor):
    ...
    def _extract_song(song_data, url=None):
        info = traverse_obj(
            song_data,
            {
                ...
                "channel_url": (
                    (None, "more_info"),
                    "label_url",
                    {urljoin("https://www.jiosaavn.com/")},
                    any,
                ),
                "release_date": (
                    (None, "more_info"),
                ...
                "language": (
                    "language",
                    {lambda x: ISO639Utils.short2long(x.casefold()) or "und"},
                ),
        ...
        if primary_artists := traverse_obj(
            song_data, ("primary_artists", {lambda x: x.split(", ") if x else None})
        ):
        ...
        for item_data in traverse_obj(
            ...
            (
                *variadic(keys, (str, bytes, dict, set)),
                lambda _, v: v["id"] and v["perma_url"],
            ),
        ):
...
class JioSaavnShowIE(JioSaavnBaseIE):
    ...
    _VALID_URL = (
        JioSaavnBaseIE._URL_BASE_RE + r"/shows/[^/?#]+/(?P<id>[^/?#]{11,})/?(?:$|[?#])"
    )
    _TESTS = [
        {
...
class JioSaavnAlbumIE(JioSaavnBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._yield_items(album_data, "songs"),
            display_id,
...
class JioSaavnShowPlaylistIE(JioSaavnBaseIE):
    ...
    def _entries(self, show_id, season_id, page):
        ...
        yield from self._yield_items(
            page_data, keys=None, parse_func=self._extract_episode
        )
    ...
    def _real_extract(self, url):
        ...
        entries = OnDemandPagedList(
            functools.partial(self._entries, show_id, season_id), self._PAGE_SIZE
        )
        return self.playlist_result(
            entries,
            playlist_id,
            traverse_obj(show_info, ("show", "title", "text", {str})),
        )
...
class JioSaavnArtistIE(JioSaavnBaseIE):
    ...
    def _entries(self, artist_id, first_page):
        for page in itertools.count():
            playlist_data = (
                first_page if not page else self._fetch_page(artist_id, page)
            )
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries(artist_id, first_page),
            artist_id,
