...
class CPACPlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        video_id = self._match_id(url)
        url_lang = (
            "fr" if any(x in url for x in ("/emission?", "/rechercher?")) else "en"
        )
        pl_type, list_type = (
            ("program", "itemList")
        ...
        total_pages = int_or_none(
            try_get(content, lambda x: x["page"][list_type]["totalPages"]), default=1
        )
        for page in range(1, total_pages + 1):
            ...
            for item in (
                try_get(content, lambda x: x["page"][list_type]["item"], list) or []
            ):
                episode_url = urljoin(
        ...
        return self.playlist_result(
            ...
            playlist_description=try_get(
                content, lambda x: x["page"]["program"][f"description_{url_lang}_t"]
            ),
        )
