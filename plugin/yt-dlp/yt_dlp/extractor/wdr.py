...
class WDRIE(InfoExtractor):
    ...
    def _asset_url(self, wdr_id):
        ...
        return "".join(
            ("https:", self.__API_URL_TPL % (wdr_id[: id_len - 4], wdr_id), ".js")
        )
        ...
        for kind, media in media_resource.items():
            ...
            for tag_name, medium_url in media.items():
                ...
                if ext == "m3u8":
                    ...
                    formats.extend(
                        self._extract_smil_formats(medium_url, "stream", fatal=False)
                    )
                else:
                    a_format = {
                        "url": medium_url,
    ...
    def _real_extract(self, url):
        ...
        for mobj in re.finditer(
            ...
            media_link_obj = self._parse_json(
                mobj.group("data"), display_id, transform_source=js_to_json, fatal=False
            )
            if not media_link_obj:
                continue
            jsonp_url = try_get(media_link_obj, lambda x: x["mediaObj"]["url"], str)
            if jsonp_url:
        ...
        if not entries:
            entries = [
                self.url_result(
                    urllib.parse.urljoin(url, mobj.group("href")), ie=WDRPageIE.ie_key()
                )
                for mobj in re.finditer(
