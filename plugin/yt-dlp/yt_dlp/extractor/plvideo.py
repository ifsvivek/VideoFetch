...
class PlVideoIE(InfoExtractor):
    IE_DESC = "Платформа"
    _VALID_URL = (
        r"https?://(?:www\.)?plvideo\.ru/(?:watch\?(?:[^#]+&)?v=|shorts/)(?P<id>[\w-]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        for quality, data in traverse_obj(
            video_data,
            ("item", "profiles", {dict.items}, lambda _, v: url_or_none(v[1]["hls"])),
        ):
            formats.append(
                {
                    "format_id": quality,
        ...
        if livestream_url := traverse_obj(
            ...
            is_live = True
            formats.extend(
                self._extract_m3u8_formats(livestream_url, video_id, "mp4", live=True)
            )
        for lang, url in traverse_obj(
            video_data,
            ("item", "subtitles", {dict.items}, lambda _, v: url_or_none(v[1])),
        ):
