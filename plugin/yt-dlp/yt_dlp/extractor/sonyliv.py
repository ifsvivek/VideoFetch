    ...
    def _call_api(self, version, path, video_id):
        try:
            ...
            if (
                ...
                and self._parse_json(e.cause.response.read().decode(), video_id)[
                    "message"
                ]
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 403:
                message = self._parse_json(e.cause.response.read().decode(), video_id)[
                    "message"
                ]
...
class SonyLIVSeriesIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?sonyliv\.com/shows/[^/?#&]+-(?P<id>\d{10})/?(?:$|[?#])"
    )
    _TESTS = [
        {
