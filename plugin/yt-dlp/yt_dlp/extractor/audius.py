    ...
    def _api_request(
        ...
        try:
            ...
            if "Failed to parse JSON" in str(exc):
                raise ExtractorError(
                    "An error occurred while receiving data. Try again", expected=True
                )
            raise exc
...
class AudiusTrackIE(AudiusIE):  # XXX: Do not subclass from concrete IE
    _VALID_URL = (
        r"""(?x)(?:audius:)(?:https?://(?:www\.)?.+/v1/tracks/)?(?P<track_id>\w+)"""
    )
...
class AudiusPlaylistIE(AudiusBaseIE):
    ...
    def _build_playlist(self, tracks):
        ...
        for track in tracks:
            ...
            entries.append(
                self.url_result(
                    f"audius:{track_id}", ie=AudiusTrackIE.ie_key(), video_id=track_id
                )
            )
        return entries
...
class AudiusProfileIE(AudiusPlaylistIE):  # XXX: Do not subclass from concrete IE
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._build_playlist(api_call), profile_audius_id, profile_id, profile_bio
        )
