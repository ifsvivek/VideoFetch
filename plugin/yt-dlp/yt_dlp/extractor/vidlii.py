    ...
    def _real_extract(self, url):
        ...
        for source in sources:
            ...
            if self._request_webpage(
                HEADRequest(source), video_id, f"Checking {height}p url", errnote=False
            ):
                formats.append(
                    {
                        "url": source,
        ...
        title = self._search_regex(
            (r"<h1>([^<]+)</h1>", r"<title>([^<]+) - VidLii<"), webpage, "title"
        )
        ...
        upload_date = unified_strdate(
            ...
            or self._search_regex(r"<date>([^<]+)", webpage, "upload date", fatal=False)
        )
