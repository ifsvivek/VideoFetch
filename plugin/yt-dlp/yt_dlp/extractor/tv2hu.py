...
class TV2HuIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_json_url = self._download_json(
            f"https://tv2play.hu/api/streaming-url?playerId={player_id}", video_id
        )["url"]
...
class TV2HuSeriesIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        json_data = self._download_json(
            f"https://tv2play.hu/api/ribbons/{playlist_id}/0?size=100000", playlist_id
        )
        entries = []
        for card in json_data.get("cards", []):
            ...
            if video_id:
                entries.append(
                    self.url_result(f"https://tv2play.hu/{video_id}", TV2HuIE, video_id)
                )
