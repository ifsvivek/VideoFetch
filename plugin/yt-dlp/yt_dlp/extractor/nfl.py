...
class NFLBaseIE(InfoExtractor):
    ...
    def _get_account_info(self):
        ...
        if len(self._ACCOUNT_INFO) != 3:
            raise ExtractorError(
                "Failed to retrieve account info with provided cookies", expected=True
            )
...
class NFLIE(NFLBaseIE):
    IE_NAME = "nfl.com"
    _VALID_URL = (
        NFLBaseIE._VALID_URL_BASE + r"(?:videos?|listen|audio)/(?P<id>[^/#?&]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        return self._parse_video_config(
            self._search_regex(self._VIDEO_CONFIG_REGEX, webpage, "video config"),
            display_id,
        )
...
class NFLPlusReplayIE(NFLBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?nfl\.com/plus/games/(?P<slug>[\w-]+)(?:/(?P<id>\d+))?"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        def entries():
            for replay in traverse_obj(
                ...
                (
                    "items",
                    lambda _, v: v["mcpPlaybackId"] and v["subType"] in requested_types,
                ),
            ):
