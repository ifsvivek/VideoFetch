...
class LocoIE(InfoExtractor):
    ...
    def _get_access_token(self, video_id):
        access_token = try_call(
            lambda: self._get_cookies("https://loco.com")["access_token"].value
        )
        if access_token and not self._is_jwt_expired(access_token):
            return access_token
        access_token = traverse_obj(
            self._download_json(
                "https://api.getloconow.com/v3/user/device_profile/",
                video_id,
                ...
                headers={
                    ...
                    "DEVICE-ID": "".join(random.choices("0123456789abcdef", k=32))
                    + "live",
                    "X-APP-LANG": "en",
                    "X-APP-LOCALE": "en-US",
    ...
    def _real_extract(self, url):
        ...
        stream = traverse_obj(
            self._search_nextjs_v13_data(webpage, video_id),
            (
                ...,
                (None, "ssrData"),
        ...
        if access_token := self._get_access_token(video_id):
            self._request_webpage(
                "https://drm.loco.com/v1/streams/playback/",
                video_id,
                "Downloading video authorization",
                fatal=False,
                headers={
