...
class TenPlayIE(InfoExtractor):
    ...
    def _refresh_access_token(self):
        try:
            refresh_data = self._download_json(
                "https://10.com.au/api/token/refresh",
                None,
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 400:
                ...
                self.cache.store(
                    self._NETRC_MACHINE, self._TOKEN_CACHE_KEY, [None, None]
                )
                self.report_warning(
        ...
        for is_retry in (False, True):
            try:
                return self._download_json_handle(
                    f"https://10.com.au/api/v1/videos/playback/{content_id}/",
                    content_id,
                    note="Downloading video JSON",
                    query={"platform": "samsung"},
                    headers=filter_dict(
                        {
    ...
    def _real_extract(self, url):
        ...
        if (
            not already_have_1080p
            and m3u8_doc
            and re.search(self._SEGMENT_BITRATE_RE, m3u8_doc)
        ):
            m3u8_doc = re.sub(self._SEGMENT_BITRATE_RE, r"-5000000-\1.ts", m3u8_doc)
            m3u8_doc = re.sub(
                r'-(?:300|150|75|55)0000\.key"', r'-5000000.key"', m3u8_doc
            )
            formats.append(
                {
