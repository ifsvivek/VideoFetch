...
class RbgTumCourseIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        course_id, hostname, year, term, slug = self._match_valid_url(url).group(
            "id", "hostname", "year", "term", "slug"
        )
        meta = (
            self._download_json(
        ...
        lectures = [
            self.url_result(f"https://{hostname}/w/{slug}/{stream_id}", RbgTumIE)
...
class RbgTumNewCourseIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            f"https://{hostname}/old/course/{year}/{term}/{slug}", RbgTumCourseIE
        )
