...
class NYTimesArticleIE(NYTimesBaseIE):
    ...
    def _real_extract(self, url):
        ...
        blocks = []
        block_filter = lambda k, v: k == "media" and v["__typename"] in (
            "Video",
            "Audio",
        )
        ...
        common_info = {
            ...
            "creators": traverse_obj(
                art_json, ("bylines", ..., "creators", ..., "displayName", {str})
            ),
            "thumbnails": self._extract_thumbnails(
                traverse_obj(
...
class NYTimesCookingRecipeIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        page_id = self._match_id(url)
        webpage = self._download_webpage(url, page_id)
        recipe_data = self._search_nextjs_data(webpage, page_id)["props"]["pageProps"][
            "recipe"
        ]
