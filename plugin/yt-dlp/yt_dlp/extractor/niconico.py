...
class NiconicoBaseIE(InfoExtractor):
    ...
    def _perform_login(self, username, password):
        ...
        webpage = self._download_webpage(
            f"{self._LOGIN_BASE}/login/redirector",
            None,
            "Logging in",
            "Unable to log in",
            headers={
        ...
        if self.is_logged_in:
            return
        elif err_msg := traverse_obj(
            webpage,
            (
                {find_element(cls="notice error")},
                {find_element(cls="notice__text")},
                {clean_html},
            ),
        ):
            ...
            mfa, urlh = self._download_webpage_handle(
                urljoin(self._LOGIN_BASE, post_url),
                None,
                "Performing MFA",
                "Unable to complete MFA",
                headers={
                    "Content-Type": "application/x-www-form-urlencoded",
                },
                data=urlencode_postdata(
            ...
            if self.is_logged_in:
                ...
                err_msg = traverse_obj(
                    mfa, ({find_element(cls="pageMainMsg")}, {clean_html})
                )
...
class NiconicoIE(NiconicoBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:(?:embed|sp|www)\.)?nicovideo\.jp/watch/(?P<id>(?:[a-z]{2})?\d+)"
    )
    _ERROR_MAP = {
        "FORBIDDEN": {
    ...
    def _extract_formats(self, api_data, video_id):
            ...
            data=json.dumps(
                {
                    "outputs": list(
                        itertools.product(
                            (v["id"] for v in videos), (a["id"] for a in audios)
                        )
                    ),
                }
            ).encode(),
            ...
            formats.append(
                {
                    ...
                    **traverse_obj(
                        audios,
                        (
                            lambda _, v: a_fmt["format_id"].startswith(v["id"]),
                            {
        ...
        v_fmts = sorted(
            (fmt for fmt in raw_fmts if fmt["vcodec"] != "none"), key=lambda f: f["tbr"]
        )
        self._remove_duplicate_formats(v_fmts)
        # Calculate the true vbr/tbr by subtracting the lowest abr
        min_abr = (
            traverse_obj(
                audios, (..., "bitRate", {float_or_none(scale=1000)}, all, {min})
            )
            or 0
        )
        for v_fmt in v_fmts:
            ...
            v_fmt["quality"] = (
                traverse_obj(
                    videos,
                    (
                        lambda _, v: v["id"] == v_fmt["format_id"],
                        "qualityLevel",
                        {int_or_none},
                        any,
                    ),
    ...
    def _real_extract(self, url):
        ...
        api_resp = self._download_json(
            f"{self._BASE_URL}/api/watch/{path}/{video_id}",
            video_id,
        ...
        if status != 200:
            ...
            err_msg = traverse_obj(
                self._ERROR_MAP, (err_code, (reason_code, "DEFAULT"), {str}, any)
            )

            if reason_code in ("DOMESTIC_VIDEO", "HIGH_RISK_COUNTRY_VIDEO"):
                self.raise_geo_restricted(countries=self._GEO_COUNTRIES)
            elif (
                reason_code == "HARMFUL_VIDEO"
                and traverse_obj(
                    api_data,
                    (
                        "viewer",
                ...
                err_msg = (
                    f"This content is scheduled to be released at {scheduled_time}"
                )
            elif reason_code in (
                "CHANNEL_MEMBER_ONLY",
                "HARMFUL_VIDEO",
        ...
        return {
            ...
            "thumbnails": [
                ...
                for key, url in traverse_obj(
                    api_data, ("video", "thumbnail", {dict}), default={}
                ).items()
            ],
            **traverse_obj(
                api_data,
        ...
        for video in traverse_obj(resp, ("items", ..., ("video", None))) or []:
            ...
            yield {
                ...
                "thumbnail": traverse_obj(
                    video, ("thumbnail", ("nHdUrl", "largeUrl", "listingUrl", "url"))
                ),
...
class NicovideoSearchBaseIE(InfoExtractor):
    ...
    def _search_results(self, query):
        return self._entries(
            self._proto_relative_url(f"//www.nicovideo.jp/{self._SEARCH_TYPE}/{query}"),
            query,
        )
...
class NiconicoUserIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?nicovideo\.jp/user/(?P<id>\d+)(?:/video)?/?(?:$|[#?])"
    )
    _TEST = {
...
class NiconicoLiveIE(NiconicoBaseIE):
    ...
    def _real_extract(self, url):
        ...
        webpage, urlh = self._download_webpage_handle(
            url, video_id, expected_status=404
        )
        if err_msg := traverse_obj(
            webpage, ({find_element(cls="message")}, {clean_html})
        ):
            raise ExtractorError(err_msg, expected=True)
        ...
        if age_limit:
            if not self.is_logged_in:
                self.raise_login_required(
                    "Login is required to access age-restricted content"
                )
            ...
            if traverse_obj(
                my,
                (
                    {find_element(id="js-initial-userpage-data", html=True)},
                    {extract_attributes},
                    "data-environment",
                    {json.loads},
                    "allowSensitiveContents",
                ...
                raise ExtractorError(
                    "Sensitive content setting must be enabled", expected=True
                )

        embedded_data = traverse_obj(
            webpage,
            (
        ...
        frontend_id = traverse_obj(
            embedded_data, ("site", "frontendId", {str_or_none}), default="9"
        )
        ...
        for cookie in cookies:
            self._set_cookie(
                ...
                cookie["name"],
                cookie["value"],
                expire_time=unified_timestamp(cookie.get("expires")),
                path=cookie["path"],
                secure=cookie["secure"],
            )

        q_iter = (
            q for q in qualities[1:] if not q.startswith("audio_")
        )  # ignore initial 'abr'
        ...
        return {
            ...
            "age_limit": age_limit,
            "downloader_options": {
                "max_quality": traverse_obj(
                    embedded_data, ("program", "stream", "maxQuality", {str})
                )
                or "normal",
                "ws": ws,
                "ws_url": ws_url,
