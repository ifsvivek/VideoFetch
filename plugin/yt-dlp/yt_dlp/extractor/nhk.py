    ...
    def _extract_stream_info(self, vod_id):
        for refresh in (False, True):
            ...
            stream_url = traverse_obj(
                meta,
                ("movie_url", ("mb_auto", "auto_sp", "auto_pc"), {url_or_none}),
                get_all=False,
            )
...
class NhkVodProgramIE(NhkBaseIE):
    ...
    def _real_extract(self, url):
        lang, m_type, program_id, episode_type = self._match_valid_url(url).group(
            "lang", "type", "id", "episode_type"
        )
...
class NhkForSchoolBangumiIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        base_values = {
            g.group(1): g.group(2)
            for g in re.finditer(r'var\s+([a-zA-Z_]+)\s*=\s*"([^"]+?)";', webpage)
        }
        ...
        chapter_durations = [
            ...
            for g in re.finditer(r"chapterTime\.push\(\'([0-9:]+?)\'\);", webpage)
        ]
        chapter_titles = [
...
class NhkForSchoolProgramListIE(InfoExtractor):
    _VALID_URL = (
        r"https?://www\.nhk\.or\.jp/school/(?P<id>(?:{})/[a-zA-Z0-9_-]+)".format(
...
class NhkRadiruIE(InfoExtractor):
    ...
    _TESTS = [
        ...
        },
        {
            ...
            "expected_warnings": [
                "Failed to download extended metadata: HTTP Error 404: Not Found"
            ],
        },
    ]
    ...
    def _format_description(self, response):
        detailed_description = (
            traverse_obj(response, ("detailedDescription", {dict})) or {}
        )
    ...
    def _get_thumbnails(self, data, keys, name=None, preference=-1):
        thumbnails = []
        for size, thumb in traverse_obj(
            data,
            (
                *variadic(keys, (str, bytes, dict, set)),
                {dict.items},
    ...
    def _extract_extended_metadata(self, episode_id, aa_vinfo):
        ...
        detail_url = try_call(
            lambda: self._API_URL_TMPL.format(
                broadcastEventId=join_nonempty(service, area, date_id)
            )
        )
        if not detail_url:
            return {}
        ...
        for num, dct in enumerate(
            traverse_obj(response, ("about", "eyecatchList", ...))
        ):
            thumbnails.extend(
                self._get_thumbnails(dct, None, join_nonempty("list", num), -2)
            )
        thumbnails.extend(
        ...
        if site_id == "18439M2W42":
            ...
            if headline_id:
                headline = traverse_obj(
                    meta,
                    ("detail_list", lambda _, v: v["headline_id"] == headline_id, any),
                )
                if not headline:
