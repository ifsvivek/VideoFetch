...
class LBRYBaseIE(InfoExtractor):
    ...
    def _call_api_proxy(self, method, display_id, params, resource):
        ...
        token = try_get(
            self._get_cookies("https://odysee.com"), lambda x: x["auth_token"].value
        )
        if token:
    ...
    def _permanent_url(self, url, claim_name, claim_id):
        return urljoin(
            url.replace("lbry://", "https://lbry.tv/"), f"/{claim_name}:{claim_id}"
        )
...
class LBRYChannelIE(LBRYBaseIE):
    ...
    _VALID_URL = (
        LBRYBaseIE._BASE_URL_REGEX + rf"(?P<id>@{LBRYBaseIE._OPT_CLAIM_ID})/?(?:[?&]|$)"
    )
    _TESTS = [
        {
