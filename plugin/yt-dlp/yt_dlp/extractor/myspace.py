...
class MySpaceIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        def formats_from_stream_urls(
            stream_url, hls_stream_url, http_stream_url, width=None, height=None
        ):
            formats = []
        ...
        if is_song:
            ...
            if song_data is None:
                ...
                self.report_warning(f"{video_id}: No downloadable song on this page")
                return
            ...
            video = self._parse_json(
                self._search_regex(r"context = ({.*?});", webpage, "context"), video_id
            )["video"]
            formats = formats_from_stream_urls(
                video.get("streamUrl"),
...
class MySpaceAlbumIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(t_path, ie=MySpaceIE.ie_key()) for t_path in tracks_paths
        ]
        return {
            "_type": "playlist",
            "id": playlist_id,
