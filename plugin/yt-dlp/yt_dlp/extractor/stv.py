...
class STVPlayerIE(InfoExtractor):
    ...
    BRIGHTCOVE_URL_TEMPLATE = (
        "http://players.brightcove.net/1486976045/default_default/index.html?videoId=%s"
    )
    _PTYPE_MAP = {
        "episode": "episodes",
        "video": "shortform",
    ...
    def _real_extract(self, url):
        ...
        props = (
            self._search_nextjs_data(webpage, video_id, default={}).get("props") or {}
        )
        player_api_cache = (
            try_get(props, lambda x: x["initialReduxState"]["playerApiCache"]) or {}
        )
        ...
        return {
            ...
            "id": video_id,
            "url": smuggle_url(
                self.BRIGHTCOVE_URL_TEMPLATE % video_id, {"geo_countries": ["GB"]}
            ),
