...
class NPOIE(InfoExtractor):
    ...
    def suitable(cls, url):
        return (
            False
            if any(
                ie.suitable(url) for ie in (NPOLiveIE, NPORadioIE, NPORadioFragmentIE)
            )
        ...
        if embed_url:
            ...
            if webpage:
                video = self._parse_json(
                    self._search_regex(
                        r"\bvideo\s*=\s*({.+?})\s*;", webpage, "video", default="{}"
                    ),
                    video_id,
                )
                if video:
...
class NPORadioFragmentIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        audio_url = self._search_regex(r"data-streams='([^']+)'", webpage, "audio url")
...
class NPOPlaylistBaseIE(NPOIE):  # XXX: Do not subclass from concrete IE
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(
                f"npo:{video_id}" if not video_id.startswith("http") else video_id
            )
...
class AndereTijdenIE(NPOPlaylistBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?anderetijden\.nl/programma/(?:[^/]+/)+(?P<id>[^/?#&]+)"
    )
    _PLAYLIST_TITLE_RE = (
