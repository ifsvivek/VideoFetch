    ...
    def _extract_webpage_player_entries(self, webpage, playlist_id, base_data):
        ...
        for data_media in re.findall(r'<[^>]+data-media="?({[^>]+})"?', webpage):
            media = self._parse_json(
                data_media, playlist_id, transform_source=unescapeHTML, fatal=False
            )
...
class PolskieRadioLegacyIE(PolskieRadioBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?polskieradio(?:24)?\.pl/\d+/\d+/[Aa]rtykul/(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        description = (
            description.replace("\xa0", " ") if description is not None else None
        )
...
class PolskieRadioCategoryIE(InfoExtractor):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?polskieradio\.pl/(?:\d+(?:,[^/]+)?/|[^/]+/Tag)(?P<id>\d+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        station = next(
            (
                ...
                if s.get("Name") == (channel.get("streamName") or channel.get("name"))
            ),
            None,
        )
        if not station:
        ...
        for stream_url in station["Streams"]:
            ...
            if stream_url.endswith("/playlist.m3u8"):
                formats.extend(
                    self._extract_m3u8_formats(stream_url, channel_url, live=True)
                )
