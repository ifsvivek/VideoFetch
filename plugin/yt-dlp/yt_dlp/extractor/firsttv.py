...
class FirstTVIE(InfoExtractor):
    ...
    def _entries(self, items):
        for item in items:
            ...
            for f in traverse_obj(
                item, ("sources", lambda _, v: url_or_none(v["src"]))
            ):
                src = f["src"]
                ...
                if ext == "m3u8":
                    ...
                    tbr = self._search_regex(
                        rf"_(\d{{3,}})\.{ext}", src, "tbr", default=None
                    )
                    formats.append(
                        {
                            "url": src,
    ...
    def _real_extract(self, url):
        ...
        item_ids = traverse_obj(
            parse_qs(playlist_url), "video_id", "videos_ids[]", "news_ids[]"
        )
        items = traverse_obj(
...
class FirstTVLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        mpd_url = traverse_obj(
            streams_list, ("mpd", ..., {url_or_none}, any, {require("mpd url")})
        )
