    ...
    def _real_extract(self, url):
        ...
        if not upload_date:
            uploaded_ago = self._search_regex(
                r">\s*(\d+[hd])\s+[aA]go\b", webpage, "uploaded ago", default=None
            )
            if uploaded_ago:
        ...
        comment_count = len(
            re.findall(r"""class\s*=\s*['"]media-comment-contents\b""", webpage)
        )
...
class MotherlessPaginatedIE(InfoExtractor):
    ...
    def _extract_entries(self, webpage, base):
        for mobj in re.finditer(
            r'href="[^"]*(?P<href>/[A-F0-9]+)"\s+title="(?P<title>[^"]+)', webpage
        ):
...
class MotherlessGroupIE(MotherlessPaginatedIE):
    _VALID_URL = (
        r"https?://(?:www\.)?motherless\.com/g[vifm]?/(?P<id>[a-z0-9_]+)/?(?:$|[#?])"
    )
    _TESTS = [
        {
...
class MotherlessGalleryIE(MotherlessPaginatedIE):
    _VALID_URL = (
        r"https?://(?:www\.)?motherless\.com/G[VIG]?(?P<id>[A-F0-9]+)/?(?:$|[#?])"
    )
    _TESTS = [
        {
