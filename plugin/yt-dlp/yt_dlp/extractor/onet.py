...
class OnetBaseIE(InfoExtractor):
    ...
    def _extract_from_id(self, video_id, webpage=None):
        ...
        description = (
            self._og_search_description(webpage, default=None) if webpage else None
...
class OnetIE(OnetBaseIE):
    _VALID_URL = (
        OnetBaseIE._URL_BASE_RE + r"[a-z]+/(?P<display_id>[0-9a-z-]+)/(?P<id>[0-9a-z]+)"
    )
    IE_NAME = "onet.tv"
...
class OnetChannelIE(OnetBaseIE):
    ...
    def _real_extract(self, url):
        ...
        entries = [
            self.url_result(video_link, OnetIE.ie_key()) for video_link in matches
        ]
...
class OnetPlIE(InfoExtractor):
    ...
    def _search_mvp_id(self, webpage, default=NO_DEFAULT):
        return self._search_regex(
            r'data-(?:params-)?mvp=["\'](\d+\.\d+)', webpage, "mvp id", default=default
        )
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(f"onetmvp:{mvp_id}", OnetMVPIE.ie_key(), video_id=mvp_id)
