...
class NextMediaIE(InfoExtractor):
    IE_DESC = "蘋果日報"
    _VALID_URL = (
        r"https?://hk\.apple\.nextmedia\.com/[^/]+/[^/]+/(?P<date>\d+)/(?P<id>\d+)"
    )
    _TESTS = [
        {
...
class AppleDailyIE(NextMediaIE):  # XXX: Do not subclass from concrete IE
    ...
    def _fetch_thumbnail(self, page):
        return self._html_search_regex(
            r"setInitialImage\(\'([^']+)'\)", page, "video thumbnail", fatal=False
        )
...
class NextTVIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        view_count = int_or_none(
            remove_start(clean_html(get_element_by_class("click", webpage)), "點閱：")
        )
