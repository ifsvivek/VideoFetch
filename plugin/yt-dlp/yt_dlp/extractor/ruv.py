...
class RuvIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?ruv\.is/(?:sarpurinn/[^/]+|node)/(?P<id>[^/]+(?:/\d+)?)"
    )
    _TESTS = [
        {
            # m3u8
    ...
    def _real_extract(self, url):
        ...
        ) or self._search_regex(FIELD_RE % "poster", webpage, "thumbnail", fatal=False)
...
class RuvSpilaIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "subtitles": subs,
            "thumbnail": episode.get("image", "").replace("$$IMAGESIZE$$", "1960")
            or None,
