...
class NexxIE(InfoExtractor):
    ...
    def _handle_error(self, response):
        if traverse_obj(response, ("metadata", "notice"), expected_type=str):
            self.report_warning(
                "{} said: {}".format(self.IE_NAME, response["metadata"]["notice"])
            )
        status = int_or_none(
    ...
    def _extract_3q_formats(self, video, video_id):
        ...
        for fd in fds:
            ...
            width, height = (
                ss[1].split("x") if len(ss[1].split("x")) == 2 else (None, None)
            )
            f = {
    ...
    def _extract_azure_formats(self, video, video_id):
        ...
        protection_token = try_get(video, lambda x: x["protectiondata"]["token"], str)
        if protection_token:
    ...
    def _real_extract(self, url):
        ...
        response = self._download_json(
            f"https://arc.nexx.cloud/api/video/{video_id}.json", video_id, fatal=False
        )
        ...
        return {
            ...
            "thumbnail": try_get(video, lambda x: x["imagedata"]["thumb"], str),
...
class NexxEmbedIE(InfoExtractor):
    _VALID_URL = (
        r"https?://embed\.nexx(?:\.cloud|cdn\.com)/\d+/(?:video/)?(?P<id>[^/?#&]+)"
    )
