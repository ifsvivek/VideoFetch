...
class TVerIE(StreaksBaseIE):
    ...
    _TESTS = [
        {
            ...
            "info_dict": {
                ...
                "_old_archive_ids": [
                    "brightcovenew ref:baeebeac-a2a6-4dbf-9eb3-c40d59b40068"
                ],
    ...
    def _real_initialize(self):
        ...
        self._STREAKS_API_INFO = self._download_json(
            "https://player.tver.jp/player/streaks_info_v2.json",
            None,
            "Downloading STREAKS API info",
            "Unable to download STREAKS API info",
        )
    ...
    def _real_extract(self, url):
        ...
        if video_type == "series":
            ...
            return self.playlist_from_matches(
                ...
                video_id,
                traverse_obj(
                    series_info, ("result", "content", "content", "title", {str})
                ),
                ie=TVerIE,
        ...
        brightcove_id = traverse_obj(
            video_info, ("video", ("videoRefID", "videoID"), {str}, any)
        )
        ...
        if backend == "brightcove" or not streaks_id:
            if backend != "brightcove":
                self.report_warning(
                    "No STREAKS ID found; falling back to Brightcove extraction",
                    video_id=video_id,
                )
            if not brightcove_id:
                raise ExtractorError(
                    "Unable to extract brightcove reference ID", expected=True
                )
            account_id = traverse_obj(
                ...
                (
                    ...
                    "accountID",
                    {str},
                    {require("brightcove account ID", expected=True)},
                ),
            )
            return {
                **metadata,
        ...
        key_idx = (
            dt.datetime.fromtimestamp(time_seconds(hours=9), dt.timezone.utc).month % 6
            or 6
        )
        ...
        try:
            streaks_info = self._extract_from_streaks_api(
                ...
                streaks_id,
                {
                    "Origin": "https://tver.jp",
                    "Referer": "https://tver.jp/",
                    "X-Streaks-Api-Key": self._STREAKS_API_INFO[project_id]["api_key"][
                        f"key0{key_idx}"
                    ],
                },
            )
            ...
            self.raise_geo_restricted(
                e.orig_msg, countries=self._GEO_COUNTRIES, metadata_available=True
            )
            streaks_info = {}
        ...
        return {
            ...
            **metadata,
            "id": video_id,
            "_old_archive_ids": (
                [make_archive_id("BrightcoveNew", brightcove_id)]
                if brightcove_id
                else None
            ),
        }
