...
class SlidesLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if "domain_not_allowed" in redirect_url:
            domain = traverse_obj(
                parse_qs(redirect_url), ("allowed_domains[]", ...), get_all=False
            )
            if not domain:
        ...
        player_data = self._download_webpage(
            f"https://slideslive.com/player/{video_id}",
            video_id,
            ...
            for slide_id, slide in enumerate(
                ...
                slides_info.append(
                    (
                        ...
                        traverse_obj(
                            slide, ("image", "extname"), default=slide_ext_default
                        ),
        ...
        if not slides and player_info.get("slides_xml_url"):
            ...
            if isinstance(slides, xml.etree.ElementTree.Element):
                slide_url_template = (
                    "https://cdn.slideslive.com/data/presentations/%s/slides/big/%s%s"
                )
        ...
        if service_name == "url":
            ...
            if service_name == "vimeo":
                info["url"] = smuggle_url(
                    f"https://player.vimeo.com/video/{service_id}", {"referer": url}
                )
        ...
        def entries():
            ...
            for slide_id, slide in enumerate(traverse_obj(slides, ("slides", ...)), 1):
                ...
                cdn_hostname = traverse_obj(
                    service_data, (video_path, "video_servers", ...), get_all=False
                )
                ...
                yield {
                    ...
                    "duration": int_or_none(
                        traverse_obj(slide, ("video", "duration_ms")), scale=1000
                    ),
                    "formats": formats,
                }
