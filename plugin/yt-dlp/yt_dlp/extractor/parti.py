    ...
    def _real_extract(self, url):
        ...
        return {
            ...
            "formats": self._extract_m3u8_formats(
                urljoin("https://watch.parti.com", data["livestream_recording"]),
                video_id,
                "mp4",
            ),
            **traverse_obj(
...
class PartiLivestreamIE(PartiBaseIE):
    ...
    _VALID_URL = (
        r"https?://(?:www\.)?parti\.com/creator/(?P<service>[\w]+)/(?P<id>[\w/-]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        creator_id = self._call_api(
            ...
            creator_slug,
            note="Fetching user ID",
        )

        data = self._call_api(
            f"get_livestream_channel_info/{creator_id}",
            creator_id,
            note="Fetching user profile feed",
        )["channel_info"]
