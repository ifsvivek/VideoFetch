...
class ShowRoomLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        room_id = self._search_regex(
            (r"SrGlobal\.roomId\s*=\s*(\d+)", r"(?:profile|room)\?room_id\=(\d+)"),
            webpage,
            "room_id",
        )

        room = self._download_json(
            urljoin(url, f"/api/room/profile?room_id={room_id}"), broadcaster_id
        )
        ...
        streaming_url_list = self._download_json(
            urljoin(url, f"/api/live/streaming_url?room_id={room_id}"), broadcaster_id
        )["streaming_url_list"]
        ...
        for stream in streaming_url_list:
            ...
            if stream_type == "hls":
                m3u8_formats = self._extract_m3u8_formats(
                    stream_url, broadcaster_id, ext="mp4", m3u8_id="hls", live=True
                )
