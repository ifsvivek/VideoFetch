...
class UdemyIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if asset_type != "Video":
            raise ExtractorError(f"Lecture {lecture_id} is not a video", expected=True)
        ...
        def extract_subtitles(track_list):
            ...
            for track in track_list:
                ...
                if not src:
                    continue
                lang = (
                    track.get("language") or track.get("srclang") or track.get("label")
                )
                sub_dict = (
                    automatic_captions
        ...
        if isinstance(captions, list):
            for cc in captions:
                ...
                sub_dict = (
                    automatic_captions if cc.get("source") == "auto" else subtitles
                )
