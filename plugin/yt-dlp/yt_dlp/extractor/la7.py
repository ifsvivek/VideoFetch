        ...
        for f in m3u8_formats:
            if f["vcodec"] != "none" and quality in f["url"]:
                ...
                urlh = self._request_webpage(
                    HEADRequest(http_url), quality, note="Check filesize", fatal=False
                )
                if urlh:
                    http_f = f.copy()
    ...
    def _extract_info(self, webpage, video_id=None, ppn=None):
        if not video_id:
            video_id = self._search_regex(
                r'data-nid=([\'"])(?P<vid>\d+)\1', webpage, "video_id", group="vid"
            )
        ...
        date = self._html_search_regex(
            r'class="data">\s*(?:<span>)?([\d\.]+)\s*</', webpage, "date", default=None
        )
        ...
        ppn = ppn or self._search_regex(
            r'ppN:\s*([\'"])(?P<ppn>.+?)\1', webpage, "ppn", group="ppn", default=None
        )
