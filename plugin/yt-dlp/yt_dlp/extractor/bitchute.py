...
class BitChuteIE(InfoExtractor):
    ...
    def _check_format(self, video_url, video_id):
        urls = orderedSet(
            re.sub(
                r"(^https?://)(seed\d+)(?=\.bitchute\.com)", rf"\g<1>{host}", video_url
            )
            for host in (
                r"\g<2>",
                "seed122",
    ...
    def _real_extract(self, url):
        ...
        if determine_ext(media_url) == "m3u8":
            formats.extend(
                self._extract_m3u8_formats(
                    media_url, video_id, "mp4", m3u8_id="hls", live=True
                )
            )
        else:
        ...
        if channel_id := traverse_obj(video, ("channel", "channel_id", {str})):
            channel = self._call_api(
                "channel", {"channel_id": channel_id}, video_id, fatal=False
            )
        ...
        return {
            **traverse_obj(
                video,
                {
                    ...
                    "channel_url": (
                        "channel",
                        "channel_url",
                        {urljoin("https://www.bitchute.com/")},
                    ),
                    "uploader_id": ("profile_id", {str}),
                    "uploader_url": (
                        "profile_id",
                        {format_field(template=self._UPLOADER_URL_TMPL)},
                        filter,
                    ),
            ...
            **traverse_obj(
                channel,
                {
                    ...
                    "channel_url": (
                        "url_slug",
                        {format_field(template=self._CHANNEL_URL_TMPL)},
                        filter,
                    ),
