...
class FaulioBaseIE(InfoExtractor):
    ...
    _LANGUAGES = ("ar", "en", "fa")
    _BASE_URL_RE = rf'https?://(?:{"|".join(map(re.escape, _DOMAINS))})/(?:(?:{"|".join(_LANGUAGES)})/)?'
...
class FaulioIE(FaulioBaseIE):
    _VALID_URL = rf"{FaulioBaseIE._BASE_URL_RE}(?:episode|media)/(?P<id>[a-zA-Z0-9-]+)"
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        video_info = self._download_json(
            f"{api_base}/video/{video_id}", video_id, fatal=False
        )
        player_info = self._download_json(
            f"{api_base}/video/{video_id}/player", video_id
        )
        ...
        if mpd_url := traverse_obj(
            player_info, ("settings", "protocols", "dash", {url_or_none})
        ):
        ...
        return {
            ...
            **traverse_obj(
                traverse_obj(video_info, ("blocks", 0)),
                {
                    "display_id": ("slug", {str}),
                    "title": ("title", {str}),
                    "episode": ("title", {str}),
...
class FaulioLiveIE(FaulioBaseIE):
    _VALID_URL = rf"{FaulioBaseIE._BASE_URL_RE}live/(?P<id>[a-zA-Z0-9-]+)"
    _TESTS = [
        {
