...
class S4CIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        for sub in traverse_obj(
            player_config, ("subtitles", lambda _, v: url_or_none(v["0"]))
        ):
        ...
        m3u8_url = self._download_json(
            ...
            note="Downloading streaming urls JSON",
        )["hls"]
...
class S4CSeriesIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        series_details = self._download_json(
            "https://www.s4c.cymru/df/series_details",
            series_id,
            query={
                "lang": "e",
                "series_id": series_id,
        ...
        return self.playlist_result(
            [
                self.url_result(
                    f"https://www.s4c.cymru/clic/programme/{episode_id}",
                    S4CIE,
                    episode_id,
                )
                for episode_id in traverse_obj(
                    series_details, ("other_progs_in_series", ..., "id")
                )
            ],
            series_id,
            traverse_obj(
                series_details, ("full_prog_details", 0, "series_title", {str})
            ),
        )
