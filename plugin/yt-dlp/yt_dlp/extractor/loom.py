...
class LoomIE(InfoExtractor):
    ...
    def _extract_formats(self, video_id, metadata, gql_data):
        ...
        cdn_url = get_first(
            gql_data, ("data", "getVideo", "nullableRawCdnUrl", "url", {url_or_none})
        )
        # cdn_url is usually a dupe, but the raw-url/transcoded-url endpoints could return errors
        valid_urls = [
            update_url(url, query=None) for url in (raw_url, transcoded_url) if url
        ]
    ...
    def _real_extract(self, url):
        ...
        metadata = (
            get_first(
                self._call_graphql_api(
                    "GetVideoSSR", video_id, "Downloading GraphQL metadata JSON"
                ),
                ("data", "getVideo", {dict}),
            )
            or {}
        ...
        return {
            ...
            "chapters": self._extract_chapters_from_description(
                get_first(gql_data, ("data", "fetchVideoChapters", "content", {str})),
                duration,
            )
            or None,
...
class LoomFolderIE(InfoExtractor):
    ...
    def _extract_folder_entries(self, folder_id, initial_folder_data=None):
        ...
        for subfolder_id in traverse_obj(
            folder_data, ("folders", lambda _, v: v["id"] != folder_id, "id", {str})
        ):
