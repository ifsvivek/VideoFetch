...
class TelemundoIE(InfoExtractor):
    _WORKING = False
    _VALID_URL = (
        r"https?:\/\/(?:www\.)?telemundo\.com\/.+?video\/[^\/]+(?P<id>tmvo\d{7})"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        redirect_url = try_get(
            metadata,
            lambda x: x["props"]["initialState"]["video"]["associatedPlaylists"][0][
                "videos"
            ][0]["videoAssets"][0]["publicUrl"],
        )
