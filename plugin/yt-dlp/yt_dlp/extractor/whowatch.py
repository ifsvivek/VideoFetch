    ...
    def _real_extract(self, url):
        ...
        if not hls_url:
            raise ExtractorError(
                live_data.get("error_message") or "The user is offline.", expected=True
            )
        ...
        formats.extend(
            self._extract_m3u8_formats(hls_url, video_id, ext="mp4", m3u8_id="hls")
        )
        ...
        timestamp = int_or_none(
            try_get(metadata, lambda x: x["live"]["started_at"], int), scale=1000
        )
