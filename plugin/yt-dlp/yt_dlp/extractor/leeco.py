...
class LePlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            entries, playlist_id, playlist_title=title, playlist_description=description
        )
        ...
        if not play_json.get("data"):
            if play_json.get("message"):
                raise ExtractorError(
                    "Letv cloud said: {}".format(play_json["message"]), expected=True
                )
            elif play_json.get("code"):
                raise ExtractorError(
                    "Letv cloud returned error %d" % play_json["code"], expected=True
                )
            else:
