...
class FloatplaneBaseIE(InfoExtractor):
    def _real_extract(self, url):
        ...
        uploader_url = (
            format_field(
                post_data, [("creator", "urlname")], f"{self._BASE_URL}/channel/%s/home"
            )
            or None
        )
        ...
        common_info = {
            ...
            "channel_url": urljoin(
                f"{uploader_url}/", traverse_obj(post_data, ("channel", "urlname"))
            ),
        ...
        for media in traverse_obj(
            ...
            stream = self._download_json(
                f"{self._BASE_URL}/api/v3/delivery/info",
                media_id,
            ...
            metadata = self._download_json(
                ...
                media_id,
                f"Downloading {media_typ} metadata",
                query={"id": media_id},
                fatal=False,
                impersonate=self._IMPERSONATE_TARGET,
            )

            cdn_base_url = traverse_obj(
                stream,
                (
                    "groups",
                    0,
                    ...
                    {url_or_none},
                    any,
                    {require("cdn base url")},
                ),
            )

            formats = []
            for variant in traverse_obj(
                stream, ("groups", 0, "variants", lambda _, v: v["url"])
            ):
                ...
                if self._IMPERSONATE_TARGET is not None:
                    m3u8_data = self._download_webpage(
                        ...
                        note=join_nonempty(
                            "Downloading", format_id, "m3u8 information", delim=" "
                        ),
                        errnote=join_nonempty(
                            "Failed to download",
                            format_id,
                            "m3u8 information",
                            delim=" ",
                    ...
                    if key_url:
                        urlh = self._request_webpage(
                            ...
                            headers=self._HEADERS,
                            note=join_nonempty(
                                "Downloading", format_id, "HLS AES key", delim=" "
                            ),
                            errnote=join_nonempty(
                                "Failed to download",
                                format_id,
                ...
                formats.append(
                    {
                        **traverse_obj(
                        ...
                        "url": format_url,
                        "ext": determine_ext(
                            format_url.partition("/chunk.m3u8")[0], "mp4"
                        ),
                        "format_id": format_id,
            ...
            for sub_data in traverse_obj(
                metadata, ("textTracks", lambda _, v: url_or_none(v["src"]))
            ):
