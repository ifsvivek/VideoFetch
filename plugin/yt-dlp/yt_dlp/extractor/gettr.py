...
class GettrBaseIE(InfoExtractor):
    ...
    def _call_api(self, path, video_id, *args, **kwargs):
        return self._download_json(
            urljoin("https://api.gettr.com/u/", path), video_id, *args, **kwargs
        )["result"]
...
class GettrIE(GettrBaseIE):
    ...
    def _real_extract(self, url):
        ...
        user_data = (
            try_get(api_data, lambda x: x["aux"]["uinf"][post_data["uid"]], dict) or {}
        )
...
class GettrStreamingIE(GettrBaseIE):
    ...
    def _real_extract(self, url):
        ...
        thumbnails = [
            ...
            for thumbnail in try_get(video_info, lambda x: x["postData"]["imgs"], list)
            or []
        ]
