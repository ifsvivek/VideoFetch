...
class VidyardBaseIE(InfoExtractor):
    ...
    def _get_additional_metadata(self, video_id):
        additional_metadata = self._download_json(
            f"https://play.vidyard.com/video/{video_id}",
            video_id,
            note="Downloading additional metadata",
            fatal=False,
        )
        return traverse_obj(
            additional_metadata,
            {
                ...
                "chapters": (
                    "videoSections",
                    lambda _, v: float_or_none(v["milliseconds"]) is not None,
                    {
                        "title": ("title", {str}),
    ...
    def _process_video_json(self, json_data, video_id):
        ...
        self._merge_subtitles(
            self._get_direct_subtitles(json_data.get("captions")), target=subtitles
        )
