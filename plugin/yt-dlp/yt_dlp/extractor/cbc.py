...
class CBCPlayerIE(InfoExtractor):
    ...
    def _parse_param(self, asset_data, name):
        return traverse_obj(
            asset_data, ("params", lambda _, v: v["name"] == name, "value", {str}, any)
        )
            ...
            if ext == "m3u8":
                ...
                best_video_fmt = (
                    traverse_obj(
                        fmts,
                        (
                            lambda _, v: v.get("vcodec") != "none" and v["tbr"],
                            all,
                            {functools.partial(sorted, key=lambda x: x["tbr"])},
                            -1,
                            {dict},
                        ),
                    )
                ...
                formats.append(
                    {
                        ...
                        "vcodec": (
                            "none"
                            if self._parse_param(asset_data, "mediaType") == "audio"
                            else None
                        ),
                    }
                )
...
class CBCGemIE(CBCGemBaseIE):
    ...
    def _call_oauth_api(self, oauth_data, note="Refreshing access token"):
        ...
        self.cache.store(
            self._NETRC_MACHINE, "token_data", [self._refresh_token, self._access_token]
        )
...
class CBCGemLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        video_info = self._search_nextjs_data(webpage, video_id)["props"]["pageProps"][
            "data"
        ]
                ...
                video_info = (
                    traverse_obj(
                        ...
                        (
                            ("freeTv", ("streams", ...)),
                            "items",
                            lambda _, v: v["key"].partition("-")[0] == video_id,
                            any,
                        ),
                    )
                    or {}
    ...
    def _real_extract(self, url):
        ...
        if not data:
            ...
            data = traverse_obj(
                preloaded_state,
                (
                    ("podcastDetailData", "showDetailData"),
                    ...,
                    "episodes",
                    lambda _, v: str(v["clipID"]) == video_id,
                    any,
                    {require("episode data")},
                ),
            )
