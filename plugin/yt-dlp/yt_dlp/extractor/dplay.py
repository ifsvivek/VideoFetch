    ...
    def _get_auth(self, disco_base, display_id, realm, needs_device_id=True):
        ...
        if not token:
            ...
            token = self._download_json(
                disco_base + "token", display_id, "Downloading token", query=query
...
class TravelChannelIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:watch\.)?travelchannel\.com/video" + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class CookingChannelIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:watch\.)?cookingchanneltv\.com/video" + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class FoodNetworkIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:watch\.)?foodnetwork\.com/video" + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class DestinationAmericaIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?destinationamerica\.com/video" + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class ScienceChannelIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?sciencechannel\.com/video" + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class DiscoveryLifeIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?discoverylife\.com/video" + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class DiscoveryPlusIndiaIE(DiscoveryPlusBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?discoveryplus\.in/videos?" + DPlayBaseIE._PATH_REGEX
    )
    _TESTS = [
        {
...
class DiscoveryNetworksDeIE(DiscoveryPlusBaseIE):
    ...
    def _real_extract(self, url):
        ...
        disco_api_info["categories"] = traverse_obj(
            meta,
            (
                "taxonomies",
                lambda _, v: v["category"] == "genre",
                "title",
                {str.strip},
                filter,
                all,
    ...
    def _entries(self, show_name):
        ...
        for season in show_json["filters"][0]["options"]:
            ...
            while page_num < total_pages:
                ...
                if page_num == 0:
                    total_pages = (
                        try_get(season_json, lambda x: x["meta"]["totalPages"], int)
                        or 1
                    )
...
class DiscoveryPlusIndiaShowIE(DiscoveryPlusShowBaseIE):
    _VALID_URL = (
        r"https?://(?:www\.)?discoveryplus\.in/show/(?P<show_name>[^/]+)/?(?:[?#]|$)"
    )
    _TESTS = [
        {
