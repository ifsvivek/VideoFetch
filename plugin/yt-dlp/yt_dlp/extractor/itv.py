...
class ITVIE(InfoExtractor):
    ...
    def _call_api(
        self, video_id, playlist_url, headers, platform_tag, featureset, fatal=True
    ):
    ...
    def _get_subtitles(
        ...
        platform_tag_subs, featureset_subs = next(
            (
                ...
                for platform_tag, featuresets in reversed(list(variants.items()))
        ...
        if platform_tag_subs and featureset_subs:
            ...
            subs = (
                try_get(
                    subs_playlist, lambda x: x["Playlist"]["Video"]["Subtitles"], list
                )
                or []
            )
            for sub in subs:
    ...
    def _real_extract(self, url):
        ...
        params = extract_attributes(
            self._search_regex(r'(?s)(<[^>]+id="video"[^>]*>)', webpage, "params")
        )
                ...
                [
                    {
                        "url": thumbnail_url.format(
                            width=1920, height=1080, quality=100, blur=0, bg="false"
                        ),
                        "width": 1920,
                        "height": 1080,
                    },
                    {
                        "url": urljoin(
                            base_url(thumbnail_url), url_basename(thumbnail_url)
                        ),
                        "preference": -2,
                    },
                ]
...
class ITVBTCCIE(InfoExtractor):
    _VALID_URL = (
        r"https?://(?:www\.)?itv\.com/(?:news|btcc)/(?:[^/]+/)*(?P<id>[^/?#&]+)"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        json_map = (
            try_get(
                ...
                lambda x: x["props"]["pageProps"]["article"]["body"]["content"],
            )
            or []
        )
        ...
        for video in json_map:
            if not any(
                traverse_obj(video, ("data", attr)) == "Brightcove"
