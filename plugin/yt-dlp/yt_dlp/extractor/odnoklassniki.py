...
class OdnoklassnikiIE(InfoExtractor):
    ...
    def _extract_desktop(self, url):
        ...
        webpage = self._download_webpage(
            ...
            headers=(
                {"Referer": smuggled["referrer"]} if smuggled.get("referrer") else {}
            ),
        )
        ...
        if (
            error == "The author of this video has not been found or is blocked"
        ...
        if dash_manifest:
            formats.extend(
                self._parse_mpd_formats(compat_etree_fromstring(dash_manifest), "mpd")
            )
        ...
        if not formats:
            ...
            if payment_info:
                self.raise_no_formats(
                    "This video is paid, subscribe to download it", expected=True
                )
    ...
    def _extract_mobile(self, url):
        ...
        webpage = self._download_webpage(
            f"https://m.ok.ru/video/{video_id}",
            video_id,
        ...
        json_data = self._search_regex(r'data-video="(.+?)"', webpage, "json data")
        json_data = self._parse_json(unescapeHTML(json_data), video_id) or {}

        redirect_url = self._request_webpage(
            HEADRequest(json_data["videoSrc"]), video_id, "Requesting download URL"
        ).url
