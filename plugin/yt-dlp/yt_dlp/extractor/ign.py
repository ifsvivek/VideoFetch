...
class IGNBaseIE(InfoExtractor):
    ...
    def _extract_video_info(self, video, fatal=True):
        ...
        if f4m_url:
            formats.extend(
                self._extract_f4m_formats(f4m_url, video_id, f4m_id="hds", fatal=False)
            )
...
class IGNVideoIE(IGNBaseIE):
    _VALID_URL = (
        r"https?://.+?\.ign\.com/(?:[a-z]{2}/)?[^/]+/(?P<id>\d+)/(?:video|trailer)/"
    )
    _TESTS = [
        {
    ...
    def _real_extract(self, url):
        ...
        new_url = urlh.url
        ign_url = urllib.parse.parse_qs(urllib.parse.urlparse(new_url).query).get(
            "url", [None]
        )[-1]
        if ign_url:
...
class IGNArticleIE(IGNBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            ...
            re.sub(r"\s+-\s+IGN\s*$", "", self._og_search_title(webpage, default=""))
            or None,
        )
