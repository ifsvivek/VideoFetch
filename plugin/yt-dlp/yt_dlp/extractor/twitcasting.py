...
class TwitCastingIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        duration = try_get(
            ...
            clean_html(get_element_by_class("tw-player-duration-time", webpage))
        )
        view_count = str_to_int(
            self._search_regex(
                (r"Total\s*:\s*Views\s*([\d,]+)", r"総視聴者\s*:\s*([\d,]+)\s*</"),
                webpage,
                "views",
                None,
            )
        ...
        timestamp = unified_timestamp(
            self._search_regex(
                r'data-toggle="true"[^>]+datetime="([^"]+)"', webpage, "datetime", None
            )
        )

        is_live = any(
            f"data-{x}" in webpage
            for x in ['is-onlive="true"', 'live-type="live"', 'status="online"']
        )
        ...
        m3u8_urls = try_get(webpage, find_dmu, list) or traverse_obj(
            video_js_data, (..., "source", "url")
        )
        ...
        if is_live:
            ...
            for quality, m3u8_url in traverse_obj(
                stream_data,
                (
                    "tc-hls",
                    "streams",
...
class TwitCastingLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(
            f"https://twitcasting.tv/{uploader_id}/movie/{current_live}", TwitCastingIE
        )
