    ...
    def _real_extract(self, url):
        ...
        try:
            ...
            if isinstance(e.cause, HTTPError) and e.cause.status == 500:
                raise ExtractorError(
                    "VR SQUARE app-only videos are not supported", expected=True
                )
            raise
        ...
        return {
            ...
            "formats": self._extract_m3u8_formats(
                traverse_obj(web_api, ("urls", ..., "url", any)),
                video_id,
                "mp4",
                fatal=False,
            ),
            ...
            **traverse_obj(
                ...
                {
                    "duration": (
                        {find_element(cls="layout-product-data-time")},
                        {parse_duration},
                    ),
...
class VrSquarePlaylistBaseIE(InfoExtractor):
    ...
    def _fetch_vids(self, source, keys=()):
        for url_path in traverse_obj(
            ...
            (
                *keys,
                {find_elements(cls="video", html=True)},
                ...,
                {extract_attributes},
                "data-url",
                {str},
            ...
            yield self.url_result(
                f'{self._BASE_URL}/contents/{url_path.removeprefix("/contents/")}',
                VrSquareIE,
            )
...
class VrSquareSearchIE(VrSquarePlaylistBaseIE):
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            self._entries("/ajax/web-search", search_query, {"w": search_query}),
            search_query,
        )
