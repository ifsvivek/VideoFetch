...
class MuxIE(InfoExtractor):
    ...
    _EMBED_REGEX = [
        r'<iframe\b[^>]+\bsrc=["\'](?P<url>(?:https?:)?//(?:stream\.new/v|player\.mux\.com)/(?P<id>[A-Za-z0-9-]+)[^"\']+)'
    ]
    _TESTS = [
        {
    ...
    def _extract_embed_urls(cls, url, webpage):
        ...
        for mux_player in re.findall(
            r"<mux-(?:player|video)\b[^>]*\bplayback-id=[^>]+>", webpage
        ):
            attrs = extract_attributes(mux_player)
            playback_id = attrs.get("playback-id")
            if not playback_id:
                continue
            token = attrs.get("playback-token") or traverse_obj(
                playback_id, ({parse_qs}, "token", -1)
            )
    ...
    def _real_extract(self, url):
        ...
        formats, subtitles = self._extract_m3u8_formats_and_subtitles(
            f"https://stream.mux.com/{video_id}.m3u8",
            video_id,
            "mp4",
