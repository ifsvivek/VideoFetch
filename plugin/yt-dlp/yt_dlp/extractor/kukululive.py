...
class KukuluLiveIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        title = clean_html(
            get_element_by_id(
                "livetitle", html.replace("<SPAN", "<span").replace("SPAN>", "span>")
            )
        )
        ...
        if self._search_regex(
            r"(var\s+timeshift\s*=\s*false)", html, "is livestream", default=False
        ):
            formats = []
            for desc, code in [("high", "Z"), ("low", "ForceLow")]:
                ...
                if (
                    ...
                    self._add_quality_formats(
                        formats,
                        self._get_quality_meta(video_id, desc, code, force_h264="1"),
                    )
