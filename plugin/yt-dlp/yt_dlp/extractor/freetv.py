...
class FreeTvMoviesIE(FreeTvBaseIE):
    ...
    def _extract_video(self, content_id, action="olyott_video_play"):
        ...
        video_id, video_url = (
            api_response["displayMeta"]["contentID"],
            api_response["displayMeta"]["streamURLVideo"],
        )
...
class FreeTvIE(FreeTvBaseIE):
    ...
    def _extract_series_season(self, season_id, series_title):
        episodes = self._get_api_response(
            season_id,
            "series",
            {
                "contentID": season_id,
        ...
        for episode in episodes:
            ...
            yield {
                ...
                "series_id": traverse_obj(
                    episode, ("contentMeta", "displayMeta", "seriesID")
                ),
                "season_id": traverse_obj(
                    episode, ("contentMeta", "displayMeta", "seasonID")
                ),
                "season_number": traverse_obj(
                    episode,
                    ("contentMeta", "displayMeta", "seasonNum"),
                    expected_type=int_or_none,
                ),
                "episode_number": traverse_obj(
                    episode,
                    ("contentMeta", "displayMeta", "episodeNum"),
                    expected_type=int_or_none,
                ),
            }
    ...
    def _real_extract(self, url):
        ...
        return self.playlist_result(
            itertools.chain.from_iterable(
                ...
                for season_id in re.findall(
                    r'<option[^>]+value=["\'](\d+)["\']', webpage
                )
            ),
            display_id,
            title,
