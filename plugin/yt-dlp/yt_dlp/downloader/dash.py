...
class DashSegmentsFD(FragmentFD):
    ...
    def real_download(self, filename, info_dict):
        ...
        requested_formats = [
            {**info_dict, **fmt} for fmt in info_dict.get("requested_formats", [])
        ]
        args = []
        for fmt in requested_formats or [info_dict]:
            ...
            if isinstance(fmt["fragments"], str):
                raise ReExtractInfo(
                    "the stream needs to be re-extracted", expected=True
                )
            ...
            ctx = {
                ...
                "live": (
                    "is_from_start" if fmt.get("is_from_start") else fmt.get("is_live")
                ),
