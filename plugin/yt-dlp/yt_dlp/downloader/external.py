...
class ExternalFD(FragmentFD):
    ...
    def _bool_option(
        ...
        return cli_bool_option(
            self.params, command_option, param, true_value, false_value, separator
        )
...
class Aria2cFD(ExternalFD):
    ...
    def supports_manifest(manifest):
        ...
        check_results = (
            not re.search(feature, manifest) for feature in UNSUPPORTED_FEATURES
        )
    ...
    def _make_cmd(self, tmpfilename, info_dict):
        ...
        if "fragments" in info_dict:
            ...
            for frag_index, fragment in enumerate(info_dict["fragments"]):
                ...
                url_list.append(
                    "{}\n\tout={}".format(
                        fragment["url"], self._aria2c_filename(fragment_filename)
                    )
                )
                ...
                status.update(
                    {
                        ...
                        "fragment_index": (
                            min(frag_count, len(completed) + 1) if fragmented else None
                        ),
                        "elapsed": time.time() - started,
                    }
                )
...
class FFmpegFD(ExternalFD):
    ...
    def _call_downloader(self, tmpfilename, info_dict):
        ...
        start_time, end_time = info_dict.get("section_start") or 0, info_dict.get(
            "section_end"
        )

        fallback_input_args = traverse_obj(
        ...
        for i, fmt in enumerate(selected_formats):
            ...
            cookies = (
                self.ydl.cookiejar.get_cookies_for_url(fmt["url"]) if is_http else []
            )
            if cookies:
                args.extend(
                    [
            ...
            if self.params.get("enable_file_urls") and url.startswith("file:"):
                ...
                url = re.sub(
                    r"^file://(?:localhost)?/",
                    "file:" if os.name == "nt" else "file:/",
                    url,
                )

            args += (
                traverse_obj(fmt, ("downloader_options", "ffmpeg_args", ...))
                or fallback_input_args
            )
        ...
        if protocol in ("m3u8", "m3u8_native"):
            ...
            if use_mpegts:
                ...
                args += ["-f", "mp4"]
                if (
                    ffpp.basename == "ffmpeg" and ffpp._features.get("needs_adtstoasc")
                ) and (
